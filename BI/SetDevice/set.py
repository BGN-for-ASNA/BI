...
def setup_device(
    platform="cpu", cores=None, deallocate=False, print_devices_found=True
):
    ...
    os.environ["XLA_FLAGS"] = " ".join(
        ["--xla_force_host_platform_device_count={}".format(int(cores))] + xla_flags
    )
    ...
    if print_devices_found:
        try:
            print("jax.local_device_count", jax.local_device_count(backend=platform))
        except RuntimeError as e:
            import warnings

            warnings.warn(
                f"Requested platform '{platform}' is not available: {e}\n"
                "Falling back to CPU.",
                RuntimeWarning,
                stacklevel=2,
            )
            ...
            print(
                "jax.local_device_count (cpu fallback)",
