...
class UnifiedDist:
    ...
    @staticmethod
    def asymmetriclaplace(loc=0.0, scale=1.0, asymmetry=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.AsymmetricLaplace(loc=loc, scale=scale, asymmetry=asymmetry, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.AsymmetricLaplace(loc=loc, scale=scale, asymmetry=asymmetry, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def asymmetriclaplacequantile(loc=0.0, scale=1.0, quantile=0.5, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.AsymmetricLaplaceQuantile(loc=loc, scale=scale, quantile=quantile, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.AsymmetricLaplaceQuantile(loc=loc, scale=scale, quantile=quantile, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def bernoulli(probs=None, logits=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Bernoulli(probs=probs, logits=logits, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Bernoulli(probs=probs, logits=logits, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def bernoullilogits(logits=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.BernoulliLogits(logits=logits, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.BernoulliLogits(logits=logits, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.BernoulliLogits(logits=logits, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def bernoulliprobs(probs, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.BernoulliProbs(probs=probs, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.BernoulliProbs(probs=probs, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.BernoulliProbs(probs=probs, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def beta(concentration1, concentration0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        Beta distribution.
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Beta(concentration1=concentration1, concentration0=concentration0, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Beta(concentration1=concentration1, concentration0=concentration0, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def betabinomial(concentration1, concentration0, total_count=1, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.BetaBinomial(concentration1=concentration1, concentration0=concentration0, total_count=total_count, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.BetaBinomial(concentration1=concentration1, concentration0=concentration0, total_count=total_count, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def betaproportion(mean, concentration, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.BetaProportion(mean=mean, concentration=concentration, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.BetaProportion(mean=mean, concentration=concentration, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def binomial(total_count=1, probs=None, logits=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Binomial(total_count=total_count, probs=probs, logits=logits, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Binomial(total_count=total_count, probs=probs, logits=logits, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def binomiallogits(logits, total_count=1, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.BinomialLogits(logits=logits, total_count=total_count, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.BinomialLogits(logits=logits, total_count=total_count, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def binomialprobs(probs, total_count=1, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.BinomialProbs(probs=probs, total_count=total_count, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.BinomialProbs(probs=probs, total_count=total_count, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def car(loc, correlation, conditional_precision, adj_matrix, is_sparse=False, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        CAR distribution.
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.CAR(loc=loc, correlation=correlation, conditional_precision=conditional_precision, adj_matrix=adj_matrix, is_sparse=is_sparse, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.CAR(loc=loc, correlation=correlation, conditional_precision=conditional_precision, adj_matrix=adj_matrix, is_sparse=is_sparse, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def categorical(probs=None, logits=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Categorical(probs=probs, logits=logits, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Categorical(probs=probs, logits=logits, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def categoricallogits(logits, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.CategoricalLogits(logits=logits, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.CategoricalLogits(logits=logits, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def categoricalprobs(probs, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.CategoricalProbs(probs=probs, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.CategoricalProbs(probs=probs, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.CategoricalProbs(probs=probs, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def cauchy(loc=0.0, scale=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        Cauchy distribution.
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Cauchy(loc=loc, scale=scale, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.Cauchy(loc=loc, scale=scale, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.Cauchy(loc=loc, scale=scale, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def chi2(df, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        Chi2 distribution.
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Chi2(df=df, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.Chi2(df=df, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.Chi2(df=df, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def circulantnormal(loc: jax.Array, covariance_row: jax.Array = None, covariance_rfft: jax.Array = None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        CirculantNormal distribution.
        ...
        if obs != None:
            if mask is None:
                return numpyro.sample(name, numpyro.distributions.CirculantNormal(loc=loc, covariance_row=covariance_row, covariance_rfft=covariance_rfft, validate_args=validate_args),obs)
            else:
                with numpyro.handlers.mask(mask=mask): 
                    return numpyro.sample(name, numpyro.distributions.CirculantNormal(loc=loc, covariance_row=covariance_row, covariance_rfft=covariance_rfft, validate_args=validate_args),obs)
        else: 
            if sample== True:
                if mask is None:
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.CirculantNormal(loc=loc, covariance_row=covariance_row, covariance_rfft=covariance_rfft, validate_args=validate_args).sample(seed, shape)
                else:
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.CirculantNormal(loc=loc, covariance_row=covariance_row, covariance_rfft=covariance_rfft, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.CirculantNormal(loc=loc, covariance_row=covariance_row, covariance_rfft=covariance_rfft, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.CirculantNormal(loc=loc, covariance_row=covariance_row, covariance_rfft=covariance_rfft, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def delta(v=0.0, log_density=0.0, event_dim=0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        Delta distribution.
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Delta(v=v, log_density=log_density, event_dim=event_dim, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Delta(v=v, log_density=log_density, event_dim=event_dim, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def dirichlet(concentration, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Dirichlet(concentration=concentration, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Dirichlet(concentration=concentration, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def dirichletmultinomial(concentration, total_count=1, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.DirichletMultinomial(concentration=concentration, total_count=total_count, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.DirichletMultinomial(concentration=concentration, total_count=total_count, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def discreteuniform(low=0, high=1, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.DiscreteUniform(low=low, high=high, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.DiscreteUniform(low=low, high=high, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def distribution(batch_shape=(), event_shape=(), validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Distribution(batch_shape=batch_shape, event_shape=event_shape, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Distribution(batch_shape=batch_shape, event_shape=event_shape, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def doublytruncatedpowerlaw(alpha, low, high, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.DoublyTruncatedPowerLaw(alpha=alpha, low=low, high=high, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.DoublyTruncatedPowerLaw(alpha=alpha, low=low, high=high, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def eulermaruyama(t, sde_fn, init_dist, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.EulerMaruyama(t=t, sde_fn=sde_fn, init_dist=init_dist, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.EulerMaruyama(t=t, sde_fn=sde_fn, init_dist=init_dist, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def expandeddistribution(base_dist, batch_shape=(), shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.ExpandedDistribution(base_dist=base_dist, batch_shape=batch_shape).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.ExpandedDistribution(base_dist=base_dist, batch_shape=batch_shape).expand(shape).mask(mask))

    @staticmethod
    def exponential(rate=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Exponential(rate=rate, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.Exponential(rate=rate, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.Exponential(rate=rate, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def foldeddistribution(base_dist, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.FoldedDistribution(base_dist=base_dist, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.FoldedDistribution(base_dist=base_dist, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def gamma(concentration, rate=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        Gamma distribution.
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Gamma(concentration=concentration, rate=rate, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Gamma(concentration=concentration, rate=rate, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def gammapoisson(concentration, rate=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.GammaPoisson(concentration=concentration, rate=rate, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.GammaPoisson(concentration=concentration, rate=rate, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def gaussiancopula(marginal_dist, correlation_matrix=None, correlation_cholesky=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.GaussianCopula(marginal_dist=marginal_dist, correlation_matrix=correlation_matrix, correlation_cholesky=correlation_cholesky, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.GaussianCopula(marginal_dist=marginal_dist, correlation_matrix=correlation_matrix, correlation_cholesky=correlation_cholesky, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def gaussiancopulabeta(concentration1, concentration0, correlation_matrix=None, correlation_cholesky=None, validate_args=False, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.GaussianCopulaBeta(concentration1=concentration1, concentration0=concentration0, correlation_matrix=correlation_matrix, correlation_cholesky=correlation_cholesky, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.GaussianCopulaBeta(concentration1=concentration1, concentration0=concentration0, correlation_matrix=correlation_matrix, correlation_cholesky=correlation_cholesky, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def gaussianrandomwalk(scale=1.0, num_steps=1, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.GaussianRandomWalk(scale=scale, num_steps=num_steps, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.GaussianRandomWalk(scale=scale, num_steps=num_steps, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def gaussianstatespace(num_steps, transition_matrix, covariance_matrix=None, precision_matrix=None, scale_tril=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.GaussianStateSpace(num_steps=num_steps, transition_matrix=transition_matrix, covariance_matrix=covariance_matrix, precision_matrix=precision_matrix, scale_tril=scale_tril, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.GaussianStateSpace(num_steps=num_steps, transition_matrix=transition_matrix, covariance_matrix=covariance_matrix, precision_matrix=precision_matrix, scale_tril=scale_tril, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def geometric(probs=None, logits=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Geometric(probs=probs, logits=logits, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Geometric(probs=probs, logits=logits, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def geometriclogits(logits, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.GeometricLogits(logits=logits, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.GeometricLogits(logits=logits, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.GeometricLogits(logits=logits, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def geometricprobs(probs, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.GeometricProbs(probs=probs, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.GeometricProbs(probs=probs, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.GeometricProbs(probs=probs, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def gompertz(concentration, rate=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Gompertz(concentration=concentration, rate=rate, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Gompertz(concentration=concentration, rate=rate, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def gumbel(loc=0.0, scale=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        Gumbel distribution.
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Gumbel(loc=loc, scale=scale, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.Gumbel(loc=loc, scale=scale, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.Gumbel(loc=loc, scale=scale, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def halfcauchy(scale=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.HalfCauchy(scale=scale, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.HalfCauchy(scale=scale, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.HalfCauchy(scale=scale, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def halfnormal(scale=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.HalfNormal(scale=scale, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.HalfNormal(scale=scale, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.HalfNormal(scale=scale, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def improperuniform(support, batch_shape, event_shape, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.ImproperUniform(support=support, batch_shape=batch_shape, event_shape=event_shape, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.ImproperUniform(support=support, batch_shape=batch_shape, event_shape=event_shape, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def independent(base_dist, reinterpreted_batch_ndims, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Independent(base_dist=base_dist, reinterpreted_batch_ndims=reinterpreted_batch_ndims, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Independent(base_dist=base_dist, reinterpreted_batch_ndims=reinterpreted_batch_ndims, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def inversegamma(concentration, rate=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.InverseGamma(concentration=concentration, rate=rate, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.InverseGamma(concentration=concentration, rate=rate, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def kumaraswamy(concentration1, concentration0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Kumaraswamy(concentration1=concentration1, concentration0=concentration0, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Kumaraswamy(concentration1=concentration1, concentration0=concentration0, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def lkj(dimension, concentration=1.0, sample_method='onion', validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        LKJ distribution.
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.LKJ(dimension=dimension, concentration=concentration, sample_method=sample_method, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.LKJ(dimension=dimension, concentration=concentration, sample_method=sample_method, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def lkjcholesky(dimension, concentration=1.0, sample_method='onion', validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.LKJCholesky(dimension=dimension, concentration=concentration, sample_method=sample_method, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.LKJCholesky(dimension=dimension, concentration=concentration, sample_method=sample_method, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def laplace(loc=0.0, scale=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Laplace(loc=loc, scale=scale, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.Laplace(loc=loc, scale=scale, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.Laplace(loc=loc, scale=scale, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def lefttruncateddistribution(base_dist, low=0.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.LeftTruncatedDistribution(base_dist=base_dist, low=low, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.LeftTruncatedDistribution(base_dist=base_dist, low=low, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def levy(loc, scale, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        Levy distribution.
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Levy(loc=loc, scale=scale, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.Levy(loc=loc, scale=scale, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.Levy(loc=loc, scale=scale, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def lognormal(loc=0.0, scale=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.LogNormal(loc=loc, scale=scale, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.LogNormal(loc=loc, scale=scale, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.LogNormal(loc=loc, scale=scale, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def loguniform(low, high, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.LogUniform(low=low, high=high, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.LogUniform(low=low, high=high, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.LogUniform(low=low, high=high, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def logistic(loc=0.0, scale=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Logistic(loc=loc, scale=scale, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.Logistic(loc=loc, scale=scale, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.Logistic(loc=loc, scale=scale, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def lowrankmultivariatenormal(loc, cov_factor, cov_diag, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.LowRankMultivariateNormal(loc=loc, cov_factor=cov_factor, cov_diag=cov_diag, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.LowRankMultivariateNormal(loc=loc, cov_factor=cov_factor, cov_diag=cov_diag, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def lowertruncatedpowerlaw(alpha, low, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.LowerTruncatedPowerLaw(alpha=alpha, low=low, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.LowerTruncatedPowerLaw(alpha=alpha, low=low, validate_args=validate_args).expand(shape).mask(mask))
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.MaskedDistribution(base_dist=base_dist, mask=mask).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.MaskedDistribution(base_dist=base_dist, mask=mask).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.MaskedDistribution(base_dist=base_dist, mask=mask).expand(shape).mask(mask))

    @staticmethod
    def matrixnormal(loc, scale_tril_row, scale_tril_column, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.MatrixNormal(loc=loc, scale_tril_row=scale_tril_row, scale_tril_column=scale_tril_column, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.MatrixNormal(loc=loc, scale_tril_row=scale_tril_row, scale_tril_column=scale_tril_column, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def mixture(mixing_distribution, component_distributions, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Mixture(mixing_distribution=mixing_distribution, component_distributions=component_distributions, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Mixture(mixing_distribution=mixing_distribution, component_distributions=component_distributions, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def mixturegeneral(mixing_distribution, component_distributions, support=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.MixtureGeneral(mixing_distribution=mixing_distribution, component_distributions=component_distributions, support=support, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.MixtureGeneral(mixing_distribution=mixing_distribution, component_distributions=component_distributions, support=support, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def mixturesamefamily(mixing_distribution, component_distribution, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.MixtureSameFamily(mixing_distribution=mixing_distribution, component_distribution=component_distribution, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.MixtureSameFamily(mixing_distribution=mixing_distribution, component_distribution=component_distribution, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def multinomial(total_count=1, probs=None, logits=None, total_count_max=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Multinomial(total_count=total_count, probs=probs, logits=logits, total_count_max=total_count_max, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Multinomial(total_count=total_count, probs=probs, logits=logits, total_count_max=total_count_max, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def multinomiallogits(logits, total_count=1, total_count_max=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.MultinomialLogits(logits=logits, total_count=total_count, total_count_max=total_count_max, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.MultinomialLogits(logits=logits, total_count=total_count, total_count_max=total_count_max, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def multinomialprobs(probs, total_count=1, total_count_max=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.MultinomialProbs(probs=probs, total_count=total_count, total_count_max=total_count_max, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.MultinomialProbs(probs=probs, total_count=total_count, total_count_max=total_count_max, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def multivariatenormal(loc=0.0, covariance_matrix=None, precision_matrix=None, scale_tril=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.MultivariateNormal(loc=loc, covariance_matrix=covariance_matrix, precision_matrix=precision_matrix, scale_tril=scale_tril, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.MultivariateNormal(loc=loc, covariance_matrix=covariance_matrix, precision_matrix=precision_matrix, scale_tril=scale_tril, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def multivariatestudentt(df, loc=0.0, scale_tril=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.MultivariateStudentT(df=df, loc=loc, scale_tril=scale_tril, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.MultivariateStudentT(df=df, loc=loc, scale_tril=scale_tril, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def negativebinomial2(mean, concentration, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.NegativeBinomial2(mean=mean, concentration=concentration, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.NegativeBinomial2(mean=mean, concentration=concentration, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def negativebinomiallogits(total_count, logits, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.NegativeBinomialLogits(total_count=total_count, logits=logits, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.NegativeBinomialLogits(total_count=total_count, logits=logits, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def negativebinomialprobs(total_count, probs, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.NegativeBinomialProbs(total_count=total_count, probs=probs, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.NegativeBinomialProbs(total_count=total_count, probs=probs, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def normal(loc=0.0, scale=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        Normal distribution.
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Normal(loc=loc, scale=scale, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.Normal(loc=loc, scale=scale, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.Normal(loc=loc, scale=scale, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def orderedlogistic(predictor, cutpoints, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.OrderedLogistic(predictor=predictor, cutpoints=cutpoints, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.OrderedLogistic(predictor=predictor, cutpoints=cutpoints, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def pareto(scale, alpha, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        Pareto distribution.
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Pareto(scale=scale, alpha=alpha, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.Pareto(scale=scale, alpha=alpha, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.Pareto(scale=scale, alpha=alpha, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def poisson(rate, is_sparse=False, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Poisson(rate=rate, is_sparse=is_sparse, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Poisson(rate=rate, is_sparse=is_sparse, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def projectednormal(concentration, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.ProjectedNormal(concentration=concentration, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.ProjectedNormal(concentration=concentration, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def relaxedbernoulli(temperature, probs=None, logits=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.RelaxedBernoulli(temperature=temperature, probs=probs, logits=logits, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.RelaxedBernoulli(temperature=temperature, probs=probs, logits=logits, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def relaxedbernoullilogits(temperature, logits, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.RelaxedBernoulliLogits(temperature=temperature, logits=logits, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.RelaxedBernoulliLogits(temperature=temperature, logits=logits, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def righttruncateddistribution(base_dist, high=0.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.RightTruncatedDistribution(base_dist=base_dist, high=high, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.RightTruncatedDistribution(base_dist=base_dist, high=high, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def sinebivariatevonmises(phi_loc, psi_loc, phi_concentration, psi_concentration, correlation=None, weighted_correlation=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.SineBivariateVonMises(phi_loc=phi_loc, psi_loc=psi_loc, phi_concentration=phi_concentration, psi_concentration=psi_concentration, correlation=correlation, weighted_correlation=weighted_correlation, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.SineBivariateVonMises(phi_loc=phi_loc, psi_loc=psi_loc, phi_concentration=phi_concentration, psi_concentration=psi_concentration, correlation=correlation, weighted_correlation=weighted_correlation, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def sineskewed(base_dist: numpyro.distributions.distribution.Distribution, skewness, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.SineSkewed(base_dist=base_dist, skewness=skewness, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.SineSkewed(base_dist=base_dist, skewness=skewness, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def softlaplace(loc, scale, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.SoftLaplace(loc=loc, scale=scale, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.SoftLaplace(loc=loc, scale=scale, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def studentt(df, loc=0.0, scale=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.StudentT(df=df, loc=loc, scale=scale, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.StudentT(df=df, loc=loc, scale=scale, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def transformeddistribution(base_distribution, transforms, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.TransformedDistribution(base_distribution=base_distribution, transforms=transforms, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.TransformedDistribution(base_distribution=base_distribution, transforms=transforms, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def truncatedcauchy(loc=0.0, scale=1.0, low=None, high=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.TruncatedCauchy(loc=loc, scale=scale, low=low, high=high, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.TruncatedCauchy(loc=loc, scale=scale, low=low, high=high, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def truncateddistribution(base_dist, low=None, high=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.TruncatedDistribution(base_dist=base_dist, low=low, high=high, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.TruncatedDistribution(base_dist=base_dist, low=low, high=high, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def truncatednormal(loc=0.0, scale=1.0, low=None, high=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.TruncatedNormal(loc=loc, scale=scale, low=low, high=high, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.TruncatedNormal(loc=loc, scale=scale, low=low, high=high, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def truncatedpolyagamma(batch_shape=(), validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.TruncatedPolyaGamma(batch_shape=batch_shape, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.TruncatedPolyaGamma(batch_shape=batch_shape, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def twosidedtruncateddistribution(base_dist, low=0.0, high=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.TwoSidedTruncatedDistribution(base_dist=base_dist, low=low, high=high, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.TwoSidedTruncatedDistribution(base_dist=base_dist, low=low, high=high, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def uniform(low=0.0, high=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Uniform(low=low, high=high, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.Uniform(low=low, high=high, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.Uniform(low=low, high=high, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def unit(log_factor, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        Unit distribution.
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Unit(log_factor=log_factor, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.Unit(log_factor=log_factor, validate_args=validate_args).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.Unit(log_factor=log_factor, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def vonmises(loc, concentration, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.VonMises(loc=loc, concentration=concentration, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.VonMises(loc=loc, concentration=concentration, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def weibull(scale, concentration, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Weibull(scale=scale, concentration=concentration, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Weibull(scale=scale, concentration=concentration, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def wishart(concentration, scale_matrix=None, rate_matrix=None, scale_tril=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.Wishart(concentration=concentration, scale_matrix=scale_matrix, rate_matrix=rate_matrix, scale_tril=scale_tril, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.Wishart(concentration=concentration, scale_matrix=scale_matrix, rate_matrix=rate_matrix, scale_tril=scale_tril, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def wishartcholesky(concentration, scale_matrix=None, rate_matrix=None, scale_tril=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.WishartCholesky(concentration=concentration, scale_matrix=scale_matrix, rate_matrix=rate_matrix, scale_tril=scale_tril, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.WishartCholesky(concentration=concentration, scale_matrix=scale_matrix, rate_matrix=rate_matrix, scale_tril=scale_tril, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def zeroinflateddistribution(base_dist, gate=None, gate_logits=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.ZeroInflatedDistribution(base_dist=base_dist, gate=gate, gate_logits=gate_logits, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.ZeroInflatedDistribution(base_dist=base_dist, gate=gate, gate_logits=gate_logits, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def zeroinflatednegativebinomial2(mean, concentration, gate=None, gate_logits=None, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.ZeroInflatedNegativeBinomial2(mean=mean, concentration=concentration, gate=gate, gate_logits=gate_logits, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.ZeroInflatedNegativeBinomial2(mean=mean, concentration=concentration, gate=gate, gate_logits=gate_logits, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def zeroinflatedpoisson(gate, rate=1.0, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.ZeroInflatedPoisson(gate=gate, rate=rate, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.ZeroInflatedPoisson(gate=gate, rate=rate, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def zerosumnormal(scale, event_shape, validate_args=None, shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.ZeroSumNormal(scale=scale, event_shape=event_shape, validate_args=validate_args).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    ...
                    return numpyro.sample(name, numpyro.distributions.ZeroSumNormal(scale=scale, event_shape=event_shape, validate_args=validate_args).expand(shape).mask(mask))

    @staticmethod
    def kl_divergence( shape=(), sample = False, seed = 0, name = 'x',obs=None, mask = None, *args, **kwargs):
        """
        ...
        if obs != None:
            ...
            if sample== True:
                if mask is None:
                    ...
                    seed = random.PRNGKey(seed)
                    return numpyro.distributions.kl_divergence(args=args, kwargs=kwargs).mask(mask).sample(seed, shape)
            else: 
                if mask is None:
                    return numpyro.sample(name, numpyro.distributions.kl_divergence(args=args, kwargs=kwargs).expand(shape))
                else:
                    return numpyro.sample(name, numpyro.distributions.kl_divergence(args=args, kwargs=kwargs).expand(shape).mask(mask))
