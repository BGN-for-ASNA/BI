...
class effects:
    ...
    def varying_effects(
        ...
        if alpha_bar is None:
            alpha_bar = dist.normal(
                5, 2, name="global_intercept", sample=sample, shape=(1,)
            )

        if beta_bar is None:
            beta_bar = dist.normal(
                -1, 0.5, name="global_beta", sample=sample, shape=(N_vars,)
            )
    ...
    def varying_intercept(
        ...
        if a_bar is None:
            a_bar = dist.normal(
                0.0, 1.5, name=f"global_intercept_{group_name}", sample=sample
            )
        if sigma is None:
