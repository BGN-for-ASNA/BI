...
r_sd = 1
sample = True
focal_individual_predictors = jnp.array([1,0,1]).reshape((1,3))
target_individual_predictors = jnp.array([1,0,1]).reshape((1,3))
# Sender receiver fixed effect -------------------------------------------------------------------------------------------------------------------------------
focal_effects = bi.dist.normal(s_mu, s_sd, shape=(N_var,), sample = sample, name = 'focal_effects')
target_effects =  bi.dist.normal( r_mu, r_sd, shape= (N_var,), sample = sample, name = 'target_effects')
print('focal_effects --------------------')
print(focal_effects)
print('\n')
...
sender_random = terms[edgl_idx[:,0],0] + terms[edgl_idx[:,1],1]
receiver_random = terms[edgl_idx[:,1],0] + terms[edgl_idx[:,0],1]
sr_terms = jnp.stack([sender_random, receiver_random], axis = 1)
print('sr_terms --------------------')
print(sr_terms)
print('\n')

# Sender receiver varying effect -------------------------------------------------------------------------------------------------------------------------------
sr_mu = 0
sr_sd = 1
sr_sigma_rate = 1
cholesky_dim = 2
...
sr_sigma =  dist.exponential(sr_sigma_rate, shape= (2,), name = 'rr', sample = sample)
...
r_sd = 1
sample = TRUE
focal_individual_predictors = c(1,0,1)
target_individual_predictors = c(1,0,1)
focal_effects = -0.20584226
target_effects = -0.20584226

sr_sigma = c(0.24372306, 1.630274)
...
for (i in 1:N_id) {
  ...
  sr_termsSTRAND_result = rbind(sr_termsSTRAND_result, tmp)
  sr_rf = rbind(sr_rf,  sr_sigma %*%  (sr_L* sr_raw[i, ]))
  sr[[i]] <-sr_sigma %*%  (sr_L* sr_raw[i, ]) + sr_termsSTRAND
}
print('sr_terms--------------------------------')
print(sr_termsSTRAND_result)
print('sr_rf-result--------------------------------')
print(sr_rf)
print('sr-result--------------------------------')
print(do.call('rbind',sr))
...
sr_raw[i, ]
(sr_L* sr_raw[i, ])
c(sr_sigma, sr_sigma) %*%  (sr_L* sr_raw[i, ])
```
