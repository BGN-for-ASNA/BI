...
# Define model ------------------------------------------------
def model(pulled_left):
    alpha = bee.dist.normal(0, 10, shape=(7,), name="alpha")  # generating k intercepts (one for each actor)
    beta1 = bee.dist.normal(0, 10, shape=(2,), name="beta")  # generating k regression coefficients for each k prosoc_left
    beta2 = bee.dist.normal(0, 10, shape=(2,), name="beta")  # generating k regression coefficients for each k condition
    lk("y", Binomial(logits=alpha[actor] + beta1[side] + beta2[cond]), obs=pulled_left)
