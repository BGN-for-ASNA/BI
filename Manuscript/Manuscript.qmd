...
\noindent\textbf{\textit{Code block 1: }} \textit{Example of prior specification differences between NumPyro, TFP, and BI}\\[-10pt]
...
3.  *BlockModelEffects* for implementing stochastic block models in network analysis.
...
## Example
To illustrate how these design features of *BI* coalesce to provide a streamlined, flexible, and powerful solution, effectively addressing the limitations identified in the existing Bayesian software landscape we will provide a basic example of how an SRM model is declared in BI, compare it with the equivalent model in Numpyro (Appendix 1) and STRAND (Appendix 2). We will also show how this model can be build from scratch with BI (Appendix 3) or its custom functions (code block 3 @lst-SRMmodel) to highligh the aceessibility-flexibility of our package by demonstrating how advance user can build custom model as well as how new user can apply pre-build *BI* models.

```{=latex}
\noindent\rule{\linewidth}{0.4pt}\\[0pt] % Top rule, line break, add 2pt space
\noindent\textbf{\textit{Code block 3: }} \textit{SRM model with prebuild functions}\\[-10pt]
\noindent\rule{\linewidth}{0.4pt}\\[-8pt] % % Bottom rule
\vspace{-\parskip}\vspace{-10pt}
```
...
```{{python}}
#| label: lst-SRMmodel
#| caption: "Python function defining the Social Relations Model structure."
def model(idx, result_outcomes, 
          dyad_effects, focal_individual_predictors, 
    ...
    sr_rf, sr_raw, sr_sigma, sr_L = 
        bi.net.nodes_random_effects(N_id, cholesky_density = 2) 

    # Dyadic effect (D) its shape is equal to n dyads 
    ## Varying intercept and slope for D
    rf, dr_raw, dr_sigma, dr_L = 
        bi.net.dyadic_random_effects(sender_receiver.shape[0], cholesky_density = 2)

    # Likelihood
    m.poisson(jnp.exp( sr_rf + rf ), obs=result_outcomes)
```
