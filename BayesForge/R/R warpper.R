...
library(reticulate)
library(jsonlite)
reticulate::py_install('BayesForge')
...
# Call distributions----------------------
attrs <- py_list_attributes(BF$dist)
no=c("__class__",
     "__delattr__",
     "__dict__",
     "__dir__",
...
for (a in attrs){
  if(!a %in% no){
    obj <- tryCatch(BF$dist[[a]], error = function(e) NULL)
    if (!is.null(obj)) {
      py_has_attr(BF$dist[[a]], "__call__")
      func_name = gsub("<function\\s+[\\w\\.]+\\.(\\w+)\\s+at\\s+0x[0-9A-Fa-f]+>", "bf.dist.\\1", as.character(BF$dist[[a]]), perl=TRUE)
      func_name2=paste(".",gsub("\\.", "\\$",func_name),sep='')
      func_name3=gsub("\\.", "",func_name)
      docName = gsub("bf.dist.", "", func_name)
      doc = data[[docName]]
      if (is.null(doc)){doc = ""}
      build_function(foo=func_name2,
                     name_file=a,
                     func_name = func_name,
                     signature = inspect$signature(BF$dist[[a]]),
                     docString = doc)
    } else {FALSE}
  }
