...
import inspect
import functools
from BayesForge.Utils.SampledData import SampledData as SampledData

# ---------------------------------------------------------------------------
# Auto-name inference via bytecode inspection (no regex)
# ---------------------------------------------------------------------------
...
def _infer_caller_name():
    """Return the LHS variable name at the call site, or None.

    Inspects the bytecode of the frame two levels up (this fn → wrapper → user
    code) and checks whether the instruction immediately after the CALL is a
    STORE_*.  Only a direct assignment triggers name inference; any other opcode
    (POP_TOP for discarded results, LIST_APPEND for list literals, another CALL
    for wrapped calls, etc.) returns None so the method falls back to its
    own default.

    In Python 3.12, f_lasti points into the inline-cache bytes that follow the
    CALL opcode, not to the CALL offset itself.  We therefore find the *last*
    instruction whose offset is <= f_lasti (that is the CALL), then check the
    very next visible instruction for a STORE_* opcode.
    """
    try:
        frame = sys._getframe(2)          # skip _infer_caller_name + wrapper
        instructions = list(dis.get_instructions(frame.f_code))
        lasti = frame.f_lasti

        # Walk forward to find the last instruction at or before f_lasti.
        call_idx = 0
...
def _has_name_kwarg(func):
    """True if *func* has a 'name' parameter that is keyword-passable with a default."""
    try:
...
class DistProxy:
    """Transparent proxy around UnifiedDist that auto-infers the *name* kwarg.

    For every distribution method that accepts an optional *name* keyword
    argument (i.e. has a default), when the caller does NOT supply *name*
    explicitly the proxy inspects the caller's bytecode to find the LHS
    variable of the assignment and uses that as the name.

    Methods like ``plate(name, shape)`` and ``track(name, x)`` where *name* is
    a required positional argument (no default) are passed through unchanged.
    """

    def __init__(self, dist_obj):
        object.__setattr__(self, '_dist', dist_obj)

    def __getattr__(self, attr):
        dist_obj = object.__getattribute__(self, '_dist')
        target = getattr(dist_obj, attr)
        if not callable(target):
            return target
        func = getattr(target, '__func__', target)
        if not _has_name_kwarg(func):
            return target

        @functools.wraps(target)
        def wrapper(*args, **kwargs):
            if 'name' not in kwargs:
                inferred = _infer_caller_name()
...
class UnifiedDist:
    ...
    def _draw_key(self, seed):
        """Resolve a PRNGKey for a direct ``sample=True`` draw.

        With no explicit ``seed`` and a central RNG manager attached, each draw
        pulls a fresh, independent sub-key from the manager's stream -- so
        consecutive draws (even of different distributions) are uncorrelated
        while the whole sequence is reproducible from a fixed base seed.
        An explicit ``seed`` is always honoured verbatim.
        """
        if seed is None and self._rng is not None:
            return self._rng.stream()
        return random.PRNGKey(self.normalize_seed(seed))
    ...
    def asymmetric_laplace(self,loc=0.0, scale=1.0, asymmetry=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - *create_obj* (bool): If True, returns the raw BF distribution object instead of creating a
                sample site. This is essential for building complex distributions like `MixtureSameFamily`.
        
        - *sample* (bool, optional): A control-flow argument. If `True`, the function will directly sample a raw JAX array from the distribution, bypassing the BF model context. If `False`, it will create a `BF.sample` site within a model. Defaults to `False`.
        ...
        - *name* (str, optional): The name of the sample site in a BF model. This is used to uniquely identify the random variable. Defaults to 'x'.

        - validate_args (bool, optional): Whether to enable validation of distribution parameters. Defaults to `None`.

        #### Returns:
        
        - When `sample=False`: A BF AsymmetricLaplace distribution object (for model building).

        - When `sample=True`: A JAX array of samples drawn from the AsymmetricLaplace distribution (for direct sampling).

        - When `create_obj=True`: The raw BF AsymmetricLaplace distribution object (for advanced use cases).
        ...
        if sample:
            ...
            if create_obj:
                ...
                infer_dict = dict(infer) if infer else None
                if mask is not None:
                    # numpyro.sample takes obs_mask as its own keyword; routing
                    # a mask through infer= makes it an inert annotation and the
                    # masked entries still contribute to the log density.
                    # handlers.mask zeroes their contribution, which is what the
                    # mask argument documents.
    ...
    def asymmetric_laplace_quantile(self,loc=0.0, scale=1.0, quantile=0.5, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - *create_obj* (bool, optional): If True, returns the raw BF distribution object instead of creating a sample site. This is essential for building complex distributions like `MixtureSameFamily`. Defaults to False.
        ...
        - When `sample=False`: A BF AsymmetricLaplaceQuantile distribution object (for model building).

        - When `sample=True`: A JAX array of samples drawn from the AsymmetricLaplaceQuantile distribution (for direct sampling).

        - When `create_obj=True`: The raw BF AsymmetricLaplaceQuantile distribution object (for advanced use cases).
    ...
    def bernoulli_logits(self,logits=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - *create_obj* (bool, optional): If True, returns the raw BF
      ...
      - When `sample=False`: A BF BernoulliLogits distribution object (for  model building).
        
      - When `sample=True`: A JAX array of samples drawn from the BernoulliLogits distribution (for direct sampling).
        
      - When `create_obj=True`: The raw BF distribution object (for advanced use cases).
    ...
    def bernoulli_probs(self,probs, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - When `sample=False`: A BF BernoulliProbs distribution object (for  model building).

        - When `sample=True`: A JAX array of samples drawn from the BernoulliLogits distribution (for direct sampling).

        - When `create_obj=True`: The raw BF BernoulliProbs object (for advanced use cases).
    ...
    def beta(self,concentration1, concentration0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - *create_obj* (bool): If True, returns the raw BF distribution object instead of creating a sample
        ...
        - When `sample=False`: A BF Beta distribution object (for  model building).

        - When `sample=True`: A JAX array of samples drawn from the BernoulliLogits distribution (for direct sampling).

        - When `create_obj=True`: The raw BF Beta object (for advanced use cases).
    ...
    def beta_binomial(self,concentration1, concentration0, total_count=1, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - *create_obj* (bool): If True, returns the raw BF distribution object instead of creating a sample site. This is essential for building complex distributions like `MixtureSameFamily`.
      ...
      - When `sample=False`: A BF BetaBinomial distribution object (for model building).

      - When `sample=True`: A JAX array of samples drawn from the BetaBinomial distribution (for direct sampling).

      - When `create_obj=True`: The raw BF BetaBinomial distribution object (for advanced use cases).
    ...
    def beta_proportion(self,mean, concentration, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - *create_obj* (bool): If True, returns the raw BF distribution
        ...
        - When `sample=False`: A BF BetaProportion distribution object (for model building).

        - When `sample=True`: A JAX array of samples drawn from the BetaProportion distribution (for direct sampling).

        - When `create_obj=True`: The raw BF BetaProportion distribution object (for advanced use cases).
    ...
    def binomial_logits(self,logits, total_count=1, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - When `sample=False`: A BF BinomialLogits distribution object (for  model building).

        - When `sample=True`: A JAX array of samples drawn from the BernoulliLogits distribution (for direct sampling).

        - When `create_obj=True`: The raw BF BinomialLogits object (for advanced use cases).
    ...
    def binomial_probs(self,probs, total_count=1, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - When `sample=False`: A BF BinomialProbs distribution object (for  model building).
    ...
    def car(self,loc, correlation, conditional_precision, adj_matrix, is_sparse=False, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * $\sigma^2$ is the variance term.
        ...
        - When `sample=False`: A BF CAR distribution object (for  model building).

        - When `sample=True`: A JAX array of samples drawn from the BernoulliLogits distribution (for direct sampling).

        - When `create_obj=True`: The raw BF CAR object (for advanced use cases).
    ...
    def categorical_logits(self,logits, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - When `sample=False`: A BF CategoricalLogits distribution object (for  model building).

        - When `sample=True`: A JAX array of samples drawn from the BernoulliLogits distribution (for direct sampling).

        - When `create_obj=True`: The raw BF CategoricalLogits object (for advanced use cases).
    ...
    def categorical_probs(self,probs, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
        - When `sample=False`: A BF CategoricalProbs distribution object (for  model building).

        - When `sample=True`: A JAX array of samples drawn from the BernoulliLogits distribution (for direct sampling).

        - When `create_obj=True`: The raw BF CategoricalProbs object (for advanced use cases).
    ...
    def cauchy(self,loc=0.0, scale=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
      ...
      - When `sample=False`: A BF Cauchy distribution object (for model building).
    ...
    def chi2(self,df, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - When `sample=False`: A BF Chi2 distribution object (for  model building).

        - When `sample=True`: A JAX array of samples drawn from the BernoulliLogits distribution (for direct sampling).

        - When `create_obj=True`: The raw BF Chi2 object (for advanced use cases).
    ...
    def circulant_normal(self,loc: jax.Array, covariance_row: jax.Array = None, covariance_rfft: jax.Array = None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - When `sample=False`: A BF Circulant Normal Distribution distribution object (for  model building).

        - When `sample=True`: A JAX array of samples drawn from the BernoulliLogits distribution (for direct sampling).

        - When `create_obj=True`: The raw BF Circulant Normal Distribution object (for advanced use cases).
    ...
    def delta(self,v=0.0, log_density=0.0, event_dim=0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - When `sample=False`: A BF Circulant Delta Distribution distribution object (for  model building).

        - When `sample=True`: A JAX array of samples drawn from the BernoulliLogits distribution (for direct sampling).

        - When `create_obj=True`: The raw BF Circulant Delta Distribution object (for advanced use cases).
    ...
    def dirichlet(self,concentration, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - When `sample=False`: A BF Dirichlet Distribution distribution object (for  model building).

        - When `sample=True`: A JAX array of samples drawn from the Dirichlet distribution (for direct sampling).

        - When `create_obj=True`: The raw BF Dirichlet Distribution object (for advanced use cases).
    ...
    def dirichlet_multinomial(self,concentration, total_count=1, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0, create_obj=False, to_jax = True):
        ...
        * A key feature is that it introduces **overdispersion** relative to a standard multinomial: counts are more “clumped” because the underlying probabilities vary according to the Dirichlet prior.
        ...
        - When `sample=False`: A BF dirichlet_multinomial Distribution distribution object (for  model building).

        - When `sample=True`: A JAX array of samples drawn from the dirichlet_multinomial distribution (for direct sampling).

        - When `create_obj=True`: The raw BF dirichlet_multinomial Distribution object (for advanced use cases).
    ...
    def discrete_uniform(self,low=0, high=1, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - When `sample=False`: A BF DiscreteUniform Distribution distribution object (for  model building).

        - When `sample=True`: A JAX array of samples drawn from the DiscreteUniform distribution (for direct sampling).

        - When `create_obj=True`: The raw BF DiscreteUniform Distribution object (for advanced use cases).
    ...
    def doubly_truncated_power_law(self,alpha, low, high, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - When `sample=False`: A BF doubly_truncated_power_law Distribution distribution object (for  model building).

        - When `sample=True`: A JAX array of samples drawn from the doubly_truncated_power_law distribution (for direct sampling).

        - When `create_obj=True`: The raw BF doubly_truncated_power_law Distribution object (for advanced use cases).
    ...
    def exponential(self,rate=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF Exponential distribution object (for model building).
    ...
    def folded_distribution(self,base_dist, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
            BF FoldedDistribution distribution object (for model building) when `sample=False`.
            JAX array of samples drawn from the FoldedDistribution distribution (for direct sampling) when `sample=True`.
            The raw BF distribution object (for advanced use cases) when `create_obj=True`.
    ...
    def gamma(self,concentration, rate=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
            Gamma: A BF Gamma distribution object (for model building).
        
            jnp.ndarray: A JAX array of samples drawn from the Gamma distribution (for direct sampling).
        
            Gamma: The raw BF distribution object (for advanced use cases).
    ...
    def gamma_poisson(self,concentration, rate=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF GammaPoisson distribution object (for model
                building).
        
      - When `sample=True`: A JAX array of samples drawn from the GammaPoisson
                distribution (for direct sampling).
        
      - When `create_obj=True`: The raw BF distribution object (for advanced
                use cases).
    ...
    def gaussian_copula(self,marginal_dist, correlation_matrix=None, correlation_cholesky=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - *create_obj* (bool, optional): If True, returns the raw BF distribution object instead of creating a sample site. This is essential for building complex distributions like `Mi  xtureSameFamily`. Defaults to False.
        ...
        #### Returns:
            BF GaussianCopula distribution object: When `sample=False` (for model building).
            JAX array: When `sample=True` (for direct sampling).
            BF distribution object: When `create_obj=True` (for advanced use cases).
    ...
    def gaussian_copula_beta(self,concentration1, concentration0, correlation_matrix=None, correlation_cholesky=None, validate_args=False, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
            GaussianCopulaBeta: A BF GaussianCopulaBeta distribution object (for model building).
            jnp.ndarray: A JAX array of samples drawn from the GaussianCopulaBeta distribution (for direct sampling).
            Distribution: The raw BF distribution object (for advanced use cases).
    ...
    def gaussian_random_walk(self,scale=1.0, num_steps=1, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0, create_obj=False, to_jax = True):
        ...
        #### Returns:
            BF.primitives.Messenger: A BF sample site object when used in a model context (`sample=False`).
            jnp.ndarray: A JAX array of samples drawn from the GaussianRandomWalk distribution (for direct sampling, `sample=True`).
            numpyro.distributions.Distribution: The raw BF distribution object (if `create_obj=True`).
    ...
    def gaussian_state_space(self,num_steps, transition_matrix, covariance_matrix=None, precision_matrix=None, scale_tril=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        - *create_obj* (bool, optional): If True, returns the raw BF distribution object instead of creating a sample site. Defaults to False.
        ...
        #### Returns:
      - When `sample=False`: A BF GaussianStateSpace distribution object (for model building).
    ...
    def geometric_logits(self,logits, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        *   p is the probability of success on each trial (derived from the logits).
        ...
        #### Returns:
            BF GeometricLogits distribution object (for model building) when `sample=False`.
    ...
    def geometric_probs(self,probs, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
            BF GeometricProbs distribution object (for model building).
            JAX array of samples drawn from the GeometricProbs distribution (for direct sampling).
            The raw BF distribution object (for advanced use cases).
    ...
    def gompertz(self,concentration, rate=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
            BF Gompertz distribution object: When `sample=False` (for model building).
            JAX array: When `sample=True` (for direct sampling).
            BF distribution object: When `create_obj=True` (for advanced use cases).
    ...
    def gumbel(self,loc=0.0, scale=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF Gumbel distribution object (for model building).
    ...
    def half_cauchy(self,scale=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF HalfCauchy distribution object (for model building).
    ...
    def half_normal(self,scale=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF HalfNormal distribution object (for model building).
    ...
    def improper_uniform(self,support, batch_shape, event_shape, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF ImproperUniform distribution object (for model building).
      - When `sample=True`: A JAX array of samples drawn from the ImproperUniform distribution (for direct sampling).
      - When `create_obj=True`: The raw BF distribution object (for advanced use cases).
        
        #### Example Usage:
            from BayesForge import sample
    ...
    def inverse_gamma(self,concentration, rate=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * (\Gamma(\alpha)) is the Gamma function.
        ...
        #### Returns:
      - When `sample=False`: A BF InverseGamma distribution object (for model building).
    ...
    def kumaraswamy(self,concentration1, concentration0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * ( x ) is the random variable.
        ...
        #### Returns:
      - When `sample=False`: A BF Kumaraswamy distribution object (for model building).
    ...
    def lkj(self,dimension, concentration=1.0, sample_method='onion', validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF LKJ distribution object (for model building).
    ...
    def lkj_cholesky(self,dimension, concentration=1.0, sample_method='onion', validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        symmetric matrix. It is particularly useful in BayesForge for models with
        ...
        * (\eta < 1): Prior allows stronger correlations.
        ...
        - *seed* (int, optional): An integer used to generate a JAX PRNGKey for reproducible sampling when `sample=True`. This argument has no effect when `sample=False`, as randomness is handled by BF's inference engine. Defaults to 0.
    ...
    def laplace(self,loc=0.0, scale=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * (b > 0) is the **scale parameter**, controlling the spread of the distribution.
        ...
        #### Returns:
      - When `sample=False`: A BF Laplace distribution object (for model building).
    ...
    def left_truncated_distribution(self,base_dist, low=0.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * **CDF** for (y > a):
        ...
        #### Returns:
      - When `sample=False`: A BF LeftTruncatedDistribution distribution object (for model building).
    ...
    def levy(self,loc, scale, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        **CDF**
        ...
        - *create_obj* (bool): If True, returns the raw BF distribution object instead of creating a sample site.
        ...
        #### Returns:
            BF Levy distribution object: When `sample=False` (for model building).
            JAX array: When `sample=True` (for direct sampling).
            BF distribution object: When `create_obj=True` (for advanced use cases).
    ...
    def log_normal(self,loc=0.0, scale=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * **CDF**
        ...
        #### Returns:
            BF LogNormal distribution object (for model building).
    ...
    def log_uniform(self,low, high, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * **CDF**
        ...
        #### Returns:
            BF LogUniform distribution object (for model building) when `sample=False`.
    ...
    def logistic(self,loc=0.0, scale=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * **Support**: $\ x \in (-\infty, +\infty) $\.
        ...
        #### Returns:
            BF Logistic distribution object (for model building) when `sample=False`.
    ...
    def lower_truncated_power_law(self,alpha, low, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF LowerTruncatedPowerLaw distribution object (for model building).
    ...
    def matrix_normal(self,loc, scale_tril_row, scale_tril_column, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF MatrixNormal distribution object (for model building).
    ...
    def mixture_general(self,mixing_distribution, component_distributions, support=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        *   **When `sample=False`**: A BF MixtureGeneral distribution object (for model building).
    ...
    def mixture_same_family(self,mixing_distribution, component_distribution, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
            *   **When `sample=False`**: A BF MixtureSameFamily distribution object (for model building).
    ...
    def multinomial_logits(self,logits, total_count=1, total_count_max=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        **Multinomial version (n draws):**
        ...
        #### Returns:
      - When `sample=False`: A BF MultinomialLogits distribution object (for model building).
    ...
    def multinomial_probs(self,probs, total_count=1, total_count_max=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
            BF MultinomialProbs distribution object (for model building).
    ...
    def multivariate_normal(self,loc=0.0, covariance_matrix=None, precision_matrix=None, scale_tril=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF MultivariateNormal distribution object (for model building).
    ...
    def multivariate_student_t(self,df, loc=0.0, scale_tril=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * $ d $ is the dimensionality of $ \mathbf{x} $.
        ...
        #### Returns:
      - When `sample=False`: A BF MultivariateStudentT distribution object (for model building).
    ...
    def negative_binomial2(self,mean, concentration, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        r"""### 
            ...
            #### Returns:
                BF NegativeBinomial distribution object: When `sample=False` (for model building).
                jnp.ndarray: A JAX array of samples drawn from the NegativeBinomial distribution (for direct sampling).
                BF NegativeBinomial distribution object: When `create_obj=True` (for advanced use cases).
    ...
    def negative_binomial_logits(self,total_count, logits, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
            Negative Binomial Logits: A BF Negative Binomial Logits distribution object (for model building).
        
            jnp.ndarray: A JAX array of samples drawn from the Negative Binomial Logits distribution (for direct sampling).
        
            Negative Binomial Logits: The raw BF distribution object (for advanced use cases).
    ...
    def negative_binomial_probs(self,total_count, probs, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
            BF NegativeBinomialProbs distribution object (for model building).
    ...
    def normal(self,loc=0.0, scale=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0, create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF Normal distribution object (for model building).
    ...
    def ordered_logistic(self,predictor, cutpoints, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF OrderedLogistic distribution object (for model building).
    ...
    def pareto(self,scale, alpha, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * a **shape parameter** $ \alpha > 0 $ (which controls the tail heaviness).
        ...
        #### Returns:
      - When `sample=False`: A BF Pareto distribution object (for model
                building).
    ...
    def poisson(self,rate, is_sparse=False, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
        - When `sample=False`: A BF Poisson distribution object (for model building).
    ...
    def projected_normal(self,concentration, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF ProjectedNormal distribution object (for
                model building).
        
      - When `sample=True`: A JAX array of samples drawn from the
                ProjectedNormal distribution (for direct sampling).
        
      - When `create_obj=True`: The raw BF distribution object (for
                advanced use cases).
    ...
    def relaxed_bernoulli_logits(self,temperature, logits, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        r"""### Relaxed Bernoulli Logits
            ...
            #### Returns:
                RelaxedBernoulliLogits: A BF RelaxedBernoulliLogits distribution object (for model building).
                jnp.ndarray: A JAX array of samples drawn from the RelaxedBernoulliLogits distribution (for direct sampling).
                RelaxedBernoulliLogits: The raw BF distribution object (for advanced use cases).
    ...
    def right_truncated_distribution(self,base_dist, high=0.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        **For a discrete random variable X:**
        ...
        #### Returns:
      - When `sample=False`: A BF RightTruncatedDistribution distribution object (for model building).
    ...
    def sine_bivariate_vonmises(self,phi_loc, psi_loc, phi_concentration, psi_concentration, correlation=None, weighted_correlation=None,    validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0, create_obj=False):
    ...
    def soft_laplace(self,loc, scale, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF SoftLaplace distribution object (for model building).
    ...
    def student_t(self,df, loc=0.0, scale=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * For $ \nu \to \infty $, this approaches $ \mathcal{N}(\mu, \sigma^2) $.
        ...
        #### Returns:
      - When `sample=False`: A BF StudentT distribution object (for model building).
    ...
    def truncated_polya_gamma(self,batch_shape=(), validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF Truncated PolyaGamma distribution object (for model building).
    ...
    def two_sided_truncated_distribution(self,base_dist, low=0.0, high=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF TwoSidedTruncatedDistribution distribution object (for model building).
    ...
    def uniform(self,low=0.0, high=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
            BF Uniform distribution object (for model building) when `sample=False`.
        
            JAX array of samples drawn from the Uniform distribution (for direct
            sampling) when `sample=True`.
        
            The raw BF distribution object (for advanced use cases) when
            `create_obj=True`.
    ...
    def unit(self,log_factor, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        r"""### Unit 
            ...
            * Often used in simulation, Monte Carlo methods, and as a building block for generating other distributions via the inverse-CDF method.
            ...
            #### Returns:
                BF Unit distribution object: When `sample=False` (for model building).
                jnp.ndarray: A JAX array of samples drawn from the Unit distribution (for direct sampling).
                BF Unit distribution object: When `create_obj=True` (for advanced use cases).
    ...
    def weibull(self,scale, concentration, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * (k > 1): increasing hazard (aging/failure over time)
        ...
        #### Returns:
            BF Weibull distribution object (for model building) when `sample=False`.
    ...
    def wishart(self,concentration, scale_matrix=None, rate_matrix=None, scale_tril=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * $\Gamma_p(\cdot)$ = multivariate Gamma function:
        ...
        - *create_obj* (bool, optional): If True, returns the raw BF distribution object instead of creating a sample
                site. This is essential for building complex distributions like `MixtureSameFamily`.
        ...
        #### Returns:
      - When `sample=False`: A BF Wishart distribution object (for model building).
    ...
    def zero_inflated_poisson(self,gate, rate=1.0, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        r"""### A Zero Inflated Poisson 
            ...
            * $\lambda>0$: mean of the Poisson component
            ...
            #### Returns:
                BF ZeroInflatedPoisson distribution object (when `sample=False`).
                JAX array of samples drawn from the ZeroInflatedPoisson distribution (when `sample=True`).
                The raw BF distribution object (when `create_obj=True`).
    ...
    def zero_sum_normal(self,scale, event_shape, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
      - When `sample=False`: A BF ZeroSumNormal distribution object (for model building).
    ...
    def bernoulli(self,probs=None, logits=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * **Failure** (often coded as 0) with probability (1 - p).
        ...
        - *create_obj* (bool, optional): If True, returns the raw BF distribution object instead of creating a sample site. This is essential for building complex distributions like `MixtureSameFamily`.
        ...
        #### Returns:
            BF Bernoulli distribution object (for model building) when `sample=False`.
    ...
    def categorical(self,probs=None, logits=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
      ...
      - When `sample=False`: A BF Categorical distribution object (for model building).
    ...
    def geometric(self,probs=None, logits=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * $X$ = number of trials until the first success (so support $1,2,3,\dots$). 
        ...
        #### Returns:
            - When `sample=False`: A BF Geometric distribution object (for model building).
    ...
    def mixture(self,mixing_distribution, component_distributions, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=0,  shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * Component distributions (densities or PMFs) $f_j(x \mid \theta_j)$, for $j = 1,\dots,K$.
        ...
        - *mixing_distribution* (BF.distribution): The distribution used to determine the mixing weights.

        - *component_distributions* (list[BF.distribution]): The list of component distributions.
      ...
      - When `sample=False`: A BF Mixture distribution object (for model
                building).
    ...
    def multinomial(self,total_count=1, probs=None, logits=None, total_count_max=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * Let $X_i$ be the number of trials (out of $n$) that produced outcome $i$. Then the vector $\mathbf X = (X_1, \dots, X_k)$ follows a multinomial distribution. 
        ...
        #### Returns:
      - When `sample=False`: A BF Multinomial distribution object (for model building).
    ...
    def relaxed_bernoulli(self,temperature, probs=None, logits=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * It is parameterised by a probability $p$ (or equivalently logits $\ell = \log\frac{p}{1-p}$).
        ...
        #### Returns:
            BF RelaxedBernoulli distribution object (for model building) when `sample=False`.
    ...
    def truncated_cauchy(self,loc=0.0, scale=1.0, low=None, high=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
            BF TruncatedCauchy distribution object (for model building) when `sample=False`.
    ...
    def truncated_distribution(self,base_dist, low=None, high=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * If both bounds are finite (i.e., doubly truncated), that is a valid and often‐used scenario. 
        ...
        #### Returns:
      - When `sample=False`: A BF TruncatedDistribution distribution object (for model building).
    ...
    def truncated_normal(self,loc=0.0, scale=1.0, low=None, high=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * The denominator $ \Phi\bigl(\tfrac{b - \mu}{\sigma}\bigr) - \Phi\bigl(\tfrac{a - \mu}{\sigma}\bigr) $ normalises the density so the total probability over ([a,b]) is 1.
        ...
        #### Returns:
            BF TruncatedNormal distribution object (for model building).
    ...
    def zero_inflated_distribution(self,base_dist, gate=None, gate_logits=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * The general PMF form is (for discrete count models):
        ...
        #### Returns:
      - When `sample=False`: A BF ZeroInflatedDistribution distribution object (for model building).
    ...
    def zero_inflated_negative_binomial2(self,mean, concentration, gate=None, gate_logits=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        * The count process is NB with parameters: mean $\mu>0$ and dispersion/shape parameter $\alpha>0$ (or equivalently size parameter (r)).
        ...
        #### Returns:
      - When `sample=False`: A BF ZeroInflatedNegativeBinomial2 distribution object (for model building).
    ...
    def negative_binomial(self,total_count, probs=None, logits=None, validate_args=None, name='x', obs=None, mask=None, infer=None, sample=False, seed=None, shape=(), event=0,create_obj=False, to_jax = True):
        ...
        #### Returns:
            BF NegativeBinomial distribution object (for model building).
