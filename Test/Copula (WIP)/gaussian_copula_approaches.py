...
def model(x_b1, x_b2):
    ...
    mu = alpha_est / (alpha_est + beta_est)
    phi = alpha_est + beta_est
    alpha = mu * phi
    beta = (1 - mu) * phi
    ...
    m.dist.gaussian_copula(
        marginal_dist=BetaPoissonMarginal(alpha, beta, lambda_est),
        correlation_cholesky=rho_est,
        obs=obs_data,
        name="obs",
    )

    # alpha_L1 = m.dist.normal(0, 1, name="alpha_L1")
    # Beta_l1 = m.dist.normal(0, 1, name="Beta_l1")
    # alpha_L2 = m.dist.normal(0, 1, name="alpha_L2")
    # Beta_l2 = m.dist.normal(0, 1, name="Beta_l2")
    #
    # L1 = alpha_L1 + Beta_l1 * X_b1
    # L2 = alpha_L2 + Beta_l2 * X_b2
    #
    # m.dist.bernoulli(L1, name="obs1", obs = x_b1)
    # m.dist.poisson(L2, name="obs2", obs = x_b2)


m.data_on_model = {"x_b1": x_b1, "x_b2": x_b2}
m.fit(model, num_samples=1000, num_warmup=1000, num_chains=1, progress_bar=True)
