...
def _cfg_seed(base: int, sigma_true: float, a: float, b: float) -> int:
    ...
    31-bit seed from the base seed and the config tuple.
    """
    key = f"{base}|{sigma_true:.6g}|{a:.6g}|{b:.6g}"
    h = int(hashlib.sha256(key.encode()).hexdigest(), 16)
    return h & 0x7FFFFFFF   # base is already in `key`; SHA-256 fills the mask
...
def run_scenario(m, priors, sigma_true, args, tag):
    ...
    # div == -1 is the "could not read divergences" sentinel (already logged at
    # readout): a monitoring gap, not a sampling failure -> let R-hat gate alone.
    ok = [r for r in results
          if r["max_rhat"] <= 1.1
          and (r["divergences"] == -1 or 0 <= r["divergences"] <= thr)]
