...
def run_bi_benchmark(model_name):
    print(f"\n--- Running BI Model {model_name} ---")
    data_dir = f"bi_data/{model_name}"
    ...
    # From Voice/bi_only.py it looks like an array we can index or take mean of
    beta_means = np.array(post).mean(axis=0)
    
    # Save results
    res_df = pd.DataFrame({'bi_mean': beta_means})
    res_df.to_csv(f"{data_dir}/results_bi.csv", index=False)
    print(f"BI Model {model_name} finished.")
