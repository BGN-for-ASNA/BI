...
# The user-facing entry point: `m.dist.<dist>(..., sample=True)` returns a
# SampledData (to_jax=False, the default), and printing it must show values.

def test_distribution_sample_true_returns_printable_sampleddata():
    """m.dist.normal(..., sample=True) yields a SampledData that prints values."""
    from BayesForge import bf
    m = bf("cpu")

    arr = m.dist.normal(0, 1, shape=(3,), sample=True)          # to_jax=False default
    assert isinstance(arr, SampledData)
    assert "at 0x" not in repr(arr)
    assert "SampledData(" in repr(arr)

    raw = m.dist.normal(0, 1, shape=(3,), sample=True, to_jax=True)
