data {
    ...
    array[N] int tech;        // technique chosen
    array[N,K] real y;        // observed personal yields of techs 1-K
    array[N,K] real ps;       // observed payoff social variables of techs 1-K
    array[N,K] real s;        // observed number of ttimes observing behaviors
    array[N,K] real ks;       // observed if kin
    array[N,K] real press;    // observed if alpha male or female
    array[N,K] real cohos;    // similarity cohort bias        
    array[N,K] real yobs;     // yob prestige bias
    array[N] int bout;        // bout or fruit
    array[N] int id;          // player id
    int N_effects;      // number of learning parameters to estimate
    array[N] real age;        //mono age
}
parameters {
