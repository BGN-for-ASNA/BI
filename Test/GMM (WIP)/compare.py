"""
Compare parameter estimates between BI (NumPyro GGMM) and reference (R phylolm).
Generates:
...
BI_CSV  = os.path.join(OUT_DIR, "bi_estimates.csv")
...
def run_bi():
    ...
    bi_df = pd.DataFrame([{"parameter": k, "estimate": v} for k, v in param_map.items()])
    bi_df.to_csv(BI_CSV, index=False)
    return bi_df


def make_outputs(ref_df, bi_df):
    merged = ref_df.merge(bi_df, on="parameter", suffixes=("_ref", "_bi"))
    merged["difference"] = merged["estimate_bi"] - merged["estimate_ref"]
    merged = merged.rename(columns={"estimate_ref": "ref_mean", "estimate_bi": "bi_mean"})
    ...
    header = (
        ...
        f"{'BI (NumPyro)':{col_w[2]}}{'Difference':{col_w[3]}}"
    )
    sep = "-" * sum(col_w)
    lines = [header, sep]
    ...
    bars_bi  = ax1.bar(x + width / 2, bi_vals,  width, label="BI (NumPyro GGMM)",
                       color="#FF5722", alpha=0.85, edgecolor="white")
    ax1.set_xticks(x)
    ax1.set_xticklabels(params, rotation=35, ha="right", fontsize=9)
    ax1.set_ylabel("Estimate")
    ax1.set_title("GGMM Parameter Comparison: Reference (R) vs BI (NumPyro)")
...
if __name__ == "__main__":
    ...
    # 2. BI
    if "--skip-bi" in sys.argv and os.path.exists(BI_CSV):
        bi_df = pd.read_csv(BI_CSV)
        print("Loaded existing bi_estimates.csv")
    else:
        bi_df = run_bi()

    # 3. Compare
    merged = make_outputs(ref_df, bi_df)
    print("\nDone.")
