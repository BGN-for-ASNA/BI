"""
GGMM (Generalized Graphical Mixed Model) in BF.
...
def run_ggmm():
    ...
    BF_df = pd.DataFrame([{"parameter": k, "estimate": v} for k, v in results.items()])
    BF_df.to_csv(os.path.join(OUT_DIR, "BF_estimates.csv"), index=False)
    print("\nBI estimates:")
