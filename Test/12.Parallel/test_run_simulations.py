...
def one_sim(N, seed, **_):
    ...
    x = m.dist.normal(0.0, 1.0, sample=True, shape=(N,))
    y = m.dist.normal(a_true + b_true * x, s_true, sample=True)

    def linreg(x=None, y=None):
        a = m.dist.normal(0.0, 5.0)
        b = m.dist.normal(0.0, 5.0)
        s = m.dist.exponential(1.0)
...
def four_chains(N, seed, chain_method, **_):
    ...
    x = m.dist.normal(0.0, 1.0, sample=True, shape=(N,))
    y = m.dist.normal(1.5 - 0.8 * x, 1.0, sample=True)
