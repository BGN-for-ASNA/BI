...
def model_bi(tank, surv, density):
    sigma = yield m.dist.exponential(1.0, shape=(1,))
    bar_alpha = yield m.dist.normal(0.0, 1.5, shape=(1,))
    alpha = yield m.dist.normal(bar_alpha, sigma, shape=(48,))
    p = alpha[tank]
...
def estimate(tank, density, a_bar_true, sigma_true, alpha_true):
    ...
    def model_rec(tank, surv, density):
        sigma = yield m_rec.dist.exponential(1.0, shape=(1,))
        bar_alpha = yield m_rec.dist.normal(0.0, 1.5, shape=(1,))
        alpha = yield m_rec.dist.normal(bar_alpha, sigma, shape=(48,))
        p = alpha[tank]
...
def param_recovery(tank, density, a_bar_sims, sigma_sims, alpha_sims, nsim):
    ...
    for i in range(nsim):
        ...
        for param in ['a_bar', 'sigma', 'alpha[0]']:
            if param == 'a_bar':
                true_val = a_bar_sims[i, 0]
            elif param == 'sigma':
                true_val = sigma_sims[i, 0]
            else:
