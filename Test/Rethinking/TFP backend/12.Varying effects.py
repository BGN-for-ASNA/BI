...
def model(cafe, wait, N_cafes, afternoon):
    alpha = yield m.dist.normal(5.0, 2.0)
    beta = yield m.dist.normal(-1.0, 0.5)
    sigma = yield m.dist.exponential(1.0)
    sigma_cafe = yield m.dist.exponential(1.0, shape=(2,))
    Rho = yield m.dist.lkj(2, 2.0)
    
    cov = jnp.outer(sigma_cafe, sigma_cafe) * Rho
    a_b_cafe = yield m.dist.multivariate_normal(jnp.stack([alpha, beta]), cov, shape=(N_cafes,))
...
def estimate_rec(afternoon, cafe_id, a_true, b_true, sigma_cafe_true, rho_true, sigma_true):
    ...
    def model_rec(cafe, wait, N_cafes, afternoon):
        alpha = yield m_rec.dist.normal(5.0, 2.0)
        beta = yield m_rec.dist.normal(-1.0, 0.5)
        sigma = yield m_rec.dist.exponential(1.0)
        sigma_cafe = yield m_rec.dist.exponential(1.0, shape=(2,))
        Rho = yield m_rec.dist.lkj(2, 2.0)
        cov = jnp.outer(sigma_cafe, sigma_cafe) * Rho
        a_b_cafe = yield m_rec.dist.multivariate_normal(jnp.stack([alpha, beta]), cov, shape=(N_cafes,))
        a_cafe, b_cafe = a_b_cafe[:, 0], a_b_cafe[:, 1]
        mu = a_cafe[cafe] + b_cafe[cafe] * afternoon
