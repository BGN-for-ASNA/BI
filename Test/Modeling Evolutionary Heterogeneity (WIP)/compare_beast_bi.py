...
def compare_model1():
    ...
    df_bi = pd.read_csv("Model_1_Spatial_Heterogeneity/bi_gamma_vec_post.csv")
    ...
    sns.kdeplot(df_bi['kappa'], ax=axes[0], label='BI (Native)', color='blue', fill=True, alpha=0.3)
    sns.kdeplot(df_beast['hky.kappa'], ax=axes[0], label='BEAST 2', color='green', linestyle='--')
    axes[0].set_title('Kappa (ts/tv)')
    axes[0].legend()
    
    sns.kdeplot(df_bi['alpha'], ax=axes[1], label='BI (Native)', color='blue', fill=True, alpha=0.3)
...
def compare_model2():
    ...
    df_bi = pd.read_csv("Model_2_Temporal_Heterogeneity/bi_ucln_vec_post.csv")
    ...
    # Let's plot ucld.mean vs exp(mu_c). Wait, in BI, branch lengths scaled by c_j = exp(mu_c + sigma_c * z). 
    ...
    # Often BI mu_c corresponds to log(ucld.mean) - 0.5*sigma_c^2 in BEAST if parameterizing real mean.
    
    sns.kdeplot(df_bi['mu_c'], ax=axes[2], label='BI (Native) mu_c', color='blue', fill=True, alpha=0.3)
    ...
    sns.kdeplot(df_bi['sigma_c'], ax=axes[3], label='BI (Native)', color='blue', fill=True, alpha=0.3)
