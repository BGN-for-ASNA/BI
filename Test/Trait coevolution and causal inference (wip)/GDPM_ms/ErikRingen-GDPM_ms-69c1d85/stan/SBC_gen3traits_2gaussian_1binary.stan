// 3-trait generative model: 2 Gaussian + 1 Binary
// Trait 1: normal (Gaussian)
// Trait 2: normal (Gaussian)
// Trait 3: bernoulli_logit (Binary)
// Converted to prior predictive sampling (no parameters)
functions {
...
data{
  ...
  int<lower=3> J; // number of response traits (must be 3)
  ...
  int<lower=3> num_effects; // number of effects being estimated
...
transformed data{
  ...
  vector[to_int(N_obs - sum(col(miss, 3)))] obs3;
...
generated quantities{
  ...
  for (j in 1:(num_effects - J)) A_offdiag[j] = normal_rng(0, 2.5);  // Independent normal priors
  ...
  array[N_tree,N_obs,J] real yrep; // predictive checks
  {
    ...
    for (i in 1:N_obs) {
      for (t in 1:N_tree) {
        // Trait 1 (Gaussian/Normal)
        yrep[t,i,1] = eta[t,tip_id[i]][1] + terminal_drift_rep[t,tip_id[i]][1];
        // Trait 2 (Gaussian/Normal)
        yrep[t,i,2] = eta[t,tip_id[i]][2] + terminal_drift_rep[t,tip_id[i]][2];
        // Trait 3 (Bernoulli-logit/Binary)
        yrep[t,i,3] = bernoulli_logit_rng(eta[t,tip_id[i]][3] + terminal_drift_rep[t,tip_id[i]][3]);
      }
    }
  }
}
