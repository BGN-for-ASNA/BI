...
def make_model(N_seg, N_tips, N_obs, J, K, noncentered=False):
    """noncentered=True reparameterises terminal_drift as L @ z with z ~ N(0, I).

    Same posterior, different geometry. The default (False) matches
    GDPM_primate.stan, which samples terminal_drift centered on a scale that is
    itself a function of A and Q -- a funnel. Stan reports E-BFMI 0.29-0.34 and
    max treedepth on 1999/2000 draws for that parameterisation, so the flag
    exists to test how much of the sampling cost it accounts for. Only the
    default is a faithful translation; the flag is a diagnostic.
    """
    def model(y, observed, idx_longevity, idx_maturity, y_mean, node_seq,
        ...
        if noncentered:
            z_term = m.dist.normal(0.0, 1.0, shape=(N_obs, K), name="z_terminal")
            terminal_drift = jnp.einsum("nij,nj->ni", L_tips[tip_id], z_term)
            m.dist.track("terminal_drift", terminal_drift)
        else:
        ...
        # longevity and age at female maturity, both partly missing: only the
        # observed rows are scored, matching Stan's `if (miss[i,j] == 0)` guard
        mu3 = alpha[2] + Lambda[2, 0] * eta_tips[idx_longevity, 0]
        m.dist.gamma(shape_[2], shape_[2] / jnp.exp(mu3),
                     obs=y[idx_longevity, 2] / y_mean[2], name="obs_longevity")
        mu4 = alpha[3] + Lambda[3, 0] * eta_tips[idx_maturity, 0]
        m.dist.gamma(shape_[3], shape_[3] / jnp.exp(mu4),
                     obs=y[idx_maturity, 3] / y_mean[3], name="obs_maturity")
