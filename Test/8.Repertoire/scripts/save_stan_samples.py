...
def main():
    ...
    fit_stan = model_stan.sample(data='../data/stan_data.json', seed=42, iter_sampling=1000, iter_warmup=1000, chains=2)
    
    # Save samples
    samples = fit_stan.draws_pd()
    samples.to_csv('../data/stan_reference_samples.csv', index=False)
