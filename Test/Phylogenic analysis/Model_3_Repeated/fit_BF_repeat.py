...
# Initialize BF
# Initialize BF with float64
# Note: BF might need jax_enable_x64=True
...
if not os.path.exists("L_repeat.csv"):
    ...
    subprocess.run(["Rscript", "save_L.R"])
...
m.data_on_model = {
    ...
    "spec_mean_cf": jnp.array(
        data_repeat["spec_mean_cf_centered"].values, dtype=jnp.float64
    ),
    ...
    "within_spec_cf": jnp.array(
        data_repeat["within_spec_cf"].values, dtype=jnp.float64
    ),
}


def model(phen, spec_mean_cf, phylo_idx, species_idx, A_cholesky, within_spec_cf):
    # Priors
    intercept = m.dist.normal(0, 50, name="Intercept")
    b_spec_mean_cf = m.dist.normal(0, 10, name="b_spec_mean_cf")
    b_within_spec_cf = m.dist.normal(0, 10, name="b_within_spec_cf")
    ...
    sd_species = m.dist.left_truncated_distribution(
        m.dist.student_t(3, 0, 20, create_obj=True), low=0.0, name="sd_species"
    )
    ...
    mu = (
        intercept
        + b_spec_mean_cf * spec_mean_cf
        + b_within_spec_cf * within_spec_cf
        + u_phylo[phylo_idx]
        + u_species[species_idx]
    )
...
post_df.to_csv("BF_post_repeat.csv", index=False)
