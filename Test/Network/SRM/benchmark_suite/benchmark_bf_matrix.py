...
# ---------------------------------------------------------------------------
N             = 400
CORES         = 20   # 400 % 5 == 0 (5 devices/chain); 80 rows/device
NUM_CHAINS    = 4
ITER_SAMPLING = int(os.environ.get('ITER_SAMPLING', '1000'))
ITER_WARMUP   = int(os.environ.get('ITER_WARMUP', '1000'))
SEED          = 42
N_GRP_ANY     = 1
N_GRP_MERICA  = 3
...
EID, DIR = NeteffectMatrix.dyad_index_maps(N)   # row-shardable dyadic-RE gather maps
...
def model_srm_matrix_sharded(Y_mat, dyadic_preds_mat, sender_preds,
    ...
    All of Y_mat, dyadic_preds_mat, sender_preds, Any_shard, Merica_shard,
    mask_mat_shard, eid_shard, dir_shard are sharded along axis 0;
    receiver_preds, full Any/Merica, N_per_grp_* come from closure (replicated).

    The dyadic random effects use the row-shardable GATHER (eid_shard/dir_shard)
    instead of the full-matrix edgl_to_mat scatter, so each device materialises
    only its (N/k, N) block of the random-effect matrix from the replicated
    edgelist — no redundant full (N,N) construction per device.
    """
    ...
    D_mat    = NeteffectMatrix.dyadic_effect(
        dyadic_preds_mat, eid=eid_shard, dir_idx=dir_shard)                  # (N/k, N)
...
def run_sharded():
    m.data_on_model = dict(
        ...
        eid_shard        = EID,                # (N,N) gather map, sharded axis 0
        dir_shard        = DIR,                # (N,N) gather map, sharded axis 0
    )
    t0 = time.perf_counter()
...
if __name__ == '__main__':
    ...
    csv_path   = os.path.join(script_dir, 'benchmark_bf_matrix.csv')
