...
# Undo the non-centered parameterisation used in bi_model_binary:
...
apply_non_centered_transform <- function(bi_draws, bi_data) {
  ...
  if (as.numeric(bi_data$num_random) > 0) {
    grp_idx <- bi_data$random_group_index   # 1-based R vector
...
test_name <- "binary_full_directed_no_zi"
...
bi_data       <- build_stan_data_bison(stan_data_raw, "binary", TRUE, FALSE, directed = TRUE)
