...
source(file.path(BASE, "BF_model_count.R"))
...
center_undirected_random_effects <- function(draws, BF_data) {
  if (as.numeric(BF_data$directed) == 1) return(draws)
  if (as.numeric(BF_data$num_random_groups) < 2) return(draws)
...
save_combination_log <- function(test_name, stan_draws, BF_draws, out_dir) {
  ...
  rows <- c(rows, "", "Category-wise Mean KL(Stan||BF)")
...
test_name <- "count_full_undirected_no_zi"
...
set.seed(42)
sim <- simulate_bison_model("count", aggregated = TRUE,
...
fit_bison <- tryCatch(
  bison_model(formula, data = df, model_type = "count",
    directed = FALSE, partial_pooling = TRUE, zero_inflated = FALSE,
...
BF_data       <- build_stan_data_bison(stan_data_raw, "count", TRUE, FALSE, directed = FALSE)
...
stan_draws  <- center_undirected_random_effects(stan_draws, BF_data)
...
tryCatch({
  ...
  m$fit(BF_model_count, num_warmup = ITER_WARMUP, num_samples = ITER_SAMPLES,
        num_chains = NUM_CHAINS, target_accept_prob = 0.999)
  raw_BF   <- get_bi_draws(m$posteriors)
  BF_draws <- normalize_bi_names(
    center_undirected_random_effects(apply_non_centered_transform(raw_BF, BF_data), BF_data),
    stan_draws)
