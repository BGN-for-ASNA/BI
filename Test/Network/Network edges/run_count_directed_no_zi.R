...
apply_non_centered_transform <- function(BF_draws, BF_data) {
  ...
  if (as.numeric(BF_data$directed) == 1 && as.numeric(BF_data$num_random) > 0) {
...
test_name <- "count_full_directed_no_zi"
...
fit_bison <- tryCatch(
  bison_model(formula, data = df, model_type = "count",
    directed = TRUE, partial_pooling = TRUE, zero_inflated = FALSE,
...
BF_data       <- build_stan_data_bison(stan_data_raw, "count", TRUE, FALSE, directed = TRUE)
