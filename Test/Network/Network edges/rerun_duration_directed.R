...
stan_param_names <- function(bi_data) {
  p <- "edge_weight"
...
run_duration_directed <- function(zero_inflated) {
  test_name <- paste0("duration_full_directed_", if (zero_inflated) "zi" else "no_zi")
  ...
  stan_data_raw <- build_stan_data_duration(df, formula, directed = TRUE,
  ...
  stan_params <- stan_param_names(stan_data_raw)
  ...
  bi_draws <- list()
  tryCatch({
    m$data_on_model <- list(data = make_jax_data(bi_data, "duration"))
...
run_duration_directed(zero_inflated = FALSE)
run_duration_directed(zero_inflated = TRUE)
