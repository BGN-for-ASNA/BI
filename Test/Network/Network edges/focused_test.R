...
run_focused <- function(model_type, bi_func, directed, zero_inflated,
  ...
  if (model_type == "duration") {
    ...
    fit_stan  <- tryCatch(
      dur_model$sample(data = stan_data_raw, chains = 1,
...
# 10 combinations (duration only supports undirected)
for (mod in list(
    list(type="binary",   func=bi_model_binary),
    list(type="count",    func=bi_model_count))) {
  for (directed in c(FALSE, TRUE)) {
    for (zi in c(FALSE, TRUE)) {
      run_focused(mod$type, mod$func, directed=directed, zero_inflated=zi,
                  num_nodes=20)
    }
  }
}
for (zi in c(FALSE, TRUE)) {
  run_focused("duration", bi_model_duration, directed=FALSE, zero_inflated=zi,
              num_nodes=20)
}
