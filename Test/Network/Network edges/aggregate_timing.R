...
cat(sprintf("\n%-35s  %8s  %8s  %7s  %s\n",
    "Model", "Stan(s)", "BF(s)", "Ratio", "Timestamp"))
cat(paste(rep("-", 75), collapse = ""), "\n")
for (i in seq_len(nrow(d))) {
  cat(sprintf("%-35s  %8.1f  %8.1f  %6.1fx  %s\n",
    d$model[i], d$stan_sec[i], d$BF_sec[i], d$stan_bi_ratio[i], d$timestamp[i]))
}
cat(paste(rep("-", 75), collapse = ""), "\n")
cat(sprintf("%-35s  %8.1f  %8.1f  %6.1fx\n",
    "MEAN", mean(d$stan_sec), mean(d$BF_sec), mean(d$stan_bi_ratio)))
cat("\n")
