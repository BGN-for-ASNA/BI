...
def free_jax_memory():
    """Drop JAX's compiled-executable cache between parameter-recovery fits.

    Every m.fit() builds a fresh NUTS/MCMC object, so each recovery iteration
    compiles a new XLA executable that JAX keeps alive (~35 MB per fit here,
    more for GP / LKJ models). Over BF_NSIM=100 iterations that exhausts host
    RAM and the run dies with "LLVM ERROR: Unable to allocate section memory".
    Clearing the cache costs nothing in speed — the next fit recompiles either
    way — and holds RSS flat across the loop.
    """
    jax.clear_caches()
    gc.collect()
...
def generate_comparison_table(df_BF, d, model_name):
    ...
    # Save to txt file
    output_path = f"{os.environ.get('BF_PLOTS_DIR', 'plots')}/{model_name}_comparison_table.txt"
