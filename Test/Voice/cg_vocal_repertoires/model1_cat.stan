// model with age covariate as categories
data{
    int N; // individuals
    int M; // tokens
    ...
    array[N] int a; // age of each individual
    int Na; // number of age categories
}
parameters {
    vector<lower=0>[M] L; // rates of each token, conditional on individual possessing it
    array[M,Na] real<lower=0,upper=1> p; // prob of each vocalization
}
model{
    L ~ exponential(1);
    for ( i in 1:Na ) p[1:M,i] ~ beta(2,2);
    // loop over recordings
    for ( j in 1:J ) {
        for ( m in 1:M ) {
            // compute vocalization probs for individual with this a[id[j]]
            real log_pa = log(p[m,a[id[j]]]);
            if ( Y[j,m] >0 ) {
