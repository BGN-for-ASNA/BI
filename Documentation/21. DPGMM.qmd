...
To discover group structures or clusters in data without pre-specifying the number of groups, we can use a **Dirichlet Process Gaussian Mixture Model (DPMM)**. This is a [non-parametric 🛈]{style="color:#0D6EFD"} clustering method. Essentially, the model assumes the data is generated from a collection of different Gaussian distributions, and it simultaneously tries to figure out:
...
This level describes how any single data point, $\mathbf{x}_i$, is generated. The process involves two steps: first, assigning the data point to a cluster, and second, drawing it from that cluster's specific distribution. We use a truncation level $T$ as a finite approximation for the infinite number of possible clusters in a true Dirichlet Process.

$$
\begin{align*}
\mathbf{x}_i \mid z_i=k &\sim \text{MultivariateNormal}(\boldsymbol{\mu}_k, \Sigma_{\text{obs}}) \\

\\
z_i &\sim \text{Categorical}(\mathbf{w}) \\
\mathbf{w} &= \text{StickBreaking}(\beta_1, ..., \beta_{T-1}) \\
\beta_k &\sim \text{Beta}(1, \alpha) \\
\alpha &\sim \text{Gamma}(1, 10)\\

\\
\boldsymbol{\mu}_k &\sim \text{MultivariateNormal}(\boldsymbol{\mu}_0, \Sigma_0)  \\
\Sigma_{\text{obs}} &= \mathbf{I}_D \\
...
*   **Latent Variables (Inferred):**
    ...
    *   $\mathbf{w}$: The vector of mixture weights, where $w_k$ is the probability of belonging to cluster $k$.
...
*   **Hyperparameters (Fixed):**
    *   $\boldsymbol{\mu}_0$: The prior mean for the cluster centers (e.g., `mean(data)`).
