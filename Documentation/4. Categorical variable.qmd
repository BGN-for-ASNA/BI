...
model <- function(kcal_per_g, index_clade){
  ...
  beta = bf.dist.normal( 0, 0.5, name = 'beta', shape = c(4))  # shape based on the number of clades
...
- As for [Multiple continuous variables](2.%20Multiple%20continuous%20variables.qmd), we can extend the linear regression model to include multiple categorical predictors.

- We can apply interaction terms similarly to [Interaction between continuous variables](3.%20Interaction%20between%20continuous%20variables.qmd).
:::
