...
model <- function(tank, surv, density){
  ...
  a_bar =  bf.dist.normal(0, 1.5, name='a_bar',shape=c(1))
  alpha = bf.dist.normal(a_bar, sigma, name='alpha', shape =c(48))
  p = alpha[tank]
  # Likelihood
  m$dist$binomial(total_count = density, logits = p, obs=surv)
} 
...
# Define model ------------------------------------------------
@BF function model(tank, surv, density)
    alpha = m.effects.varying_intercept(N_groups=48,group_id=tank, group_name = "tank")
    m.dist.binomial(total_count = density, logits = alpha, obs=surv)
end
...
## Notes
::: callout-note 
- We can apply multiple variables similarly to [Multiple continuous variables](2.%20Multiple%20continuous%20variables.qmd).
