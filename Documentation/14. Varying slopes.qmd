...
data_path = files('BEE.resources.data') / 'Sim data multivariatenormal.csv'
...
model <- function(cafe, afternoon, wait, N_cafes = as.integer(20) ){
  a = bee.dist.normal(5, 2, name = 'a')
  b = bee.dist.normal(-1, 0.5, name = 'b')
  sigma_cafe = bee.dist.exponential(1, shape= c(2), name = 'sigma_cafe')
  sigma = bee.dist.exponential( 1, name = 'sigma')
  Rho = bee.dist.lkj(as.integer(2), as.integer(2), name = 'Rho')
  cov = jnp$outer(sigma_cafe, sigma_cafe) * Rho
  
  a_cafe_b_cafe = bee.dist.multivariatenormal(
