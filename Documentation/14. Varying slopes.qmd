...
For more than two varying effects ($k$ and $g$), we apply the same principle but with a covariance matrix for each varying effect that is summed to generate the varying intercept and slope. For example, if we want to generate random slopes for $i$ actors and $k$ groups, we can define the formula as follows:
...
$$
\mu_i = \left( \alpha_{k(i)} + \pi_{g(i)} \right) + \left( \beta_{k(i)} + \gamma_{g(i)} \right) X_i 
$$ 
