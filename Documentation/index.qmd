...
<h3>
  <p >
  <strong>Analyze. Share. Reproduce.</strong> You have a story to tell with data—tell it with BI.
  </p>
  <button class="btn btn-lg btn-primary" type="button" href="#gallery" >Get Started</button>
</h3>
</div>
...
### Prebuilt function for complex models
```R
def model(kcal_per_g, index_clade):
    alpha = m.bi.dist.normal(0, 0.5, shape=(4,), name = 'a') # shape based on the number of clades
    beta = m.bi.dist.normal(0, 0.5, shape=(4,), name = 'b')
...
document.addEventListener('DOMContentLoaded', function() {
  // 1. The element we want to watch for
  const gallerySection = document.querySelector('#gallery');

  // 2. The element we will add a class to (the main body)
  const body = document.body;

  // Only run the script if the gallery section actually exists on the page
  if (gallerySection) {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        // 3. If the gallery is on screen...
        if (entry.isIntersecting) {
          // ...add the class to the body.
          body.classList.add('gallery-in-view');
        }else {
          // ...otherwise, remove the class to SHOW the categories again.
          body.classList.remove('gallery-in-view');
        }
      });
    }, { 
      // Trigger when 10% of the gallery is visible
      threshold: 0.1 
    });

    // 5. Start watching the gallery section.
