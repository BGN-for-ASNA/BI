...
**BF** provides a unified experience across Julia, Python, and R. Whether you work in R's formula syntax, Python's object-oriented approach, or Julia's mathematical elegance, the model logic remains consistent.
...
# Setup device------------------------------------------------
m = bf(platform='cpu') # cpu, gpu or tpu
...
def model(kcal_per_g, index_clade):
    alpha = m.bf.dist.normal(0, 0.5, shape=(4,), name = 'a') # shape based on the number of clades
    beta = m.bf.dist.normal(0, 0.5, shape=(4,), name = 'b')
    sigma = m.bf.dist.exponential( 1, name = 's')    
...
<!-- Only ONE of {Network Exploration, Distributions visualization} can be a
     tab: each emits a large raw-HTML output (the NetExplorer <iframe>, the
     Plotly figure) that terminates the panel-tabset and drops every tab after
     it. Network Exploration is kept as the last tab; Distributions
     visualization follows as a section right after the tab-set. -->
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const observer = new IntersectionObserver(
    ...
    {
      // Only trigger when the gallery grid is substantially in the viewport
      threshold: 0,
