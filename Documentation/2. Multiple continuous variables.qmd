...
## Example
Below is example code demonstrating Bayesian multiple linear regression using the BayesForge (BF) package. Data consist of three continuous variables (*height*, *weight*, *age*), and the goal is to estimate the effect of *weight* and *age* on *height*. This example is based on @mcelreath2018statistical.
...
model <- function(height, weight, age){
  # Parameter prior distributions
  alpha = bf.dist.normal(0, 0.5, name = 'a')
  beta1 = bf.dist.normal(0, 0.5, name = 'b1')
  beta2 = bf.dist.normal(0, 0.5, name = 'b2')   
  sigma = bf.dist.uniform(0, 50, name = 's')
  # Likelihood
  bf.dist.normal(alpha + beta1 * weight + beta2 * age, sigma, obs=height)
}
...
# Define model ------------------------------------------------
@BF function model(height, weight, age)
    # Parameter prior distributions
    alpha = m.dist.normal(0, 0.5, name = "alpha")    
    beta1 = m.dist.normal(0, 0.5, name = "beta1")
    beta2 = m.dist.normal(0, 0.5, name = "beta2")
...
```python
betas = bf.dist.normal(0, 0.5, name = 'regression_coefficients', shape = (2,))
```
