...
- A Beta distribution has two parameters: the rates for each probabilities and a shape parameter θ. θ influence how probabilities are distributed between 0 and 1. Specifically, it consists of two parameters, $\gamma$  and $\eta$ , which determine the concentration of probability around 0 and 1. The values of 𝛼 and 𝛽 shape the distribution:

  - If both are greater than 1, the distribution is bell-shaped and centered around the mean 𝑝. 

  - If $\gamma$ < 1 and $\eta$ < 1, the distribution is U-shaped, indicating that outcomes are more likely to be near 0 or 1.

  - If 𝛼 = $\gamma$ and $\eta$ = 1, the Beta distribution is uniform. 

Thus, the shape parameters 𝛼 $\gamma$ $\eta$ provide flexibility in modeling various types of prior beliefs about probabilities.
...
def model(gid, applications, admit):
    ...
    m.lk("y", dist.betabinomial(total_count = applications, concentration1 = concentration1, concentration0 = concentration0), obs=admit)
...
model <- function(gid, applications, admit){
  ...
  eta = (1 - pbar) * t
  # Likelihood
  m$lk("y", bi.dist.betabinomial(total_count=applications, concentration1=gamma, concentration0=eta), obs=admit)
}
...
# Summary ------------------------------------------------
m$sampler$print_summary() # Get posterior distributions
```
...
- $\overline{\rho}$ is the probability of success for the *i*-th observation. The logit function transforms the linear predictor α (which can take any real value) into a probability value between 0 and 1. 
  
- $\tau$ is derived from 𝜙 and is used as a scaling factor for the shape parameters 𝛾 and 𝜂.
  
- α is a vector of parameters, each representing the effect of the group variable *i on the success probability.
