...
- A Beta distribution has two parameters: the rates for each probability and a shape parameter θ. θ influences how probabilities are distributed between 0 and 1. Specifically, it consists of two parameters, $\alpha$ and $\beta$, which determine the concentration of probability around 0 and 1.
...
data_path = files('BEE.resources.data') / 'UCBadmit.csv'
...
model <- function(gid, applications, admit){
  ...
  phi = bee.dist.exponential(1, name = 'phi',shape=c(1))
  alpha = bee.dist.normal(0., 1.5, shape= c(2), name='alpha')
  t = phi + 2
  pbar = jax$nn$sigmoid(alpha[gid])
  gamma = pbar * t
  eta = (1 - pbar) * t
...
- α is a vector of parameters, each representing the effect of group *i* on the success probability.
