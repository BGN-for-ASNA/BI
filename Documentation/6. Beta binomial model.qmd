...
- A Beta distribution has two parameters: the rates for each probabilities and a shape parameter θ. θ influence how probabilities are distributed between 0 and 1. Specifically, it consists of two parameters, $\alpha$  and $\beta$ , which determine the concentration of probability around 0 and 1.

  - If both are equal or greater than 1, the distribution is bell-shaped and centered around 0.5. 
  - If 𝛼 > 𝛽 , the distribution is skewed toward 1 and if $\beta > \alpha$, it is skewed toward 0.
...
3) Additionally, we have the number of applications (applications) per individuals' gender, which will be used to account for independent rates.
