...
## Example
Below is an example code snippet demonstrating Bayesian Beta-Binomial regression using the BayesForge (BF) package. The data consist of:
...
model <- function(gid, applications, admit){
  ...
  alpha = bf.dist.normal(0., 1.5, shape= c(2), name='alpha')
  t = phi + 2
  pbar = m$link$inv_logit(alpha[gid])
  gamma = pbar * t
  eta = (1 - pbar) * t
...
# Define model ------------------------------------------------
@BF function model(gid, applications, admit)
    # Prior for overall concentration scaling (positive, via exponential)
    phi = m.dist.exponential(1, name="phi")
