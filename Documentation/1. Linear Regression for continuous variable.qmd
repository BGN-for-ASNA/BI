...
-   Ussually, we use *Normal* distribution for $\alpha$ , $\beta$ and an *exponential* or *Gamma* distributiuon for $\sigma$ (basically any distribution that are positivelly defined).
...
In the Bayesian formulation we define each parameters with [priors](# "Priors are distribution that represent the value of the parameter before contioning it on the data."). We can express a Bayesian version of this regression model using the following model: 
$$
...
Where: 
- $Y_i$ is the target variable. 
- $\beta$ and $\alpha$ are the regression coefficients and intercept paramters, respectively.
- $\sigma$, is prior .
