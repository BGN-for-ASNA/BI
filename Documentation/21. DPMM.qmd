...
::: callout-caution
-   A DPMM is a Bayesian model 🛈 that considers uncertainty in all its parameters. The core idea is to use the Dirichlet Process prior that allows for a potentially infinite number of clusters. In practice, we use a finite approximation called the [Stick-Breaking Process 🛈]{style="color:#0D6EFD"}.
...
-   The model is often implemented in its [marginalized form 🛈]{style="color:#0D6EFD"}. Instead of explicitly assigning each data point to a cluster, we integrate out this choice. This creates a smoother probability surface for the inference algorithm to explore, leading to much more efficient computation.
:::
...
\pi  = \text{StickBreaking}(\beta_1, ..., \beta_{K-1}) \\
...
*   $\begin{pmatrix} Y_{i,1} \\ \vdots \\ Y_{i,D} \end{pmatrix}$ is the $i$-th observed with its D-dimensional factors.

*   $\begin{pmatrix}\mu_{z_i,1} \\ \vdots \\ \mu_{z_i,D}\end{pmatrix}$: The D-dimensional mean vector of the $z_i$-th cluster.
*   $\begin{pmatrix} A{k,1} \\ \vdots \\ A{k,D} \end{pmatrix}$: Is the D-dimensional vector prior of the $k$-th cluster mean.
*   $B$: Is the prior covariance of the cluster means setup as a diagonal matrix with 0.1 in the diagonal.

*   $\Sigma_k$: Is the DxD covariance matrix of the $k$-th cluster (composed of $\sigma_k$ and $\Omega_k$)$.
*   $\sigma_k$ : Is the standard deviation of the $k$-th cluster.
*   $\Omega_k$: Is the cholesky decomposition of the $k$-th cluster.
  
*   $z_i$: Is the cluster assignment of the $i$-th observation.
*   $\pi$: Is the vector of $K$ cluster weights with prior 0.5 for each cluster.
