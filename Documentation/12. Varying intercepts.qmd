# Varying intercepts
## General Principles
To model the relationship between dependent variables and an independent variable while allowing for different intercepts across groups or clusters, we can use a _Varying Intercepts_ model. This approach is particularly useful when data is grouped (e.g., by subject, location, or time period) and we expect the baseline level of the outcome to vary across these groups.
...
- Each intercept have is own prior -i.e. a [<span style="color:#0D6EFD">hyper-prior 🛈</span>]{#hyperP}
