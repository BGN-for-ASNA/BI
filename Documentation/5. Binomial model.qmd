...
description: "Modeling a binary outcome (e.g., success/failure) based on one or more predictor variables."
categories: [Classification, Regression, GLM]
image: "Figures/binomial.svg"
order: 6
---
...
## Example
Below is an example code snippet that demonstrates Bayesian binomial regression using the Bayesian Inference (BI) package. The data consist of one binary dependent variable (*pulled_left*), which represents which side individuals pulled. The goal is to evaluate the probability of pulling the left side. This example is based on @mcelreath2018statistical.
...
[<span style="color:#0D6EFD">priors 🛈</span>]{#prior}. We can express the Bayesian regression model accounting for prior distributions as follows:

$$
Y_i \sim Binomial(n = 1, p_i)
$$

$$
logit(p_i) \sim \alpha + \beta X_i
$$
