# Binomial Model
## General Principles
To model the relationship between a binary dependent variable —e.g., success/failure, yes/no, or 1/0— and one or more independent variables, we can use a _Binomial model_. 
...
- We have the first [<span style="color:#0D6EFD">link function 🛈</span>]{#linkF} _logit_. The _logit_ link function in the Bayesian binomial model converts the linear combination of predictor variables into probabilities, making it suitable for modeling binary outcomes. It helps to estimate the relationship between predictors and the probability of success, ensuring results fall within the bounds of the binomial distribution. 
...
model <- function(pulled_left){
  ...
  # Likelihood
  m$lk("Y", bi.dist.binomial(logits = alpha), obs=pulled_left)
}
