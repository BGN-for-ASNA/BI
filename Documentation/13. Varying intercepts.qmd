...
- In the code below, the intercept ```alpha``` for each of the $k$ declared groups shares two *hyper-priors*, ```a_bar``` and ```sigma```, which are respectively modeled by a Normal and an Exponential distribution.
