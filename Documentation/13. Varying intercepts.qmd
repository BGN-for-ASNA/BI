...
- Each intercept has its own prior - i.e., a [<span style="color:#0D6EFD">hyper-prior 🛈</span>]{#hyperP}.
...
data_path = files('BEE.resources.data') / 'reedfrogs.csv'
...
model <- function(tank, surv, density){
  ...
  sigma = bee.dist.exponential( 1,  name = 'sigma',shape=c(1))
  a_bar =  bee.dist.normal(0, 1.5, name='a_bar',shape=c(1))
  alpha = bee.dist.normal(a_bar, sigma, name='alpha', shape =c(48))
  p = alpha[tank]
  # Likelihood
...
We can express the Bayesian regression model accounting for [<span style="color:#0D6EFD">priors 🛈</span>]{#prior} distributions as follows:
