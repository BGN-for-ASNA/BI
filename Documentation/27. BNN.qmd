...
In the Bayesian formulation, we place [<span style="color:#0D6EFD">priors 🛈</span>]{#prior} on all weights and biases and define a likelihood for the output. For a regression task with a one-hidden-layer BNN with $J$ neurons per hidden layer and a $5$-vector of predictors and a $2$-vector of outcomes we can run the model as below.
