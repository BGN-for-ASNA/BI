...
# Bayesian analysis
Bayesian analysis is a statistical approach that uses probability theory to update beliefs about the parameters of a model as new data becomes available. Bayesian methods have several key advantages as they allow [<span style="color:#0D6EFD">direct uncertainty quantification 🛈</span>]{#Uncertainty}, [<span style="color:#0D6EFD">incorporation of prior knowledge 🛈</span>]{#Uncertainty} and [<span style="color:#0D6EFD">flexibility for complex models 🛈</span>]{#Uncertainty}.
...
For each parameters of our equation that describe $\mu$ , we need to define a  [<span style="color:#0D6EFD">*prior distribution* 🛈</span>]{#prior} that encodes our initial beliefs about the parameter. In the case of the linear regression model, we need to specify *prior distributions* for ($\alpha$), slope ($\beta$), and standard deviation ($\sigma$):
...
Once data are observed, [<span style="color:#0D6EFD">*Bayes’ Theorem* 🛈</span>]{#BT} to evaluate how well a given set of parameters value fits the data:
