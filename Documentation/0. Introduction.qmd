...
```
Notice that for eah parameter declared in the model you will need to give it a unique name as well as a shape. The shape is the number of parameters you want to estimate. For example, if you want to estimate a different $\beta$ for each predictor, you would need to declare $\beta$ with a shape equal to the number of predictors. By default, the shape is one thus technically you don't need to specify it. In this example we only wanted to highlight this feature.
