...
[$$y \sim  Normal(μ,σ)
$$](bi\doc\0.%20%Introduction.md "This is the 'main' equation, which uses a specific distribution to account for uncertainty in the model.")

[$$ μ \sim α + βx
$$](bi\doc\0.%20%Introduction.md "This is the 'likelihood' that combines the different terms of the model")
...
We can also sample the posterior distribution and generate _dummy data_, which can help us to check and validate the model through _observations and p uncertainty propagation on the samples_. In some aspects, it is the opposite of a null model as it represents an expected model.


## 1.4 Link functions
We will see different families of regressions that have different distribtions. For the moment, we just need to know that those different distribtions required _link function (for each specific family we will discuss the corresponding link function):


## Vocabulary
This method evaluates if the variable that we want to predict -the dependent variable (_Y_)- and the variable(s) that may affect-independent variables (_Xs_)- this dependent variable is
