...
m.data_on_model = dict(
    idx = idx,
    result_outcomes = BEE.net.mat_to_edgl(data['outcomes']), 
    dyad_effects = BEE.net.prepare_dyadic_effect(kinship), # Can be a jax array of multiple dimensions
...
idx = BEE.net.vec_node_to_edgle(jnp.stack([ids, ids], axis = -1))
...
def model2(idx, result_outcomes, dyad_effects, focal_individual_predictors, target_individual_predictors):
    ...
    sr_rf, sr_raw, sr_sigma, sr_L = BEE.net.nodes_random_effects(N_id, cholesky_density = 2) 
    sender_receiver = sr_rf

    # Dyadic effect (D), its shape is equal to n dyads -----------------------------------------
    ## Varying intercept and slope for D
    rf, dr_raw, dr_sigma, dr_L = BEE.net.dyadic_random_effects(sender_receiver.shape[0], cholesky_density = 2)
    dr =  rf

    m.poisson(jnp.exp( sender_receiver + dr ), obs=result_outcomes)
...
model <- function(idx, idxShape, result_outcomes,focal_individual_predictors, target_individual_predictors){
  ...
  ## Block ---------------------------------------
  B = bee.dist.normal(tmp, 2.5, shape=c(1), name = 'block')
  
  #SR ---------------------------------------
  sr =  m$net$sender_receiver(focal_individual_predictors,target_individual_predictors)
  
  ### Dyadic--------------------------------------  
  #dr, dr_raw, dr_sigma, dr_L = BEE.net.dyadic_random_effects(idx.shape[0], cholesky_density = 2)# shape = n dyads
...
def model2(idx, result_outcomes, dyad_effects, focal_individual_predictors, target_individual_predictors):
    ...
    # Sender Receiver effect (SR), its shape is equal to N_id ---------------------------------------
    ## Covariates for SR
    sr_terms, focal_effects, target_effects = BEE.net.nodes_terms(focal_individual_predictors, target_individual_predictors) 
    ...
    # Dyadic effect (D), its shape is equal to n dyads -----------------------------------------
    ## Covariates for D
    dr_terms, dyad_effects = BEE.net.dyadic_terms(dyad_effects)
