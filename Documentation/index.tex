...
This document is a guide to Bayesian analysis and the implementation of
Bayesian inference (BI). It is intended for users ranging from those
with little or no experience to advanced practitioners. In this
introduction, we outline the main steps of Bayesian analysis. Subsequent
chapters present increasingly complex models. Each following chapter
will have the same structure in order to allow users to easily find the
information they are looking for. The structure is as follows:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \emph{Introduction to the model with non-mathematical explanations.}
\item
  \emph{Specific considerations for the model.}
\item
  \emph{Code to run the model.}
\item
  \emph{Mathematical formulas of the model.}
\item
  \emph{Notes related to the model.}
\end{enumerate}

We recommend reading the introduction first since some key concepts here
will not be revisited in later chapters.

\bookmarksetup{startatroot}

\chapter{Bayesian analysis}\label{bayesian-analysis}

Bayesian analysis is a statistical approach that uses probability theory
to update beliefs about the parameters of a model as new data becomes
available. Bayesian methods have several key advantages, as they allow
\phantomsection\label{Uncertainty}{{direct uncertainty quantification
🛈}}, \phantomsection\label{priorKnowledge}{{incorporation of prior
knowledge 🛈}} and \phantomsection\label{Flexibility}{{flexibility for
complex models 🛈}}.

\bookmarksetup{startatroot}

\chapter{Model set-up, Bayesian linear regression
example}\label{model-set-up-bayesian-linear-regression-example}

To illustrate the basics of setting up a Bayesian model, let's consider
a simple linear regression example. In this example, we build a simple
model where we predict a \phantomsection\label{DP}{{dependent variable
🛈}} \(y\) from a single \phantomsection\label{IP}{{independent variable
🛈}} \(x\). Here, we will not focus on the equations and their details
but rather use them to describe the different components of a Bayesian
model, how parameter optimization algorithms work in Bayesian analysis,
and provide basic technical vocabulary related to Bayesian methods. For
further details on the mathematical formulas of the model, please refer
to
\href{1.\%20Linear\%20Regression\%20for\%20continuous\%20variable.qmd}{Chapter
1: Linear Regression for continuous variable}.

\subsection{The Likelihood}\label{the-likelihood}

The \phantomsection\label{lk}{{\emph{likelihood} 🛈}} can be considered
as your main equation that combines the different terms of the model.
Depending on the type of problem you are trying to solve
(classification, regression, etc.) and the type of data you are working
with (continuous, discrete, binomial, etc.), the likelihood will be
different. Additionally, a model can have multiple likelihoods (e.g.,
network models).

As both the \emph{dependent variable} \(y\) and the \emph{independent
variable} \(x\) are continuous variables, we can use a \emph{Gaussian
model} to describe the relationship between these two variables. As we
are using Bayesian methods, we are describing this relationship using a
\phantomsection\label{pdist}{{\emph{probability distribution} 🛈}}:

\[
y \sim \text{Normal}(\mu, \sigma)
\]

Basically, our likelihood is saying that the \emph{dependent variable}
\(y\) is normally distributed with a mean \(\mu\) and a standard
deviation \(\sigma\). \emph{Probability distributions} are available in
BI through the class \texttt{bi.dist.XXX} where \texttt{XXX} is the name
of the distribution you want to use. Within any \texttt{bi.dist.XXX}
class, you can define which distribution needs to be used as a
likelihood by adding an argument \texttt{obs}, e.g.,
\texttt{m.normal(alpha\ +\ beta\ *\ x,\ sigma,\ obs=y)}. You can see
this as instructing the model to adjust its parameters to best explain
the observed dependent variable 𝑦, given the independent variable 𝑥, by
maximizing the likelihood.

\section{Modeling Likelihood}\label{modeling-likelihood}

Once the \emph{likelihood} is defined, we can now define the
mathematical equations that describe our parameters (\(\mu\) and
\(\sigma\)) and their relationship with the \emph{dependent variable}
\(y\). We can express this relationship in the form of a linear
function:
...
Where \(\alpha\) is the
\phantomsection\label{intercept}{{\emph{intercept} 🛈}} and \(\beta\) is
the \phantomsection\label{slope}{{\emph{slope} 🛈}} of the line. These
parameters are the \emph{unknowns} that we want to estimate to evaluate
the strength and direction of the relationship between the
\emph{independent variable} \(x\) and the \emph{dependent variable}
\(y\).
...
Depending on the type of problem you are trying to solve
(classification, regression, etc.) and the type of data you are working
with (continuous, discrete, binomial, etc.), you will need to choose the
appropriate distribution to describe the relationship between the data.
By using a different distribution, you will need to use a different
\phantomsection\label{linkF}{{\emph{link function} 🛈}}.

For the moment, we just need to know that these different distributions
require a \emph{link function} (for each specific family we will discuss
the corresponding link function); however, below is a table summarizing
some of the most common link functions, the mathematical form of each,
their typical applications, and how to interpret them. \emph{Link
functions} in BI can be accessed through the class \texttt{BI.link.XXX}
where \texttt{XXX} is the name of the link function.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2371}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3144}}@{}}
\toprule\noalign{}
...
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Typical Use / Model}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Interpretation \& Range}
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{Identity} & \(g(\mu) = \mu\) & Linear regression (Normal) &
Directly models \(\mu\); \(\mu\) can be any real number. \\
\textbf{Logit} & \(g(\mu) = \log\left(\frac{\mu}{1-\mu}\right)\) &
Logistic regression (Binomial) & Models probabilities (0, 1);
coefficients reflect log-odds. \\
\textbf{Probit} & \(g(\mu) = \Phi^{-1}(\mu)\) & Probit regression
(Binomial) & Similar to logit; uses the inverse standard normal CDF. \\
\textbf{Log} & \(g(\mu) = \log(\mu)\) & Poisson, Gamma regression (Count
data) & Ensures \(\mu > 0\); coefficients represent multiplicative
effects. \\
\textbf{Inverse} & \(g(\mu) = \frac{1}{\mu}\) & Gamma regression &
Models positive \(\mu\); relates changes inversely to \(\mu\). \\
\end{longtable}

\section{The Prior Distributions}\label{the-prior-distributions}

For each parameter of our equation that describes \(\mu\), we need to
define a \phantomsection\label{prior}{{\emph{prior distribution} 🛈}}
that encodes our initial beliefs about the parameter. In the case of the
linear regression model, we need to specify \emph{prior distributions}
for the intercept \(\alpha\), the slope \(\beta\), and standard
deviation \(\sigma\).
...
In BI, you will need to define this model within a function in which you
will be able to use any probability distributions, link functions, and
mathematical operations required for your model. BI has been designed to
allow you to declare your model as close as possible to the mathematical
notation. For example, the model above can be written in BI as:
...
\NormalTok{    x }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{178}\NormalTok{, }\DecValTok{20}\NormalTok{, sample }\OperatorTok{=} \VariableTok{True}\NormalTok{),   }
\NormalTok{    y }\OperatorTok{=}\NormalTok{ m.dist.lognormal( }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, sample }\OperatorTok{=} \VariableTok{True}\NormalTok{)   }
\NormalTok{)}
\NormalTok{m.data\_on\_model }\OperatorTok{=}\NormalTok{ dat}

\KeywordTok{def}\NormalTok{ model(x, y):    }
\NormalTok{    alpha }\OperatorTok{=}\NormalTok{ m.dist.normal( }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}alpha\textquotesingle{}}\NormalTok{, shape}\OperatorTok{=}\NormalTok{ (}\DecValTok{1}\NormalTok{,))}
\NormalTok{    beta }\OperatorTok{=}\NormalTok{ m.dist.normal( }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}beta\textquotesingle{}}\NormalTok{, shape}\OperatorTok{=}\NormalTok{ (}\DecValTok{1}\NormalTok{,))   }
\NormalTok{    sigma }\OperatorTok{=}\NormalTok{ m.dist.uniform( }\DecValTok{0}\NormalTok{, }\DecValTok{50}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}sigma\textquotesingle{}}\NormalTok{, shape }\OperatorTok{=}\NormalTok{ (}\DecValTok{1}\NormalTok{,))}
\NormalTok{    m.normal(alpha }\OperatorTok{+}\NormalTok{ beta }\OperatorTok{*}\NormalTok{ x, sigma, obs}\OperatorTok{=}\NormalTok{y)}
\end{Highlighting}
\end{Shaded}

The code snippet provides several key features of the \emph{BI} package:
...
\item
  Then, you can store data as a JAX array dictionary using the
  \texttt{m.data\_on\_model} function. If all the data can be stored in
  a data frame (e.g.,
  \href{1.\%20Linear\%20Regression\%20for\%20continuous\%20variable.qmd}{Linear
  Regression for continuous variable}), you do not need to use
  \texttt{m.data\_on\_model}, as the BI object automatically detects the
  data provided in the model arguments. However, sometimes you may need
  different data structures such as vectors and 2D arrays (e.g.,
  \href{20.\%20Network\%20model.qmd}{Network model}).
\item
  Regarding distribution parameters, note the difference depending on
  whether you are generating data outside a function (e.g., for
  simulation purposes) or specifying priors inside a model function. In
  the former case, the argument \texttt{sample} should be set to
  \texttt{True}. However, if you are specifying priors within a model
  function, this argument must be set to \texttt{False}.
\item
  Finally, note that each parameter declared in the model must have a
  unique \texttt{name} as well as a \texttt{shape}. The shape refers to
  the number of parameters you want to estimate. For example, if you
  want to estimate a different \(\beta\) for each independent variable,
  you would declare \(\beta\) with a shape equal to the number of
  independent variables. By default, the shape is one, so technically
  you don't need to specify it. In this example, we highlight this
  feature explicitly.
\end{enumerate}

\subsection{Which prior distribution range to
use?}\label{which-prior-distribution-range-to-use}

The choice of prior ranges can significantly affect Bayesian analysis
results. There are several approaches to selecting them:

\begin{itemize}
\item
  \textbf{Expert Knowledge}: The prior distributions can be based on
  expert knowledge or historical data. This approach is useful when
  there is a lot of information available about the parameters.
\item
  \textbf{Noninformative Priors}: When there is little or no information
  about the parameters, noninformative priors can be used. These priors
  are designed to have minimal influence on the posterior distribution,
  allowing the data to dominate the inference process.
\item
  \textbf{Scaled data}: If the data is
  \phantomsection\label{scaled}{{\emph{scaled} 🛈}}, the prior
  distributions can be chosen to reflect this. For example, if the data
  are scaled, the prior distributions for the intercept and slope can be
  centered around 0 and 1, respectively. By scaling the independent
  variable, we obtain a unit of change based on variance; that is, the
  effect represents a one--standard--deviation change in \(x\) on \(y\).
  Scaling the data improves both numerical stability and
  interpretability. When all data are scaled to the same range, it leads
  to more stable numerical behavior during estimation. Additionally, it
  facilitates setting priors that are both meaningful and relatively
  uninformative. By aligning the scale of the data with the scale
  assumed in the priors, we ensure that the posterior distributions
  exhibit reasonable spread and that our uncertainty quantification is
  consistent with the data's scale. For the remainder of this document,
  we will assume that the data are scaled.
\end{itemize}

\section{Model fit and posterior
distribution}\label{model-fit-and-posterior-distribution}

Once data are observed, \phantomsection\label{BT}{{\emph{Bayes' Theorem}
🛈}} is used to evaluate how well a given set of parameter values fits
the data:
...
\item
  \(P(\theta)\) is the \textbf{prior distribution}, representing our
  beliefs about \(\theta\) before seeing the data.
\item
  \(P(\text{data} \mid \theta)\) is the \textbf{likelihood},
  representing the model of how the data are generated given \(\theta\).
\item
  \(P(\theta \mid \text{data})\) is the \textbf{posterior distribution},
  representing our updated beliefs after observing the data. It tells us
  not only the most likely value of \(\theta\) (e.g., \(\alpha\),
  \(\beta\), and \(\sigma\) in our case) but also quantifies the
  uncertainty in these estimates.
\end{itemize}

We can use \phantomsection\label{BUpdating}{{\emph{Bayesian updating}
🛈}} using the \emph{Bayesian theorem} to `reshape' the prior
distributions by considering every possible combination of values for
our parameters, and scoring each combination by its relative
plausibility in light of the data. These relative plausibilities are the
posterior probabilities of each combination of our parameters: the
\emph{posterior distributions}. Various techniques can be used to
approximate the mathematical definition of Bayes' theorem: \emph{grid
approximation}, \emph{quadratic approximation}, and Markov chain Monte
Carlo (\emph{MCMC}). Descriptions of these algorithms are out of the
scope of this document. For more information, please refer to the
\href{https://en.wikipedia.org/wiki/Markov_chain_Monte_Carlo}{Bayesian
Inference}. In BI, we use MCMC and it can be called as
\texttt{m.run(model)} where \texttt{model} is the function that
describes the model.

\section{Model `diagnostic'}\label{model-diagnostic}

Once a Bayesian model has been fit, it is crucial to evaluate how well
it captures the observed data and to assess whether the Markov chain
Monte Carlo (MCMC) sampling has converged. Bayesian model diagnostics
help us answer questions like: ``Are our uncertainty estimates
reliable?'', ``Does the model generate data similar to what we
observed?'', and ``Have the chains mixed well?'' Multiple diagnostics
approaches can be used to assess the model's performance. Below are some
key diagnostic tools and techniques available in BI within the class
\texttt{BI.diag.XXX} where \texttt{XXX} is the name of the diagnostic
tool.
...
\bottomrule\noalign{}
\endlastfoot
\phantomsection\label{PPCs}{{\emph{posterior predictive checks} (PPCs)
🛈}} & Assess if the model can reproduce observed data & Graphs,
p-values, summary stats & Good fit if simulated data resemble observed
data \\
Credible Interval (CI) & Quantify uncertainty in parameter estimates &
95\% CI or other percentage & 95\% probability the parameter lies within
the interval \\
\phantomsection\label{HPDI}{{\emph{highest posterior density intervals}
(HPDI) 🛈}} & Identify the narrowest interval containing a given
probability mass & 95\% HPDI & Smallest interval capturing 95\% of the
posterior density \\
\phantomsection\label{ESS}{{\emph{effective sample size} (ESS) 🛈}} &
Measure independent information in the chain & ESS value (ideally high)
& Low ESS indicates high autocorrelation (poor mixing) \\
\phantomsection\label{Rhat}{{\emph{potential scale reduction factor}
(Rhat) 🛈}} & Check convergence across multiple chains & Rhat ≈ 1
(typically \textless1.1) & Values near 1 indicate convergence;
\textgreater\textgreater1 suggests non-convergence \\
\phantomsection\label{TP}{{\emph{Trace plots} 🛈}} & Visualize the
sampling path to check convergence and mixing & Plot showing parameter
values over iterations & Stationary, `hairy caterpillar' pattern
suggests convergence \\
\phantomsection\label{AutoCor}{{\emph{autocorrelation plots} 🛈}} &
Assess dependency between samples over lags & Autocorrelation values
across lags & Rapid decay to zero suggests good mixing; slow decay
indicates poor mixing \\
\phantomsection\label{DesPlot}{{\emph{density plots} 🛈}} & Visualize the
posterior distribution of a parameter & Smoothness and shape of the
curve & Unimodal and smooth suggests convergence; multimodal or
irregular may suggest poor mixing \\
\end{longtable}

\section{Model comparison}\label{model-comparison}

Model comparison is performed by evaluating how well different models
explain the observed data while accounting for model complexity.
Multiple criteria can be used to compare models, and are summarized in
the table below. In BI, we can compare models using
\emph{Watanabe-Akaike Information Criterion (WAIC)} with the function
\texttt{m.diag.waic(model1,\ model2)}.
...
\bottomrule\noalign{}
\endlastfoot
\textbf{DIC (Deviance Information Criterion)} & Measures model fit while
penalizing complexity & Lower values indicate better model fit & Simple
to compute, useful for hierarchical models & Sensitive to the number of
parameters, not always reliable in complex models \\
\textbf{WAIC (Watanabe-Akaike Information Criterion)} & Estimates
out-of-sample predictive accuracy while penalizing complexity & Lower
values indicate better models & More robust than DIC, accounts for
overfitting & Computationally intensive for large models \\
\textbf{Bayes Factor (BF)} & Quantifies relative support for two models
based on marginal likelihoods & \texttt{BF\ \textgreater{}\ 1} favors
the numerator model, \texttt{BF\ \textless{}\ 1} favors the denominator
...
\chapter{Linear Regression for a Continuous
Variable}\label{linear-regression-for-a-continuous-variable}
...
\begin{tcolorbox}[enhanced jigsaw, titlerule=0mm, coltitle=black, toprule=.15mm, bottomtitle=1mm, leftrule=.75mm, breakable, arc=.35mm, left=2mm, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Caution}, rightrule=.15mm, toptitle=1mm, colbacktitle=quarto-callout-caution-color!10!white, colframe=quarto-callout-caution-color-frame, colback=white, bottomrule=.15mm, opacityback=0, opacitybacktitle=0.6]
...
\item
  ...
  distributions 🛈}} for each model parameter, in this case for:
...
\item
  ...
  \item
    ...
    that is positively defined, such as the Exponential or Uniform
    distribution.
  \end{itemize}
\item
  ...
  🛈}} (see introduction). This simplifies interpretation, as
  coefficients represent direct changes in the outcome variable.
\end{itemize}
...
\NormalTok{data\_path }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}Howell1.csv\textquotesingle{}}
...
\KeywordTok{def}\NormalTok{ model(weight, height):    }
\NormalTok{    a }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{178}\NormalTok{, }\DecValTok{20}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{) }
\NormalTok{    b }\OperatorTok{=}\NormalTok{ m.dist.lognormal(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{) }
\NormalTok{    s }\OperatorTok{=}\NormalTok{ m.dist.uniform(}\DecValTok{0}\NormalTok{, }\DecValTok{50}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}s\textquotesingle{}}\NormalTok{) }
\NormalTok{    m.normal(a }\OperatorTok{+}\NormalTok{ b }\OperatorTok{*}\NormalTok{ weight , s, obs }\OperatorTok{=}\NormalTok{ height) }
...
\NormalTok{  s }\OtherTok{=} \FunctionTok{bi.dist.uniform}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{50}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}s\textquotesingle{}}\NormalTok{)}
\NormalTok{  a }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(}\DecValTok{178}\NormalTok{, }\DecValTok{20}\NormalTok{,  }\AttributeTok{name =} \StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{)}
\NormalTok{  b }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{)}
  
  \CommentTok{\# Likelihood}
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{normal}\NormalTok{(a }\SpecialCharTok{+}\NormalTok{ b }\SpecialCharTok{*}\NormalTok{ weight, s, }\AttributeTok{obs =}\NormalTok{ height)}
\NormalTok{\}}
...
\item
  \(\sigma_i\) is the error term for observation \emph{i}.
\end{itemize}
...
\item
  \(Y_i\) is the dependent variable for observation \emph{i}.
\item
  \(\alpha\) and \(\beta\) are the intercept and regression coefficient,
  respectively.
\item
  \(X_i\) is the input variable for observation \emph{i}.
\item
  \(\sigma\) is the standard deviation of the normal distribution, which
  describes the variance in the relationship between the dependent
  variable \(Y\) and the independent variable \(X\).
\end{itemize}

\section{Notes}\label{notes}

\begin{tcolorbox}[enhanced jigsaw, titlerule=0mm, coltitle=black, toprule=.15mm, bottomtitle=1mm, leftrule=.75mm, breakable, arc=.35mm, left=2mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, rightrule=.15mm, toptitle=1mm, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, colback=white, bottomrule=.15mm, opacityback=0, opacitybacktitle=0.6]
...
\item
  ...
  each one-unit increase in weight, holding all other variable(s)
  constant (age).
\end{itemize}
...
\NormalTok{    sigma }\OperatorTok{=}\NormalTok{ m.dist.uniform(}\DecValTok{0}\NormalTok{,}\DecValTok{50}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}sigma\textquotesingle{}}\NormalTok{)}
    \CommentTok{\# Likelihood}
\NormalTok{    m.normal(alpha }\OperatorTok{+}\NormalTok{ beta1 }\OperatorTok{*}\NormalTok{ weight }\OperatorTok{+}\NormalTok{ beta2 }\OperatorTok{*}\NormalTok{ age, sigma, obs }\OperatorTok{=}\NormalTok{ height)}
...
\CommentTok{\# Import Data \& Data Manipulation {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{m}\SpecialCharTok{$}\FunctionTok{data}\NormalTok{(}\FunctionTok{paste}\NormalTok{(}\FunctionTok{system.file}\NormalTok{(}\AttributeTok{package =} \StringTok{"BI"}\NormalTok{),}\StringTok{"/data/Howell1.csv"}\NormalTok{, }\AttributeTok{sep =} \StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{), }\AttributeTok{sep=}\StringTok{\textquotesingle{};\textquotesingle{}}\NormalTok{)}\CommentTok{\# Import}
...
\NormalTok{  sigma }\OtherTok{=} \FunctionTok{bi.dist.uniform}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{50}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}s\textquotesingle{}}\NormalTok{)}
  \CommentTok{\# Likelihood}
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{normal}\NormalTok{(alpha }\SpecialCharTok{+}\NormalTok{ beta1 }\SpecialCharTok{*}\NormalTok{ weight }\SpecialCharTok{+}\NormalTok{ beta2 }\SpecialCharTok{*}\NormalTok{ age, sigma, }\AttributeTok{obs=}\NormalTok{height)}
\NormalTok{\}}
...
\item
  \(\beta_k\) are the prior distributions for the \emph{k} distinct
...
To study the relationships between two independent continuous variables
and their interaction effect on a dependent variable (e.g., temperature
and humidity affecting energy consumption), we can use Regression
Analysis with Interaction Terms. In this approach, we extend the simple
linear regression model to include an interaction term (a
multiplication) between the two continuous variables.
...
\item
  ...
  for continuous variable}.
\item
  We wish to model the relationship between dependent variable \emph{Y}
  and independent variable \(X_1\) to vary as a function of independent
  variable \(X_2\). To do this, we explicitly model the hypothesis that
  the slope between \emph{Y} and \(X_1\) depends---is conditional---upon
  \(X_2\).
\item
  ...
  variable.
\item
  The interpretation of estimates is more complex. The estimate for a
  non-interaction term reflects the expected change in \emph{Y} when
  \(X_1\) increases by one unit, holding \(X_2\) constant at its average
  value. The estimate for the interaction term represents how the effect
  of \(X_1\) on \emph{Y} changes depending on the value of \(X_2\), and
...
Inference (BI) package. The data consist of three continuous variables
...
\CommentTok{\# Import}
\ImportTok{from}\NormalTok{ importlib.resources }\ImportTok{import}\NormalTok{ files}
\NormalTok{data\_path }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}tulips.csv\textquotesingle{}}
\NormalTok{m.data(data\_path, sep}\OperatorTok{=}\StringTok{\textquotesingle{};\textquotesingle{}}\NormalTok{)}
...
\NormalTok{    sigma }\OperatorTok{=}\NormalTok{ m.dist.exponential(}\DecValTok{1}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}sigma\textquotesingle{}}\NormalTok{, shape }\OperatorTok{=}\NormalTok{ (}\DecValTok{1}\NormalTok{,))}
\NormalTok{    bws }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\FloatTok{0.25}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}bws\textquotesingle{}}\NormalTok{, shape }\OperatorTok{=}\NormalTok{ (}\DecValTok{1}\NormalTok{,))}
\NormalTok{    bs }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\FloatTok{0.25}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}bs\textquotesingle{}}\NormalTok{, shape }\OperatorTok{=}\NormalTok{ (}\DecValTok{1}\NormalTok{,))}
\NormalTok{    bw }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\FloatTok{0.25}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}bw\textquotesingle{}}\NormalTok{, shape }\OperatorTok{=}\NormalTok{ (}\DecValTok{1}\NormalTok{,))}
\NormalTok{    a }\OperatorTok{=}\NormalTok{ m.dist.normal(}\FloatTok{0.5}\NormalTok{, }\FloatTok{0.25}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{, shape }\OperatorTok{=}\NormalTok{ (}\DecValTok{1}\NormalTok{,))}
\NormalTok{    mu }\OperatorTok{=}\NormalTok{ a }\OperatorTok{+}\NormalTok{ bw}\OperatorTok{*}\NormalTok{water }\OperatorTok{+}\NormalTok{ bs}\OperatorTok{*}\NormalTok{shade }\OperatorTok{+}\NormalTok{ bws}\OperatorTok{*}\NormalTok{water}\OperatorTok{*}\NormalTok{shade}
\NormalTok{    m.normal(mu, sigma, obs}\OperatorTok{=}\NormalTok{blooms)}
...
\NormalTok{model }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(blooms, water,shade)\{}
  \CommentTok{\# Parameter prior distributions}
\NormalTok{  alpha }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{( }\FloatTok{0.5}\NormalTok{, }\FloatTok{0.25}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{)}
\NormalTok{  beta1 }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{( }\DecValTok{0}\NormalTok{,  }\FloatTok{0.25}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}b1\textquotesingle{}}\NormalTok{)}
\NormalTok{  beta2 }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(  }\DecValTok{0}\NormalTok{,  }\FloatTok{0.25}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}b2\textquotesingle{}}\NormalTok{)}
\NormalTok{  beta\_interaction\_ }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(  }\DecValTok{0}\NormalTok{, }\FloatTok{0.25}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}bint\textquotesingle{}}\NormalTok{)}
\NormalTok{  sigma }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{50}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}s\textquotesingle{}}\NormalTok{)}
  \CommentTok{\# Likelihood}
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{normal}\NormalTok{(alpha }\SpecialCharTok{+}\NormalTok{ beta1}\SpecialCharTok{*}\NormalTok{water }\SpecialCharTok{+}\NormalTok{ beta2}\SpecialCharTok{*}\NormalTok{shade }\SpecialCharTok{+}\NormalTok{ beta\_interaction\_}\SpecialCharTok{*}\NormalTok{water}\SpecialCharTok{*}\NormalTok{shade, sigma, }\AttributeTok{obs=}\NormalTok{blooms)}
\NormalTok{\}}
...
In the Bayesian formulation, we define each parameter with
\phantomsection\label{prior}{{priors 🛈}}. We can express the Bayesian
regression model accounting for prior distributions as follows:
...
Below is an example of code that demonstrates Bayesian regression with
...
\CommentTok{\# Import}
\ImportTok{from}\NormalTok{ importlib.resources }\ImportTok{import}\NormalTok{ files}
\NormalTok{data\_path }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}milk.csv\textquotesingle{}}
\NormalTok{m.data(data\_path, sep}\OperatorTok{=}\StringTok{\textquotesingle{};\textquotesingle{}}\NormalTok{) }
...
\NormalTok{    a }\OperatorTok{=}\NormalTok{ m.bi.dist.normal(}\DecValTok{0}\NormalTok{, }\FloatTok{0.5}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(}\DecValTok{4}\NormalTok{,), name }\OperatorTok{=} \StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{) }\CommentTok{\# shape based on the number of clades}
\NormalTok{    s }\OperatorTok{=}\NormalTok{ m.bi.dist.exponential( }\DecValTok{1}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}s\textquotesingle{}}\NormalTok{)    }
\NormalTok{    mu }\OperatorTok{=}\NormalTok{ a[index\_clade]}
\NormalTok{    m.normal(mu, s, obs}\OperatorTok{=}\NormalTok{kcal\_per\_g)}
...
\NormalTok{m}\SpecialCharTok{$}\FunctionTok{scale}\NormalTok{(}\FunctionTok{list}\NormalTok{(}\StringTok{\textquotesingle{}kcal.per.g\textquotesingle{}}\NormalTok{)) }\CommentTok{\# Manipulate}
...
\NormalTok{model }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(kcal\_per\_g, index\_clade)\{}
  \CommentTok{\# Parameter prior distributions}
\NormalTok{  beta }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{( }\DecValTok{0}\NormalTok{, }\FloatTok{0.5}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}beta\textquotesingle{}}\NormalTok{, }\AttributeTok{shape =} \FunctionTok{c}\NormalTok{(}\DecValTok{4}\NormalTok{))  }\CommentTok{\# shape based on the number of clades}
\NormalTok{  sigma }\OtherTok{=}\FunctionTok{bi.dist.exponential}\NormalTok{(}\DecValTok{1}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}s\textquotesingle{}}\NormalTok{)}
  \CommentTok{\# Likelihood}
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{normal}\NormalTok{(beta[index\_clade], sigma, }\AttributeTok{obs=}\NormalTok{kcal\_per\_g)}
\NormalTok{\}}
...
we can use a \emph{Binomial model}.
...
\item
  ...
  for continuous variable}.
\item
  The first link function is the \emph{logit}. The \emph{logit} link
  function in the Bayesian binomial model converts the linear
  combination of predictor variables into probabilities, making it
  suitable for modeling binary outcomes. It helps to estimate the
  relationship between predictors and the probability of success,
  ensuring results fall within the bounds of the binomial distribution
  \(\in[0,1]\).
\end{itemize}
...
which side individuals pulled. The goal is to evaluate the probability
of pulling the left side.
...
\CommentTok{\# setup platform{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{m }\OperatorTok{=}\NormalTok{ bi(platform}\OperatorTok{=}\StringTok{\textquotesingle{}cpu\textquotesingle{}}\NormalTok{)}
\CommentTok{\# import data {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\CommentTok{\# Import}
\ImportTok{from}\NormalTok{ importlib.resources }\ImportTok{import}\NormalTok{ files}
\NormalTok{data\_path }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}chimpanzees.csv\textquotesingle{}}
\NormalTok{m.data(data\_path, sep}\OperatorTok{=}\StringTok{\textquotesingle{};\textquotesingle{}}\NormalTok{)}
...
\NormalTok{    a }\OperatorTok{=}\NormalTok{ m.dist.normal( }\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(}\DecValTok{1}\NormalTok{,), name }\OperatorTok{=} \StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{)}
\NormalTok{    m.binomial(logits}\OperatorTok{=}\NormalTok{a[}\DecValTok{0}\NormalTok{], obs}\OperatorTok{=}\NormalTok{pulled\_left)}
...
\NormalTok{  alpha }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{( }\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}alpha\textquotesingle{}}\NormalTok{)}
  \CommentTok{\# Likelihood}
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{binomial}\NormalTok{(}\AttributeTok{logits =}\NormalTok{ alpha, }\AttributeTok{obs=}\NormalTok{pulled\_left)}
\NormalTok{\}}
...
\CommentTok{\# Summary {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{m}\SpecialCharTok{$}\FunctionTok{summary}\NormalTok{() }\CommentTok{\# Get posterior distribution}
...
\item
  ...
  regression coefficient and intercept, respectively.
\item
...
\item
  ...
  (\emph{pulled\_left}), which represents which side individuals pulled,
...
\ImportTok{from}\NormalTok{ BI }\ImportTok{import}\NormalTok{ bi}
\NormalTok{m }\OperatorTok{=}\NormalTok{ bi(platform}\OperatorTok{=}\StringTok{\textquotesingle{}cpu\textquotesingle{}}\NormalTok{)}
\NormalTok{m.data(}\StringTok{\textquotesingle{}../resources/data/chimpanzees.csv\textquotesingle{}}\NormalTok{, sep}\OperatorTok{=}\StringTok{\textquotesingle{};\textquotesingle{}}\NormalTok{)}
\NormalTok{m.df[}\StringTok{\textquotesingle{}treatment\textquotesingle{}}\NormalTok{] }\OperatorTok{=}\NormalTok{  m.df.prosoc\_left }\OperatorTok{+} \DecValTok{2} \OperatorTok{*}\NormalTok{ m.df.condition}
\NormalTok{m.df[}\StringTok{\textquotesingle{}actor\textquotesingle{}}\NormalTok{] }\OperatorTok{=}\NormalTok{ m.df[}\StringTok{\textquotesingle{}actor\textquotesingle{}}\NormalTok{] }\OperatorTok{{-}} \DecValTok{1}

\NormalTok{m.data\_to\_model([}\StringTok{\textquotesingle{}actor\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}treatment\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}pulled\_left\textquotesingle{}}\NormalTok{])}

\KeywordTok{def}\NormalTok{ model(actor, treatment, pulled\_left):}
\NormalTok{    a }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\FloatTok{1.5}\NormalTok{, shape }\OperatorTok{=}\NormalTok{ (}\DecValTok{7}\NormalTok{,), name}\OperatorTok{=}\StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{)}
\NormalTok{    b }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\FloatTok{0.5}\NormalTok{, shape }\OperatorTok{=}\NormalTok{ (}\DecValTok{4}\NormalTok{,), name}\OperatorTok{=}\StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{)}
\NormalTok{    p }\OperatorTok{=}\NormalTok{ a[actor] }\OperatorTok{+}\NormalTok{ b[treatment]}
\NormalTok{    m.lk(}\StringTok{"y"}\NormalTok{, m.dist.binomial(}\DecValTok{1}\NormalTok{, logits}\OperatorTok{=}\NormalTok{p), obs}\OperatorTok{=}\NormalTok{pulled\_left)}
...
\item
  A Beta distribution has two parameters: the rates for each probability
  and a shape parameter θ. θ influences how probabilities are
  distributed between 0 and 1. Specifically, it consists of two
  parameters, \(\alpha\) and \(\beta\), which determine the
  concentration of probability around 0 and 1.

  \begin{itemize}
  \tightlist
  \item
    If both are equal to or greater than 1, the distribution is
    bell-shaped and centered around 0.5.
  \item
    If \(\alpha > \beta\), the distribution is skewed toward 1, and if
    \(\beta > \alpha\), it is skewed toward 0. Thus, the shape
    parameters \(\gamma\) and \(\eta\) provide flexibility in modeling
    various types of prior beliefs about probabilities.
  \end{itemize}
\end{itemize}
...
\item
  ...
  gender, which will be used to account for independent rates.
\end{enumerate}
...
\CommentTok{\# Import}
\ImportTok{from}\NormalTok{ importlib.resources }\ImportTok{import}\NormalTok{ files}
\NormalTok{data\_path }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}UCBadmit.csv\textquotesingle{}}
\NormalTok{m.data(data\_path, sep}\OperatorTok{=}\StringTok{\textquotesingle{};\textquotesingle{}}\NormalTok{) }
...
\NormalTok{    phi }\OperatorTok{=}\NormalTok{ m.dist.exponential(}\DecValTok{1}\NormalTok{,  name }\OperatorTok{=} \StringTok{\textquotesingle{}phi\textquotesingle{}}\NormalTok{)}
\NormalTok{    alpha }\OperatorTok{=}\NormalTok{ m.dist.normal( }\FloatTok{0.}\NormalTok{, }\FloatTok{1.5}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(}\DecValTok{2}\NormalTok{,), name }\OperatorTok{=} \StringTok{\textquotesingle{}alpha\textquotesingle{}}\NormalTok{)}
...
\NormalTok{    m.dist.betabinomial(total\_count }\OperatorTok{=}\NormalTok{ applications, concentration1 }\OperatorTok{=}\NormalTok{ concentration1, concentration0 }\OperatorTok{=}\NormalTok{ concentration0, obs}\OperatorTok{=}\NormalTok{admit)}
...
\NormalTok{  eta }\OtherTok{=}\NormalTok{ (}\DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ pbar) }\SpecialCharTok{*}\NormalTok{ t}
  \CommentTok{\# Likelihood}
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{betabinomial}\NormalTok{(}\AttributeTok{total\_count=}\NormalTok{applications, }\AttributeTok{concentration1=}\NormalTok{gamma, }\AttributeTok{concentration0=}\NormalTok{eta, }\AttributeTok{obs=}\NormalTok{admit)}
\NormalTok{\}}
...
\begin{itemize}
\item
  \(Y_i\) is the count of successes for the \emph{i}-th observation,
...
\item
  α is a vector of parameters, each representing the effect of group
  \emph{i} on the success probability.
\item
...
\item
  ...
  \emph{log}. The \emph{log} link ensures that \emph{λ} is always
  positive.
\item
  To invert the log link function and linearly model the relationship
...
\CommentTok{\# Import}
\ImportTok{from}\NormalTok{ importlib.resources }\ImportTok{import}\NormalTok{ files}
\NormalTok{data\_path }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}Kline.csv\textquotesingle{}}
\NormalTok{m.data(data\_path, sep}\OperatorTok{=}\StringTok{\textquotesingle{};\textquotesingle{}}\NormalTok{)}
\NormalTok{m.scale([}\StringTok{\textquotesingle{}population\textquotesingle{}}\NormalTok{])}
...
\CommentTok{\#m.data\_to\_model([\textquotesingle{}total\_tools\textquotesingle{}, \textquotesingle{}population\textquotesingle{}, \textquotesingle{}cid\textquotesingle{}])}
\KeywordTok{def}\NormalTok{ model(cid, population, total\_tools):}
\NormalTok{    a }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{3}\NormalTok{, }\FloatTok{0.5}\NormalTok{, shape}\OperatorTok{=}\NormalTok{ (}\DecValTok{2}\NormalTok{,), name}\OperatorTok{=}\StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{)}
\NormalTok{    b }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\FloatTok{0.2}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(}\DecValTok{2}\NormalTok{,), name}\OperatorTok{=}\StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{)}
\NormalTok{    l }\OperatorTok{=}\NormalTok{ jnp.exp(a[cid] }\OperatorTok{+}\NormalTok{ b[cid]}\OperatorTok{*}\NormalTok{population)}
\NormalTok{    m.poisson(l, obs}\OperatorTok{=}\NormalTok{total\_tools)}
...
\NormalTok{model }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(total\_tools, population, cid)\{}
  \CommentTok{\# Parameter prior distributions}
\NormalTok{  alpha }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(}\DecValTok{3}\NormalTok{, }\FloatTok{0.5}\NormalTok{, }\AttributeTok{name=}\StringTok{\textquotesingle{}alpha\textquotesingle{}}\NormalTok{, }\AttributeTok{shape =} \FunctionTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{))}
\NormalTok{  beta }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FloatTok{0.2}\NormalTok{, }\AttributeTok{name=}\StringTok{\textquotesingle{}beta\textquotesingle{}}\NormalTok{, }\AttributeTok{shape =} \FunctionTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{))}
\NormalTok{  l }\OtherTok{=}\NormalTok{ jnp}\SpecialCharTok{$}\FunctionTok{exp}\NormalTok{(alpha[cid] }\SpecialCharTok{+}\NormalTok{ beta[cid]}\SpecialCharTok{*}\NormalTok{population)}
  \CommentTok{\# Likelihood}
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{poisson}\NormalTok{(l, }\AttributeTok{obs=}\NormalTok{total\_tools)}
\NormalTok{\}}
...
\item
  ...
  and the regression coefficients, respectively.
\item
  \(\lambda_i\) is the mean rate parameter of the Poisson distribution,
...
Below is an example code snippet demonstrating a Bayesian Gamma-Poisson
...
\ImportTok{from}\NormalTok{ BI }\ImportTok{import}\NormalTok{ bi}
\CommentTok{\# Setup device {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{m }\OperatorTok{=}\NormalTok{ bi(platform}\OperatorTok{=}\StringTok{\textquotesingle{}cpu\textquotesingle{}}\NormalTok{) }\CommentTok{\# Import}

\CommentTok{\# Import Data \& Data Manipulation {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\CommentTok{\# Import}
\ImportTok{from}\NormalTok{ importlib.resources }\ImportTok{import}\NormalTok{ files}
\NormalTok{data\_path }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}Sim dat Gamma poisson.csv\textquotesingle{}}
\NormalTok{m.data(data\_path, sep}\OperatorTok{=}\StringTok{\textquotesingle{},\textquotesingle{}}\NormalTok{) }
...
\CommentTok{\# Define model {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\KeywordTok{def}\NormalTok{ model(log\_days, monastery, y):}
\NormalTok{    a }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(}\DecValTok{1}\NormalTok{,))}
\NormalTok{    b }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(}\DecValTok{1}\NormalTok{,))}
\NormalTok{    l }\OperatorTok{=}\NormalTok{ m.jnp.exp(log\_days }\OperatorTok{+}\NormalTok{ a }\OperatorTok{+}\NormalTok{ b }\OperatorTok{*}\NormalTok{ monastery)}
\NormalTok{    m.poisson(rate }\OperatorTok{=}\NormalTok{ l, obs}\OperatorTok{=}\NormalTok{y)}
\CommentTok{\# Run MCMC {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{m.run(model) }\CommentTok{\# Optimize model parameters through MCMC sampling}
...
\NormalTok{  beta }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\AttributeTok{name=}\StringTok{\textquotesingle{}beta\textquotesingle{}}\NormalTok{, }\AttributeTok{shape=}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{))}
\NormalTok{  l }\OtherTok{=}\NormalTok{ jnp}\SpecialCharTok{$}\FunctionTok{exp}\NormalTok{(log\_days }\SpecialCharTok{+}\NormalTok{ alpha }\SpecialCharTok{+}\NormalTok{ beta }\SpecialCharTok{*}\NormalTok{ monastery)}
  \CommentTok{\# Likelihood}
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{poisson}\NormalTok{(}\AttributeTok{rate=}\NormalTok{l, }\AttributeTok{obs=}\NormalTok{y)}
\NormalTok{\}}
...
\item
  ...
  ``jax.nn.softmax'' line in the code).
\item
...
Below is an example code snippet demonstrating a Bayesian multinomial
model using the Bayesian Inference (BI) package:
...
\CommentTok{\# Import}
\ImportTok{from}\NormalTok{ importlib.resources }\ImportTok{import}\NormalTok{ files}
\NormalTok{data\_path }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}Sim data multinomial.csv\textquotesingle{}}
\NormalTok{m.data(data\_path, sep}\OperatorTok{=}\StringTok{\textquotesingle{},\textquotesingle{}}\NormalTok{) }

\CommentTok{\# Define model {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\KeywordTok{def}\NormalTok{ model(career, income):}
\NormalTok{    a }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(}\DecValTok{2}\NormalTok{,), name }\OperatorTok{=} \StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{)}
\NormalTok{    b }\OperatorTok{=}\NormalTok{ m.dist.halfnormal(}\FloatTok{0.5}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(}\DecValTok{1}\NormalTok{,), name }\OperatorTok{=} \StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{)}
...
\NormalTok{    s\_3 }\OperatorTok{=}\NormalTok{ [}\DecValTok{0}\NormalTok{] }\CommentTok{\#pivot}
...
\NormalTok{    m.dist.categorical(probs}\OperatorTok{=}\NormalTok{p, obs}\OperatorTok{=}\NormalTok{career)}
...
\CommentTok{\# import data {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{m}\SpecialCharTok{$}\FunctionTok{data}\NormalTok{(}\FunctionTok{paste}\NormalTok{(}\FunctionTok{system.file}\NormalTok{(}\AttributeTok{package =} \StringTok{"BI"}\NormalTok{),}\StringTok{"/data/Sim data multinomial.csv"}\NormalTok{, }\AttributeTok{sep =} \StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{), }\AttributeTok{sep=}\StringTok{\textquotesingle{},\textquotesingle{}}\NormalTok{)}
...
\NormalTok{values }\OtherTok{\textless{}{-}} \FunctionTok{list}\NormalTok{(jnp}\SpecialCharTok{$}\FunctionTok{array}\NormalTok{(}\FunctionTok{as.integer}\NormalTok{(income)),jnp}\SpecialCharTok{$}\FunctionTok{array}\NormalTok{( }\FunctionTok{as.integer}\NormalTok{(m}\SpecialCharTok{$}\NormalTok{df}\SpecialCharTok{$}\NormalTok{career)))}
\NormalTok{m}\SpecialCharTok{$}\NormalTok{data\_on\_model }\OtherTok{=} \FunctionTok{py\_dict}\NormalTok{(keys, values, }\AttributeTok{convert =} \ConstantTok{TRUE}\NormalTok{)}

\CommentTok{\# Define model {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{model }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(income, career)\{}
  \CommentTok{\# Parameter prior distributions}
\NormalTok{  alpha }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\AttributeTok{name=}\StringTok{\textquotesingle{}alpha\textquotesingle{}}\NormalTok{, }\AttributeTok{shape =} \FunctionTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{))}
\NormalTok{  beta }\OtherTok{=} \FunctionTok{bi.dist.halfnormal}\NormalTok{(}\FloatTok{0.5}\NormalTok{, }\AttributeTok{name=}\StringTok{\textquotesingle{}beta\textquotesingle{}}\NormalTok{)}
  
\NormalTok{  s\_1 }\OtherTok{=}\NormalTok{ alpha[}\DecValTok{0}\NormalTok{] }\SpecialCharTok{+}\NormalTok{ beta }\SpecialCharTok{*}\NormalTok{ income[}\DecValTok{0}\NormalTok{]}
...
\NormalTok{  p }\OtherTok{=}\NormalTok{ jax}\SpecialCharTok{$}\NormalTok{nn}\SpecialCharTok{$}\FunctionTok{softmax}\NormalTok{(jnp}\SpecialCharTok{$}\FunctionTok{stack}\NormalTok{(}\FunctionTok{list}\NormalTok{(s\_1, s\_2, s\_3)))}

  \CommentTok{\# Likelihood}
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{categorical}\NormalTok{(}\AttributeTok{probs=}\NormalTok{p[career], }\AttributeTok{obs=}\NormalTok{career)}
\NormalTok{\}}
...
\begin{itemize}
\item
  \(p_{ik}\) is the probability of the 𝑖-th observation being in
  category 𝑘.
\item
...
function is: \[
Multinomial(y|θ)=\frac{N!}{\prod^K_{k=1}y_k!} \prod_{k=1}^{K} θ_{k}^{y_k}
\]
...
\item
  \(\frac{N!}{\prod^K_{k=1}y_k!}\) is the multinomial coefficient that
...
\item
  We have the same considerations as for the
  \href{9.\%20Multinomial\%20model.qmd}{Multinomial model}.
\item
...
\KeywordTok{def}\NormalTok{ model(income, career):}
    \CommentTok{\# Parameter prior distributions}
\NormalTok{    alpha }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(}\DecValTok{2}\NormalTok{,), name}\OperatorTok{=}\StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{)}
\NormalTok{    beta }\OperatorTok{=}\NormalTok{ m.dist.halfnormal(}\FloatTok{0.5}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(}\DecValTok{1}\NormalTok{,), name}\OperatorTok{=}\StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{)}
...
\NormalTok{    p }\OperatorTok{=}\NormalTok{ jax.nn.exp(jnp.stack([s\_1[}\DecValTok{0}\NormalTok{], s\_2[}\DecValTok{0}\NormalTok{], s\_3[}\DecValTok{0}\NormalTok{]]))}
    \CommentTok{\# Likelihood}
\NormalTok{    m.dirichletmultinomial(p[career], lambda\_, obs}\OperatorTok{=}\NormalTok{career)}
...
\NormalTok{drink }\OperatorTok{=}\NormalTok{ m.dist.binomial(}\DecValTok{1}\NormalTok{, prob\_drink, shape }\OperatorTok{=}\NormalTok{ (N,), sample }\OperatorTok{=} \VariableTok{True}\NormalTok{)}
\NormalTok{y }\OperatorTok{=}\NormalTok{ (}\DecValTok{1} \OperatorTok{{-}}\NormalTok{ drink) }\OperatorTok{*}\NormalTok{ m.dist.poisson(rate\_work, shape }\OperatorTok{=}\NormalTok{ (N,), sample }\OperatorTok{=} \VariableTok{True}\NormalTok{)}
...
\NormalTok{    m.zeroinflatedpoisson(p, lambda\_, obs}\OperatorTok{=}\NormalTok{y)}
...
\NormalTok{drink }\OtherTok{=} \FunctionTok{bi.dist.binomial}\NormalTok{(}\AttributeTok{total\_count =} \FunctionTok{as.integer}\NormalTok{(}\DecValTok{1}\NormalTok{), }\AttributeTok{probs =}\NormalTok{ prob\_drink, }\AttributeTok{shape =} \FunctionTok{c}\NormalTok{(N), }\AttributeTok{sample =}\NormalTok{ T ) }\CommentTok{\# An example of sampling a distribution with BI}
\NormalTok{y }\OtherTok{=}\NormalTok{ (}\DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ drink) }\SpecialCharTok{*}  \FunctionTok{bi.dist.poisson}\NormalTok{(rate\_work, }\AttributeTok{shape =} \FunctionTok{c}\NormalTok{(N), }\AttributeTok{sample =}\NormalTok{ T)}
...
\NormalTok{  al }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(}\DecValTok{1}\NormalTok{, }\FloatTok{0.5}\NormalTok{, }\AttributeTok{name=}\StringTok{\textquotesingle{}al\textquotesingle{}}\NormalTok{, }\AttributeTok{shape=}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{))}
\NormalTok{  ap }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{, }\AttributeTok{name=}\StringTok{\textquotesingle{}ap\textquotesingle{}}\NormalTok{, }\AttributeTok{shape=}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{))}
\NormalTok{  p }\OtherTok{=}\NormalTok{ jax}\SpecialCharTok{$}\NormalTok{scipy}\SpecialCharTok{$}\NormalTok{special}\SpecialCharTok{$}\FunctionTok{expit}\NormalTok{(ap)}
\NormalTok{  lambda\_ }\OtherTok{=}\NormalTok{ jnp}\SpecialCharTok{$}\FunctionTok{exp}\NormalTok{(al)}
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{zeroinflatedpoisson}\NormalTok{(p, lambda\_, }\AttributeTok{obs=}\NormalTok{y)}
\NormalTok{\}}
...
\item
  \(X_i\) are the independent variables' values for observation
  \emph{i}.
\end{itemize}
...
\item
  ...
  covariates, random effects, and incorporate uncertainty more naturally
...
Here's an example of a Bayesian survival analysis using the
\textbf{Bayesian Inference (BI)} package. The data come from a clinical
trial of mastectomy for breast cancer. The goal is to estimate the
effect of the \texttt{metastasized} covariate, coded as 0 (no
metastasis) and 1 (metastasis), on the survival outcome \texttt{event}
for each patient. Time is continuous and censoring is indicated by the
event variable.
...
\CommentTok{\# Import}
\ImportTok{from}\NormalTok{ importlib.resources }\ImportTok{import}\NormalTok{ files}
\NormalTok{data\_path }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}mastectomy.csv\textquotesingle{}}
\NormalTok{m.data(data\_path, sep}\OperatorTok{=}\StringTok{\textquotesingle{},\textquotesingle{}}\NormalTok{) }
...
\NormalTok{    lambda0 }\OperatorTok{=}\NormalTok{ m.dist.gamma(}\FloatTok{0.01}\NormalTok{, }\FloatTok{0.01}\NormalTok{, shape}\OperatorTok{=}\NormalTok{ intervals.shape, name }\OperatorTok{=} \StringTok{\textquotesingle{}lambda0\textquotesingle{}}\NormalTok{)}
    \CommentTok{\#\# Covariate effect distribution}
\NormalTok{    beta }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\DecValTok{1000}\NormalTok{, shape }\OperatorTok{=}\NormalTok{ (}\DecValTok{1}\NormalTok{,),  name}\OperatorTok{=}\StringTok{\textquotesingle{}beta\textquotesingle{}}\NormalTok{)}
    \CommentTok{\#\#\# Likelihood}
...
\NormalTok{    y }\OperatorTok{=}\NormalTok{ m.poisson(mu }\OperatorTok{+}\NormalTok{ jnp.finfo(mu.dtype).tiny, obs }\OperatorTok{=}\NormalTok{ death)}
...
\item
  ...
  \item
    ...
    the hazard are assigned a normal prior.
  \item
...
To model the relationship between a dependent variable and an
independent variable while allowing for different intercepts across
groups or clusters, we can use a \emph{Varying Intercepts} model. This
approach is particularly useful when data is grouped (e.g., by subject,
location, or time period) and we expect the baseline level of the
outcome to vary across these groups.
...
\item
  ...
  the intercept \(\alpha_k\) is defined based on the \(k\) declared
  groups.
\item
...
consists of a dependent variable representing individuals' survival
...
\CommentTok{\# Import}
\ImportTok{from}\NormalTok{ importlib.resources }\ImportTok{import}\NormalTok{ files}
\NormalTok{data\_path }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}reedfrogs.csv\textquotesingle{}}
\NormalTok{m.data(data\_path, sep}\OperatorTok{=}\StringTok{\textquotesingle{};\textquotesingle{}}\NormalTok{) }
\CommentTok{\# Manipulate}
\NormalTok{m.df[}\StringTok{"tank"}\NormalTok{] }\OperatorTok{=}\NormalTok{ np.arange(m.df.shape[}\DecValTok{0}\NormalTok{]) }

\CommentTok{\# Define model {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\KeywordTok{def}\NormalTok{ model(tank, surv, density):}
\NormalTok{    sigma }\OperatorTok{=}\NormalTok{ m.dist.exponential( }\DecValTok{1}\NormalTok{,  name }\OperatorTok{=} \StringTok{\textquotesingle{}sigma\textquotesingle{}}\NormalTok{)}
\NormalTok{    a\_bar }\OperatorTok{=}\NormalTok{ m.dist.normal( }\FloatTok{0.}\NormalTok{, }\FloatTok{1.5}\NormalTok{,  name }\OperatorTok{=} \StringTok{\textquotesingle{}a\_bar\textquotesingle{}}\NormalTok{)}
\NormalTok{    alpha }\OperatorTok{=}\NormalTok{ m.dist.normal( a\_bar, sigma, shape}\OperatorTok{=}\NormalTok{ tank.shape, name }\OperatorTok{=} \StringTok{\textquotesingle{}alpha\textquotesingle{}}\NormalTok{)}
\NormalTok{    p }\OperatorTok{=}\NormalTok{ alpha[tank]}
\NormalTok{    m.dist.binomial(total\_count }\OperatorTok{=}\NormalTok{ density, logits }\OperatorTok{=}\NormalTok{ p, obs}\OperatorTok{=}\NormalTok{surv)}
...
\CommentTok{\# Import data {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{m}\SpecialCharTok{$}\FunctionTok{data}\NormalTok{(}\FunctionTok{paste}\NormalTok{(}\FunctionTok{system.file}\NormalTok{(}\AttributeTok{package =} \StringTok{"BI"}\NormalTok{),}\StringTok{"/data/reedfrogs.csv"}\NormalTok{, }\AttributeTok{sep =} \StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{), }\AttributeTok{sep=}\StringTok{\textquotesingle{};\textquotesingle{}}\NormalTok{)}
...
\NormalTok{m}\SpecialCharTok{$}\NormalTok{data\_on\_model}\SpecialCharTok{$}\NormalTok{tank }\OtherTok{=}\NormalTok{ m}\SpecialCharTok{$}\NormalTok{data\_on\_model}\SpecialCharTok{$}\NormalTok{tank}\SpecialCharTok{$}\FunctionTok{astype}\NormalTok{(jnp}\SpecialCharTok{$}\NormalTok{int32) }\CommentTok{\# Manipulate}
\NormalTok{m}\SpecialCharTok{$}\NormalTok{data\_on\_model}\SpecialCharTok{$}\NormalTok{surv }\OtherTok{=}\NormalTok{ m}\SpecialCharTok{$}\NormalTok{data\_on\_model}\SpecialCharTok{$}\NormalTok{surv}\SpecialCharTok{$}\FunctionTok{astype}\NormalTok{(jnp}\SpecialCharTok{$}\NormalTok{int32) }\CommentTok{\# Manipulate}


\CommentTok{\# Define model {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{model }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(tank, surv, density)\{}
  \CommentTok{\# Parameter prior distributions}
\NormalTok{  sigma }\OtherTok{=} \FunctionTok{bi.dist.exponential}\NormalTok{( }\DecValTok{1}\NormalTok{,  }\AttributeTok{name =} \StringTok{\textquotesingle{}sigma\textquotesingle{}}\NormalTok{,}\AttributeTok{shape=}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{))}
\NormalTok{  a\_bar }\OtherTok{=}  \FunctionTok{bi.dist.normal}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FloatTok{1.5}\NormalTok{, }\AttributeTok{name=}\StringTok{\textquotesingle{}a\_bar\textquotesingle{}}\NormalTok{,}\AttributeTok{shape=}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{))}
\NormalTok{  alpha }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(a\_bar, sigma, }\AttributeTok{name=}\StringTok{\textquotesingle{}alpha\textquotesingle{}}\NormalTok{, }\AttributeTok{shape =}\FunctionTok{c}\NormalTok{(}\DecValTok{48}\NormalTok{))}
\NormalTok{  p }\OtherTok{=}\NormalTok{ alpha[tank]}
  \CommentTok{\# Likelihood}
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{binomial}\NormalTok{(}\AttributeTok{total\_count =}\NormalTok{ density, }\AttributeTok{logits =}\NormalTok{ p, }\AttributeTok{obs=}\NormalTok{surv)}
\NormalTok{\} }
...
We model the relationship between the independent variable \(X\) and the
outcome variable \(Y\) with varying intercepts \(\alpha\) for each group
...
We can express the Bayesian regression model accounting for
\phantomsection\label{prior}{{priors 🛈}} distributions as follows:
...
\item
  We have the same considerations as for
  \href{13.\%20Varying\%20intercepts.qmd}{12. Varying intercepts}.
\item
  ...
  we model a \phantomsection\label{MatCov}{{matrix of covariance 🛈}}.
\item
  The covariance matrix requires a correlation matrix distribution which
  is modeled using an \(LKJcorr\) distribution that holds a parameter
...
\CommentTok{\# Import}
\ImportTok{from}\NormalTok{ importlib.resources }\ImportTok{import}\NormalTok{ files}
\NormalTok{data\_path }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}Sim data multivariatenormal.csv\textquotesingle{}}
\NormalTok{m.data(data\_path, sep}\OperatorTok{=}\StringTok{\textquotesingle{},\textquotesingle{}}\NormalTok{) }

\CommentTok{\# Define model {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\KeywordTok{def}\NormalTok{ model(cafe, wait, N\_cafes, afternoon):}
\NormalTok{    a }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{5}\NormalTok{, }\DecValTok{2}\NormalTok{,  name }\OperatorTok{=} \StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{)}
\NormalTok{    b }\OperatorTok{=}\NormalTok{ m.dist.normal(}\OperatorTok{{-}}\DecValTok{1}\NormalTok{, }\FloatTok{0.5}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{)}
\NormalTok{    sigma\_cafe }\OperatorTok{=}\NormalTok{ m.dist.exponential(}\DecValTok{1}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(}\DecValTok{2}\NormalTok{,),  name }\OperatorTok{=} \StringTok{\textquotesingle{}sigma\_cafe\textquotesingle{}}\NormalTok{)}
\NormalTok{    sigma }\OperatorTok{=}\NormalTok{ m.dist.exponential( }\DecValTok{1}\NormalTok{,  name }\OperatorTok{=} \StringTok{\textquotesingle{}sigma\textquotesingle{}}\NormalTok{)}
\NormalTok{    Rho }\OperatorTok{=}\NormalTok{ m.dist.lkj(}\DecValTok{2}\NormalTok{, }\DecValTok{2}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}Rho\textquotesingle{}}\NormalTok{)}
\NormalTok{    cov }\OperatorTok{=}\NormalTok{ jnp.outer(sigma\_cafe, sigma\_cafe) }\OperatorTok{*}\NormalTok{ Rho}
\NormalTok{    a\_cafe\_b\_cafe }\OperatorTok{=}\NormalTok{ m.dist.multivariatenormal(jnp.stack([a, b]), cov, shape }\OperatorTok{=}\NormalTok{ [N\_cafes], name }\OperatorTok{=} \StringTok{\textquotesingle{}a\_b\_cafe\textquotesingle{}}\NormalTok{)    }
...
\NormalTok{    m.dist.normal(mu, sigma, obs}\OperatorTok{=}\NormalTok{wait)}
...
\CommentTok{\# Import data {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{m}\SpecialCharTok{$}\FunctionTok{data}\NormalTok{(}\FunctionTok{paste}\NormalTok{(}\FunctionTok{system.file}\NormalTok{(}\AttributeTok{package =} \StringTok{"BI"}\NormalTok{),}\StringTok{"/data/Sim data multivariatenormal.csv"}\NormalTok{, }\AttributeTok{sep =} \StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{), }\AttributeTok{sep=}\StringTok{\textquotesingle{},\textquotesingle{}}\NormalTok{)}
...
\NormalTok{  a }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(}\DecValTok{5}\NormalTok{, }\DecValTok{2}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{)}
\NormalTok{  b }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{, }\FloatTok{0.5}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{)}
\NormalTok{  sigma\_cafe }\OtherTok{=} \FunctionTok{bi.dist.exponential}\NormalTok{(}\DecValTok{1}\NormalTok{, }\AttributeTok{shape=} \FunctionTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{), }\AttributeTok{name =} \StringTok{\textquotesingle{}sigma\_cafe\textquotesingle{}}\NormalTok{)}
\NormalTok{  sigma }\OtherTok{=} \FunctionTok{bi.dist.exponential}\NormalTok{( }\DecValTok{1}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}sigma\textquotesingle{}}\NormalTok{)}
\NormalTok{  Rho }\OtherTok{=} \FunctionTok{bi.dist.lkj}\NormalTok{(}\FunctionTok{as.integer}\NormalTok{(}\DecValTok{2}\NormalTok{), }\FunctionTok{as.integer}\NormalTok{(}\DecValTok{2}\NormalTok{), }\AttributeTok{name =} \StringTok{\textquotesingle{}Rho\textquotesingle{}}\NormalTok{)}
\NormalTok{  cov }\OtherTok{=}\NormalTok{ jnp}\SpecialCharTok{$}\FunctionTok{outer}\NormalTok{(sigma\_cafe, sigma\_cafe) }\SpecialCharTok{*}\NormalTok{ Rho}
  
\NormalTok{  a\_cafe\_b\_cafe }\OtherTok{=} \FunctionTok{bi.dist.multivariatenormal}\NormalTok{(}
\NormalTok{    jnp}\SpecialCharTok{$}\FunctionTok{squeeze}\NormalTok{(jnp}\SpecialCharTok{$}\FunctionTok{stack}\NormalTok{(}\FunctionTok{list}\NormalTok{(a, b))), }
\NormalTok{    cov, }\AttributeTok{shape =} \FunctionTok{c}\NormalTok{(N\_cafes), }\AttributeTok{name =} \StringTok{\textquotesingle{}a\_cafe\textquotesingle{}}\NormalTok{)  }
...
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{normal}\NormalTok{(mu, sigma, }\AttributeTok{obs=}\NormalTok{wait)}
\NormalTok{\}}
...
\item
  \(\left(\begin{array}{cc} \sigma_\alpha^2 & \rho_{\alpha\beta} \sigma_\alpha \sigma_\beta \\ \rho_{\alpha\beta} \sigma_\alpha \sigma_\beta & \sigma_\beta^2 \end{array}\right)\)
...
\item
  ...
  \item
    \(\rho_{\alpha\beta} \sigma_\alpha \sigma_\beta\) is the covariance
    between \(\alpha_k\) and \(\beta_k\).
  \end{itemize}
\end{itemize}

For computational reasons, it is often better to implement a
\phantomsection\label{RFcentered}{{non-centered parameterization 🛈}}
that is equivalent to the \emph{Multivariate Normal distribution}
approach:
...
\item
  ...
  \item
    ...
    matrix using the \phantomsection\label{LKJcorr}{{Cholesky Factor 🛈}}
  \end{itemize}
\end{itemize}
...
\href{2.\%20Multiple\%20continuous\%20variable.qmd}{Chapter 2}. In this
case, we apply the same principle, but with a covariance matrix with a
dimension equal to the number of varying slopes we define. For example,
if we want to generate random slopes for \(i\) observations in a model
with two independent variables \(X_1\) and \(X_2\), we can define the
formula as follows:
...
a covariance matrix for each varying effect that is summed to generate
...
\item
  ...
  Binomial multivariate model with an interaction between two
  independent variables \(X_1\) and \(X_2\) and multiple varying effects
  for each actor and each group.
\end{itemize}
...
\NormalTok{    dist(}\StringTok{"L"}\NormalTok{, dist.Binomial(logits}\OperatorTok{=}\NormalTok{logit\_p), obs}\OperatorTok{=}\NormalTok{L)}
    ...
    \CommentTok{\# compute ordinary correlation matrices from Cholesky factors}
...
Through varying intercepts and slopes, we have seen how to quantify some
of the unique features that generate variation across clusters and
...
clusters, we are inherently assuming linear relationships between
...
Basically, a GP is a varying-slope model with a covariance matrix where
each element of the matrix is a \phantomsection\label{kernel}{{kernel
function 🛈}}.
...
\item
  ...
  thus, GPs require choosing an appropriate kernel function that
  captures the expected behavior of your data.
\item
...
\item
  ...
  datasets, which has led to neural networks being used instead to
  resolve large non-linear problems.
\end{itemize}
...
regression using the Bayesian Inference (BI) package. Data consist of a
continuous dependent variable (\emph{total\_tools}), representing the
number of tools invented in the islands, and a continuous independent
variable (\emph{population}), representing the population of the
islands. The goal is to estimate the effect of population on the total
tools. We use the distance matrix of the islands for the kernel function
in order to capture the spatial dependence of the relationship.
...
\CommentTok{\# Import}
\ImportTok{from}\NormalTok{ importlib.resources }\ImportTok{import}\NormalTok{ files}
\NormalTok{data\_path }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}Kline2.csv\textquotesingle{}}
\NormalTok{m.data(data\_path, sep}\OperatorTok{=}\StringTok{\textquotesingle{};\textquotesingle{}}\NormalTok{) }

\NormalTok{data\_path2 }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}Kline2.csv\textquotesingle{}}
\NormalTok{islandsDistMatrix }\OperatorTok{=}\NormalTok{ pd.read\_csv(data\_path2, index\_col}\OperatorTok{=}\DecValTok{0}\NormalTok{)}
...
\NormalTok{    a }\OperatorTok{=}\NormalTok{ m.dist.exponential(}\DecValTok{1}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{)}
\NormalTok{    b }\OperatorTok{=}\NormalTok{ m.dist.exponential(}\DecValTok{1}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{)}
\NormalTok{    g }\OperatorTok{=}\NormalTok{ m.dist.exponential(}\DecValTok{1}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}g\textquotesingle{}}\NormalTok{)}

    \CommentTok{\# non{-}centered Gaussian Process prior}
\NormalTok{    etasq }\OperatorTok{=}\NormalTok{ m.dist.exponential(}\DecValTok{2}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}etasq\textquotesingle{}}\NormalTok{)}
\NormalTok{    rhosq }\OperatorTok{=}\NormalTok{ m.dist.exponential(}\FloatTok{0.5}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}rhosq\textquotesingle{}}\NormalTok{)}
\NormalTok{    SIGMA }\OperatorTok{=}\NormalTok{ cov\_GPL2(Dmat, etasq, rhosq, }\FloatTok{0.01}\NormalTok{)}
\NormalTok{    k }\OperatorTok{=}\NormalTok{ m.dist.multivariatenormal(}\DecValTok{0}\NormalTok{, SIGMA, name }\OperatorTok{=} \StringTok{\textquotesingle{}k\textquotesingle{}}\NormalTok{)}
    \CommentTok{\#k = m.gaussian.gaussian\_process(Dmat, etasq, rhosq, 0.01, shape = (10,))}
\NormalTok{    k }\OperatorTok{=}\NormalTok{ m.gaussian.kernel\_L2(Dmat, etasq, rhosq, }\FloatTok{0.01}\NormalTok{)}
...
\NormalTok{    m.dist.poisson(lambda\_, obs}\OperatorTok{=}\NormalTok{total\_tools)}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(BI)}
\NormalTok{pd}\OtherTok{=}\FunctionTok{import}\NormalTok{(}\StringTok{\textquotesingle{}pandas\textquotesingle{}}\NormalTok{)}
\CommentTok{\# setup platform{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{m}\OtherTok{=}\FunctionTok{importbi}\NormalTok{(}\AttributeTok{platform=}\StringTok{\textquotesingle{}cpu\textquotesingle{}}\NormalTok{)}

\CommentTok{\# Import data {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{m}\SpecialCharTok{$}\FunctionTok{data}\NormalTok{(}\FunctionTok{paste}\NormalTok{(}\FunctionTok{system.file}\NormalTok{(}\AttributeTok{package =} \StringTok{"BI"}\NormalTok{),}\StringTok{"/data/Kline2.csv"}\NormalTok{, }\AttributeTok{sep =} \StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{), }\AttributeTok{sep=}\StringTok{\textquotesingle{};\textquotesingle{}}\NormalTok{)}
\NormalTok{islandsDistMatrix }\OtherTok{=}\NormalTok{ pd}\SpecialCharTok{$}\FunctionTok{read\_csv}\NormalTok{(}\FunctionTok{paste}\NormalTok{(}\FunctionTok{system.file}\NormalTok{(}\AttributeTok{package =} \StringTok{"BI"}\NormalTok{),}\StringTok{"/data/islandsDistMatrix.csv"}\NormalTok{, }\AttributeTok{sep =} \StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{), }\AttributeTok{index\_col=}\FunctionTok{as.integer}\NormalTok{(}\DecValTok{0}\NormalTok{))}
...
\NormalTok{m}\SpecialCharTok{$}\NormalTok{data\_on\_model}\SpecialCharTok{$}\NormalTok{society }\OtherTok{=}\NormalTok{ jnp}\SpecialCharTok{$}\FunctionTok{arange}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{10}\NormalTok{, }\AttributeTok{dtype=}\StringTok{\textquotesingle{}int64\textquotesingle{}}\NormalTok{)}
\NormalTok{m}\SpecialCharTok{$}\NormalTok{data\_on\_model}\SpecialCharTok{$}\NormalTok{Dmat }\OtherTok{=}\NormalTok{ jnp}\SpecialCharTok{$}\FunctionTok{array}\NormalTok{(islandsDistMatrix)}


\CommentTok{\# Define model {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{model }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(Dmat, population, society, total\_tools)\{}
\NormalTok{  a }\OtherTok{=} \FunctionTok{bi.dist.exponential}\NormalTok{(}\DecValTok{1}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{)}
\NormalTok{  b }\OtherTok{=} \FunctionTok{bi.dist.exponential}\NormalTok{(}\DecValTok{1}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{)}
\NormalTok{  g }\OtherTok{=} \FunctionTok{bi.dist.exponential}\NormalTok{(}\DecValTok{1}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}g\textquotesingle{}}\NormalTok{)}
  
  \CommentTok{\# non{-}centered Gaussian Process prior}
\NormalTok{  etasq }\OtherTok{=} \FunctionTok{bi.dist.exponential}\NormalTok{(}\DecValTok{2}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}etasq\textquotesingle{}}\NormalTok{)}
\NormalTok{  rhosq }\OtherTok{=} \FunctionTok{bi.dist.exponential}\NormalTok{(}\FloatTok{0.5}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}rhosq\textquotesingle{}}\NormalTok{)}
\NormalTok{  z }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{, }\AttributeTok{name =} \StringTok{\textquotesingle{}z\textquotesingle{}}\NormalTok{, }\AttributeTok{shape =} \FunctionTok{c}\NormalTok{(}\DecValTok{10}\NormalTok{))}
\NormalTok{  r }\OtherTok{=}\NormalTok{ m}\SpecialCharTok{$}\FunctionTok{kernel\_sq\_exp}\NormalTok{(Dmat, z, etasq, rhosq, }\FloatTok{0.01}\NormalTok{)}
...
\NormalTok{  lambda\_ }\OtherTok{=}\NormalTok{ a }\SpecialCharTok{*}\NormalTok{ population}\SpecialCharTok{**}\NormalTok{b }\SpecialCharTok{/}\NormalTok{ g }\SpecialCharTok{*}\NormalTok{ jnp}\SpecialCharTok{$}\FunctionTok{exp}\NormalTok{(k[society])}
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{poisson}\NormalTok{(lambda\_, }\AttributeTok{obs=}\NormalTok{total\_tools)}
\NormalTok{\}}
...
the dependent variable \(Y\) and the independent variable \(X\) while
...
\item
  \(\gamma_{Z_i}\) is the Gaussian process i-th value for the
...
\item
  ...
  distribution of the same size as the number of random variables and
  set to \phantomsection\label{kernelMean0}{{zero 🛈}}.
\item
  \(K\) is the covariance matrix of the random variable \(Z\). Each
  element \(K_{ij}\) of the matrix is given by the kernel function
...
\tightlist
\item
  Multiple kernel functions exist and will be discussed in the
...
\item
  \(\delta_{ij}\) is the Kronecker delta, taking a value of one when
  \(i = j\) and zero otherwise, allowing the self-covariance to be
...
\item
  ...
  noise variance only when \(i = j\), ensuring that the diagonal
...
\item
  \(\beta\) is the regression coefficient term with a prior of
  \(Normal(0,1)\).
\item
  \(X_i\) is the i-th value for the independent variable \(X\).
\item
...
\item
  ...
  points, \(K_{ij} = k(Z_i, Z_j)\), with priors of HalfCauchy(0,1) for
...
\item
  \emph{Periodic kernel} allows for modeling functions that repeat
  themselves exactly:
...
Measurement error refers to the variability in the measurement of a
variable, and measurement error can be generated by several factors,
such as sampling bias, censoring bias, and group size heterogeneity. It
is an important consideration in many fields, including statistics,
economics, and engineering, where accurate measurements are crucial for
making informed decisions. To account for measurement error, we can use
a \emph{measurement error model}. This model assumes that the
measurement of a variable is subject to an error, which can be modeled
using a probability distribution. The model can be used to estimate the
parameters of the measurement error distribution, such as the mean and
variance, and to make predictions about the measurements based on the
estimated parameters. Measurement error models are \emph{composed
models} (i.e., models with sub-models) that evaluate different
generative processes, starting with the measurement error process, which
is then used to generate the observed data.

\section{Example}\label{example-15}

Below is an example code snippet demonstrating a Bayesian measurement
error model using the Bayesian Inference (BI) package. The data consist
of three continuous variables (marriage rate, divorce rate, age), and
the goal is to estimate the effect of age and marriage rate on the
divorce rate while considering that the divorce rate has a measurement
error.
...
\CommentTok{\# Import}
\ImportTok{from}\NormalTok{ importlib.resources }\ImportTok{import}\NormalTok{ files}
\NormalTok{data\_path }\OperatorTok{=}\NormalTok{ files(}\StringTok{\textquotesingle{}BI.resources.data\textquotesingle{}}\NormalTok{) }\OperatorTok{/} \StringTok{\textquotesingle{}WaffleDivorce.csv\textquotesingle{}}
\NormalTok{m.data(data\_path, sep}\OperatorTok{=}\StringTok{\textquotesingle{};\textquotesingle{}}\NormalTok{) }
\NormalTok{m.scale([}\StringTok{\textquotesingle{}Divorce\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}Divorce SE\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}MedianAgeMarriage\textquotesingle{}}\NormalTok{]) }\CommentTok{\# Scale}
\NormalTok{dat }\OperatorTok{=} \BuiltInTok{dict}\NormalTok{(}
\NormalTok{    D\_obs }\OperatorTok{=}\NormalTok{ jnp.array(m.df[}\StringTok{\textquotesingle{}Divorce\textquotesingle{}}\NormalTok{].values),   }
\NormalTok{    D\_sd }\OperatorTok{=}\NormalTok{ jnp.array(m.df[}\StringTok{\textquotesingle{}Divorce\textquotesingle{}}\NormalTok{].values), }
\NormalTok{    A }\OperatorTok{=}\NormalTok{ jnp.array(m.df[}\StringTok{\textquotesingle{}MedianAgeMarriage\textquotesingle{}}\NormalTok{].values), }
\NormalTok{    N }\OperatorTok{=}\NormalTok{ m.df.shape[}\DecValTok{0}\NormalTok{]   }
\NormalTok{)}
\NormalTok{m.data\_on\_model }\OperatorTok{=}\NormalTok{ dat }\CommentTok{\# Send to model (convert to jax array)}


\CommentTok{\# Define model {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\KeywordTok{def}\NormalTok{ model(D\_obs, D\_sd, A, N):  }
\NormalTok{    a }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\FloatTok{0.2}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{) }
\NormalTok{    bA }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\FloatTok{0.5}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}bA\textquotesingle{}}\NormalTok{) }
\NormalTok{    s }\OperatorTok{=}\NormalTok{ m.dist.exponential(}\DecValTok{1}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}s\textquotesingle{}}\NormalTok{) }
\NormalTok{    mu }\OperatorTok{=}\NormalTok{ a }\OperatorTok{+}\NormalTok{ bA }\OperatorTok{*}\NormalTok{ A }\OperatorTok{+}\NormalTok{ bM }\OperatorTok{*}\NormalTok{ M}
\NormalTok{    D\_true }\OperatorTok{=}\NormalTok{ m.dist.normal(mu, s, name }\OperatorTok{=} \StringTok{\textquotesingle{}D\_true\textquotesingle{}}\NormalTok{) }
\NormalTok{    m.normal(D\_true , D\_sd, obs }\OperatorTok{=}\NormalTok{ D\_obs) }
...
This is an approach that can be extended to any kind of model previously
described. For example, one could generate a Bernoulli measurement error
model by generating a process for the probabilities of success and
failure. We can even go further by potentially having an error rate that
is present only in one of the two outcomes.
...
are inferred from the data---can help model this hidden structure. These
...
\NormalTok{means }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(NY,), sample}\OperatorTok{=}\VariableTok{True}\NormalTok{, seed}\OperatorTok{=}\DecValTok{10}\NormalTok{)}
\NormalTok{offsets }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(NV, }\DecValTok{1}\NormalTok{), sample}\OperatorTok{=}\VariableTok{True}\NormalTok{, seed}\OperatorTok{=}\DecValTok{20}\NormalTok{)}
...
\NormalTok{    means }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(NY,), name}\OperatorTok{=}\StringTok{\textquotesingle{}means\textquotesingle{}}\NormalTok{)}
\NormalTok{    offset }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(NV, }\DecValTok{1}\NormalTok{), name}\OperatorTok{=}\StringTok{\textquotesingle{}offset\textquotesingle{}}\NormalTok{)}
\NormalTok{    sigma }\OperatorTok{=}\NormalTok{ m.dist.exponential(}\DecValTok{1}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(NY,), name}\OperatorTok{=}\StringTok{\textquotesingle{}sigma\textquotesingle{}}\NormalTok{)}
...
\NormalTok{    m.normal(mu\_l, jnp.tile(sigma, [NV, }\DecValTok{1}\NormalTok{]), obs}\OperatorTok{=}\NormalTok{Y2)}
...
coordinate system where the greatest variance of the data is projected
onto the first coordinates (called principal components). This method
helps capture the underlying structure of high-dimensional data by
identifying patterns based on variance.
...
quantify the uncertainty around them and avoid overfitting by
...
\NormalTok{plt.title(}\StringTok{"Dataset"}\NormalTok{)}
\NormalTok{plt.show()}
...
\pandocbounded{\includegraphics[keepaspectratio]{pca1.png}}
...
\item
  ...
  (latent features with \(K \ll D\)). \(Z\) is defined by a Normal
  distribution with mean 0 and variance 1.
\item
  \(W \in \mathbb{R}^{D \times K}\) is the matrix of principal
  components (\emph{projection matrix}). \(W\) is defined by a Normal
  distribution with mean 0 and variance 1.
\item
...
\item
  ...
  ambiguity 🛈}}} in PCA, we can align the signs of the estimated
  parameters with the true parameters before comparison. To do this,
  calculate the dot product between the true and estimated parameters.
  If it is negative, multiply the estimated parameters by -1 to align
  them with the true parameters. Below, a code snippet highlights how to
  do this:
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio]{pca2.png}}

\section{Reference(s)}\label{references-17}

https://www.tensorflow.org/probability/examples/Probabilistic\_PCA
...
\chapter{Dirichlet Process Gaussian Mixture
Model}\label{dirichlet-process-gaussian-mixture-model}
...
the number of groups, we can use a \textbf{Dirichlet Process Gaussian
Mixture Model (DPMM)}. This is a {non-parametric 🛈} clustering method.
Essentially, the model assumes the data is generated from a collection
of different Gaussian distributions, and it simultaneously tries to
figure out:
...
\tightlist
\item
  \textbf{How many clusters (\texttt{K}) exist}: Unlike algorithms like
  K-Means, the DPMM infers the most probable number of clusters directly
  from the data.
\item
  \textbf{The properties of each cluster}: For each inferred cluster, it
  estimates its center (mean \(\mu\)) and its shape/spread (covariance
  \(\sigma\)).
\item
...
\begin{itemize}
\item
  A DPMM is a Bayesian model 🛈 that considers uncertainty in all its
  parameters. The core idea is to use the Dirichlet Process prior that
  allows for a potentially infinite number of clusters. In practice, we
  use a finite approximation called the {Stick-Breaking Process 🛈}.
\item
  ...
  \tightlist
  \item
    \textbf{Concentration} \(\alpha\): This single parameter controls
    the tendency to create new clusters. A low \texttt{α} favors fewer,
    larger clusters, while a high \texttt{α} allows for many smaller
    clusters. We typically place a \texttt{Gamma} prior on \(\alpha\) to
    learn its value from the data.
  \item
    \textbf{Cluster Weights \texttt{w}}: Generated via the
    Stick-Breaking process from \(\alpha\). These are the probabilities
    of drawing a data point from any given cluster.
  \item
    \textbf{Cluster Parameters (}\(\mu\), \(\sigma\)): Each potential
    cluster has a mean \(\mu\) and a covariance matrix \(\sigma\). If
    the data have multiple dimensions, we use a multivariate normal
    distribution (see chapter, \href{14.\%20Varying\%20slopes.qmd}{14}).
    Howver, if the data is one-dimensional, we use a univariate normal
    distribution.
  \end{itemize}
\item
  The model is often implemented in its {marginalized form 🛈}. Instead
  of explicitly assigning each data point to a cluster, we integrate out
  this choice. This creates a smoother probability surface for the
  inference algorithm to explore, leading to much more efficient
  computation.
\end{itemize}
...
Below is an example of a DPMM implemented in BI. The goal is to cluster
a synthetic dataset into its underlying groups. The code first generates
data with 4 distinct centers and then applies the DPMM to recover these
clusters.
...
\KeywordTok{def}\NormalTok{ dpmm\_marginal(data, T}\OperatorTok{=}\DecValTok{10}\NormalTok{):}
    \CommentTok{\# 1) stick{-}breaking weights}
\NormalTok{    alpha }\OperatorTok{=}\NormalTok{ m.dist.gamma(}\FloatTok{1.0}\NormalTok{, }\FloatTok{10.0}\NormalTok{,name}\OperatorTok{=}\StringTok{\textquotesingle{}alpha\textquotesingle{}}\NormalTok{)}
\NormalTok{    beta }\OperatorTok{=}\NormalTok{ m.dist.beta(}\DecValTok{1}\NormalTok{, alpha,name}\OperatorTok{=}\StringTok{\textquotesingle{}beta\textquotesingle{}}\NormalTok{,shape}\OperatorTok{=}\NormalTok{(T}\OperatorTok{{-}}\DecValTok{1}\NormalTok{,))}
\NormalTok{    w }\OperatorTok{=}\NormalTok{ numpyro.deterministic(}\StringTok{"w"}\NormalTok{,dist.transforms.StickBreakingTransform()(beta))}


    \CommentTok{\# 2) component parameters}
\NormalTok{    data\_mean }\OperatorTok{=}\NormalTok{ jnp.mean(data, axis}\OperatorTok{=}\DecValTok{0}\NormalTok{)}
    \ControlFlowTok{with}\NormalTok{ numpyro.plate(}\StringTok{"components"}\NormalTok{, T):}
\NormalTok{        mu }\OperatorTok{=}\NormalTok{ m.dist.multivariatenormal(loc}\OperatorTok{=}\NormalTok{data\_mean, covariance\_matrix}\OperatorTok{=}\FloatTok{100.0}\OperatorTok{*}\NormalTok{jnp.eye(D),name}\OperatorTok{=}\StringTok{\textquotesingle{}mu\textquotesingle{}}\NormalTok{)}\CommentTok{\# shape (T, D)        }
\NormalTok{        sigma }\OperatorTok{=}\NormalTok{ m.dist.halfcauchy(}\DecValTok{1}\NormalTok{,shape}\OperatorTok{=}\NormalTok{(D,),event}\OperatorTok{=}\DecValTok{1}\NormalTok{,name}\OperatorTok{=}\StringTok{\textquotesingle{}sigma\textquotesingle{}}\NormalTok{)}\CommentTok{\# shape (T, D)}
\NormalTok{        Lcorr }\OperatorTok{=}\NormalTok{ m.dist.lkjcholesky(dimension}\OperatorTok{=}\NormalTok{D, concentration}\OperatorTok{=}\FloatTok{1.0}\NormalTok{,name}\OperatorTok{=}\StringTok{\textquotesingle{}Lcorr\textquotesingle{}}\NormalTok{)}\CommentTok{\# shape (T, D, D)}

\NormalTok{        scale\_tril }\OperatorTok{=}\NormalTok{ sigma[..., }\VariableTok{None}\NormalTok{] }\OperatorTok{*}\NormalTok{ Lcorr  }\CommentTok{\# shape (T, D, D)}

    \CommentTok{\# 3) marginal mixture over obs}
\NormalTok{    m.dist.mixturesamefamily(}
\NormalTok{        mixing\_distribution}\OperatorTok{=}\NormalTok{m.dist.categoricalprobs(w,name}\OperatorTok{=}\StringTok{\textquotesingle{}cat\textquotesingle{}}\NormalTok{, create\_obj}\OperatorTok{=}\VariableTok{True}\NormalTok{),}
\NormalTok{        component\_distribution}\OperatorTok{=}\NormalTok{m.dist.multivariatenormal(loc}\OperatorTok{=}\NormalTok{mu, scale\_tril}\OperatorTok{=}\NormalTok{scale\_tril,name}\OperatorTok{=}\StringTok{\textquotesingle{}mvn\textquotesingle{}}\NormalTok{, create\_obj}\OperatorTok{=}\VariableTok{True}\NormalTok{),}
\NormalTok{        name}\OperatorTok{=}\StringTok{"obs"}\NormalTok{,  }
\NormalTok{        obs}\OperatorTok{=}\NormalTok{data   }
\NormalTok{    )}
\NormalTok{m.data\_on\_model }\OperatorTok{=} \BuiltInTok{dict}\NormalTok{(data}\OperatorTok{=}\NormalTok{data)}
\NormalTok{m.run(dpmm\_marginal)  }\CommentTok{\# Optimize model parameters through MCMC sampling}
\end{Highlighting}
\end{Shaded}
...
\$p(z) = \sum\_\{i\}\^{}\{K\}\pi\_k \mathcal{N}(z
\textbar{}\mu\_i,\Sigma\_i)\$

The model describes the generative process for a set of \(N\)
D-dimensional data points,
\(X = \{\mathbf{x}_1, \dots, \mathbf{x}_N\}\).

The gaussian mixture distribution is defined as follows:

\$\$ p(z) = \textbackslash sum\_\{i\}\^{}\{K\}\textbackslash pi\_k
\textbackslash mathcal\{N\}(z
\textbar\textbackslash mu\_i,\textbackslash Sigma\_i) \$\$

Where K is the number of clusters. We define a variable z in which
exists only one k s.t zk=1 while z¬k=0. We define \$\$ p(z\_k == 1) =
\textbackslash pi\_k

\$\$ So it must follow that: \$\$
\textbackslash sum\_\{i\}\^{}\{K\}\textbackslash pi\_k = 1 \$\$ \$\$ 0
\textbackslash leq \textbackslash pi\_k \textbackslash leq 1 \$\$

and

\$\$ p(z) = \textbackslash prod\_\{i\}\^{}\{K\}
\textbackslash pi\_\{k\}\^{}\{z\_k\}

\$\$ We can express the conditional probability of z given z as: \$\$
p(z\textbar z) =
\textbackslash prod\_\{i\}\^{}\{K\}\textbackslash mathcal\{N\}(z
\textbar\textbackslash mu\_i,\textbackslash Sigma\_i)\^{}\{z\_k\} \$\$
and so from the joint probability p(z)p(z\textbar z) we obtain by
marginalization: \$\$ p(z) = \textbackslash sum\_\{z\}p(z)p(z\textbar z)
= \textbackslash sum\_\{i\}\^{}\{K\}\textbackslash pi\_k
\textbackslash mathcal\{N\}(z
\textbar\textbackslash mu\_i,\textbackslash Sigma\_i)

\$\$

So far, we have introduced for each datapoint zi a corrisponding latent
variable zi that specify the cluster to which it belongs. We have
therefore found an equivalent formulation of the Gaussian mixture in-
volving an explicit latent variable. Another quantity we will use is the
conditional probability of z given z: \$\$ \textbackslash gamma(z\_k)=
p(z\_k
\textbar z)=\textbackslash dfrac\{p(z\_k=1)p(z\textbar z\_k\}\{\textbackslash sum\_\{i\}\^{}K
p(z\_i=1)p(z\textbar z\_i=1)\} =
\textbackslash dfrac\{\textbackslash pi\_k\textbackslash mathcal\{N\}(z\textbar\textbackslash mu\_k,\textbackslash Sigma\_k)\}\{\textbackslash sum\_\{i\}\^{}K\textbackslash pi\_i\textbackslash mathcal\{N\}(z\textbar\textbackslash mu\_i,\textbackslash Sigma\_i)\}
\$\$

We shall view πk as the probability of zk=1 and γ(zk) as the probability
of zk given z.\\
\strut \\
...
A standard, parametric approach to clustering is the \textbf{Finite
Gaussian Mixture Model (GMM)}, where the number of clusters \texttt{K}
must be specified in advance. The model is:

\[
y_i \sim \sum_{k=1}^{K} w_k \cdot \text{Normal}(\mu_k, \Sigma_k)
\]

Where: - \(y_i\) is the data for observation \emph{i}. - \texttt{K} is
the fixed number of clusters. - \(w_k\) are fixed mixing weights that
sum to 1. - \(\mu_k\) and \(\Sigma_k\) are the mean and covariance of
the \emph{k}-th cluster.

\subsection{\texorpdfstring{\emph{Non-Parametric Bayesian Formulation:
DPMM}}{Non-Parametric Bayesian Formulation: DPMM}}\label{non-parametric-bayesian-formulation-dpmm}

The DPMM replaces the fixed \texttt{K} with a flexible, data-driven
process. The model is specified hierarchically:

\textbf{1. Stick-Breaking Construction for Weights} \[
\begin{align*}
...
\textbf{3. Likelihood} Where \texttt{T} is a truncation level chosen to
be larger than the expected number of clusters.

\section{Notes}\label{notes-9}

\begin{tcolorbox}[enhanced jigsaw, titlerule=0mm, coltitle=black, toprule=.15mm, bottomtitle=1mm, leftrule=.75mm, breakable, arc=.35mm, left=2mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, rightrule=.15mm, toptitle=1mm, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, colback=white, bottomrule=.15mm, opacityback=0, opacitybacktitle=0.6]

The primary advantage of the DPMM over methods like K-Means or a finite
GMM is the \textbf{automatic inference of the number of clusters}.
Instead of running the model multiple times with different values of
\texttt{K} and comparing them, the DPMM explores different numbers of
clusters as part of its fitting process. The posterior distribution of
the weights \texttt{w} reveals which components are ``active'' (have
significant weight) and thus gives a probabilistic estimate of the
...
\href{13.\%20Varying\%20intercepts.qmd}{intercepts} and
\href{14.\%20Varying\%20slopes.qmd}{slopes}. By adding such parameters
...
\item
  ...
  \phantomsection\label{DyadicF}{{dyadic effects 🛈}}. These varying
  intercepts and slopes are identical to those described in previous
  chapters and will therefore not be detailed further. Only the
...
with a sender-receiver effect:

\section{Python}\label{python-17}
...
\NormalTok{    result\_outcomes }\OperatorTok{=}\NormalTok{ m.net.mat\_to\_edgl(data[}\StringTok{\textquotesingle{}outcomes\textquotesingle{}}\NormalTok{]), }
\NormalTok{    kinship }\OperatorTok{=}\NormalTok{ m.net.mat\_to\_edgl(kinship),}
...
\NormalTok{    exposure\_mat }\OperatorTok{=}\NormalTok{ data[}\StringTok{\textquotesingle{}exposure\textquotesingle{}}\NormalTok{]}
\NormalTok{)}


\KeywordTok{def}\NormalTok{ model(idx, result\_outcomes, }
\NormalTok{    exposure,}
\NormalTok{    kinship, }
\NormalTok{    focal\_individual\_predictors, target\_individual\_predictors, }
\NormalTok{    Any, Merica, Quantum):}
      \CommentTok{\# Block {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{      B\_any }\OperatorTok{=}\NormalTok{ m.net.block\_model(Any,}\DecValTok{1}\NormalTok{)}
\NormalTok{      B\_Merica }\OperatorTok{=}\NormalTok{ m.net.block\_model(Merica,}\DecValTok{3}\NormalTok{)}
\NormalTok{      B\_Quantum }\OperatorTok{=}\NormalTok{ m.net.block\_model(Quantum,}\DecValTok{2}\NormalTok{)}

      \CommentTok{\#\# SR shape =  N individuals{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{      sr }\OperatorTok{=}\NormalTok{  m.net.sender\_receiver(focal\_individual\_predictors,target\_individual\_predictors)}

      \CommentTok{\# Dyadic shape = N dyads{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}  }
\NormalTok{      dr }\OperatorTok{=}\NormalTok{ m.net.dyadic\_effect(dyadic\_predictors)}

\NormalTok{      m.dist.poisson(jnp.exp(B\_any }\OperatorTok{+}\NormalTok{ B\_Merica }\OperatorTok{+}\NormalTok{ B\_Quantum }\OperatorTok{+}\NormalTok{ sender\_receiver }\OperatorTok{+}\NormalTok{ dr), obs }\OperatorTok{=}\NormalTok{ result\_outcomes, name}\OperatorTok{=} \StringTok{\textquotesingle{}latent network\textquotesingle{}}\NormalTok{ )}
...
\FunctionTok{load}\NormalTok{(}\FunctionTok{paste}\NormalTok{(}\FunctionTok{system.file}\NormalTok{(}\AttributeTok{package =} \StringTok{"BI"}\NormalTok{),}\StringTok{\textquotesingle{}/data/STRAND sim sr only.Rdata\textquotesingle{}}\NormalTok{, }\AttributeTok{sep =} \StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{))}

\NormalTok{ids }\OtherTok{=} \DecValTok{0}\SpecialCharTok{:}\NormalTok{(model\_dat}\SpecialCharTok{$}\NormalTok{N\_id}\DecValTok{{-}1}\NormalTok{)}
\NormalTok{idx }\OtherTok{=}\NormalTok{ m}\SpecialCharTok{$}\NormalTok{net}\SpecialCharTok{$}\FunctionTok{vec\_node\_to\_edgle}\NormalTok{(jnp}\SpecialCharTok{$}\FunctionTok{stack}\NormalTok{(jnp}\SpecialCharTok{$}\FunctionTok{array}\NormalTok{(}\FunctionTok{list}\NormalTok{(ids, ids)), }\AttributeTok{axis =} \SpecialCharTok{{-}}\FunctionTok{as.integer}\NormalTok{(}\DecValTok{1}\NormalTok{)))}

\NormalTok{keys }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"idx"}\NormalTok{,}
          \StringTok{\textquotesingle{}idxShape\textquotesingle{}}\NormalTok{,}
          \StringTok{"result\_outcomes"}\NormalTok{,}
          \StringTok{\textquotesingle{}focal\_individual\_predictors\textquotesingle{}}\NormalTok{,}
          \StringTok{\textquotesingle{}target\_individual\_predictors\textquotesingle{}}\NormalTok{)}

\NormalTok{values }\OtherTok{\textless{}{-}} \FunctionTok{list}\NormalTok{(}
\NormalTok{  idx,}
\NormalTok{  idx}\SpecialCharTok{$}\NormalTok{shape[[}\DecValTok{1}\NormalTok{]],}
\NormalTok{  m}\SpecialCharTok{$}\NormalTok{net}\SpecialCharTok{$}\FunctionTok{mat\_to\_edgl}\NormalTok{(model\_dat}\SpecialCharTok{$}\NormalTok{outcomes[,,}\DecValTok{1}\NormalTok{]),}
\NormalTok{  jnp}\SpecialCharTok{$}\FunctionTok{array}\NormalTok{(model\_dat}\SpecialCharTok{$}\NormalTok{individual\_predictors)}\SpecialCharTok{$}\FunctionTok{reshape}\NormalTok{(}\FunctionTok{as.integer}\NormalTok{(}\DecValTok{1}\NormalTok{),}\FunctionTok{as.integer}\NormalTok{(}\DecValTok{50}\NormalTok{)),}
\NormalTok{  jnp}\SpecialCharTok{$}\FunctionTok{array}\NormalTok{(model\_dat}\SpecialCharTok{$}\NormalTok{individual\_predictors)}\SpecialCharTok{$}\FunctionTok{reshape}\NormalTok{(}\FunctionTok{as.integer}\NormalTok{(}\DecValTok{1}\NormalTok{),}\FunctionTok{as.integer}\NormalTok{(}\DecValTok{50}\NormalTok{))}
\NormalTok{)}
\NormalTok{data }\OtherTok{=} \FunctionTok{py\_dict}\NormalTok{(keys, values, }\AttributeTok{convert =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{m}\SpecialCharTok{$}\NormalTok{data\_on\_model}\OtherTok{=}\NormalTok{data}

\CommentTok{\# Define model {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{model }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(idx, idxShape, result\_outcomes,focal\_individual\_predictors, target\_individual\_predictors)\{}
\NormalTok{  N\_id }\OtherTok{=} \DecValTok{50}
...
\NormalTok{  B }\OtherTok{=} \FunctionTok{bi.dist.normal}\NormalTok{(tmp, }\FloatTok{2.5}\NormalTok{, }\AttributeTok{shape=}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{), }\AttributeTok{name =} \StringTok{\textquotesingle{}block\textquotesingle{}}\NormalTok{)}
  
  \CommentTok{\#SR {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{  sr }\OtherTok{=}\NormalTok{  m}\SpecialCharTok{$}\NormalTok{net}\SpecialCharTok{$}\FunctionTok{sender\_receiver}\NormalTok{(focal\_individual\_predictors,target\_individual\_predictors)}
  
  \DocumentationTok{\#\#\# Dyadic{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}  }
  \CommentTok{\#dr, dr\_raw, dr\_sigma, dr\_L = m.net.dyadic\_random\_effects(idx.shape[0], cholesky\_density = 2)\# shape = n dyads}
\NormalTok{  dr }\OtherTok{=}\NormalTok{ m}\SpecialCharTok{$}\NormalTok{net}\SpecialCharTok{$}\FunctionTok{dyadic\_effect}\NormalTok{(}\AttributeTok{shape =} \FunctionTok{c}\NormalTok{(idxShape))}
  ...
  \DocumentationTok{\#\# SR {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}                                                      }
\NormalTok{  m}\SpecialCharTok{$}\FunctionTok{poisson}\NormalTok{(jnp}\SpecialCharTok{$}\FunctionTok{exp}\NormalTok{(B }\SpecialCharTok{+}\NormalTok{ sr }\SpecialCharTok{+}\NormalTok{ dr), }\AttributeTok{obs=}\NormalTok{result\_outcomes)  }
\NormalTok{\}}
...
\CommentTok{\# Summary {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{summary }\OtherTok{=}\NormalTok{m}\SpecialCharTok{$}\FunctionTok{summary}\NormalTok{()}

\NormalTok{summary[}\FunctionTok{rownames}\NormalTok{(summary) }\SpecialCharTok{\%in\%} \FunctionTok{c}\NormalTok{(}\StringTok{\textquotesingle{}focal\_effects[0]\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}target\_effects[0]\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}block[0]\textquotesingle{}}\NormalTok{),]}
\end{Highlighting}
\end{Shaded}
...
\begin{itemize}
\item
  \(Y_{ij}\) is the weight of the link between \emph{i} and \emph{j}.
\item
...
\href{13.\%20Varying\%20intercepts.qmd}{intercepts} and
\href{14.\%20Varying\%20slopes.qmd}{slopes} identical to those described
in previous chapters and are defined through the following equations:
...
Similarly, for each dyad we can define varying intercepts and slopes to
account for the correlation between the propensity to emit and receive
links of a dyad:
...
\item
  ...
  coefficient to \(\lambda_i\), \(\pi_j\) and \(\delta_{ij}\). Of
  course, for \(\lambda_i\) and \(\pi_j\), as they represent nodal
  effects, these covariates need to be nodal characteristics (e.g., sex,
  age), whereas for \(\delta_{ij}\), as it represents dyadic effects,
  these covariates need to be dyadic characteristics (e.g., genetic
  distances). Considering the previous example, given a vector of nodal
  characteristics, \emph{individual\_predictors}, and a matrix of dyadic
  characteristics, \emph{kinship}, we can incorporate these covariates
  into the sender-receiver and dyadic effects, respectively, as follows:
\end{itemize}
...
\NormalTok{    sr\_terms, focal\_effects, target\_effects }\OperatorTok{=}\NormalTok{ m.net.nodes\_terms(focal\_individual\_predictors, target\_individual\_predictors) }

    \CommentTok{\#\# Varying intercept and slope for SR}
\NormalTok{    sr\_rf, sr\_raw, sr\_sigma, sr\_L }\OperatorTok{=}\NormalTok{ m.net.nodes\_random\_effects(N\_id, cholesky\_density }\OperatorTok{=} \DecValTok{2}\NormalTok{) }
...
\NormalTok{    dr\_terms, dyad\_effects }\OperatorTok{=}\NormalTok{ m.net.dyadic\_terms(dyad\_effects)}

    \CommentTok{\#\# Varying intercept and slope for D}
\NormalTok{    rf, dr\_raw, dr\_sigma, dr\_L }\OperatorTok{=}\NormalTok{ m.net.dyadic\_random\_effects(sender\_receiver.shape[}\DecValTok{0}\NormalTok{], cholesky\_density }\OperatorTok{=} \DecValTok{2}\NormalTok{)}
\NormalTok{    dr }\OperatorTok{=}\NormalTok{ dr\_terms }\OperatorTok{+}\NormalTok{ rf}

\NormalTok{    lk(}\StringTok{\textquotesingle{}Y\textquotesingle{}}\NormalTok{, Poisson(jnp.exp( sender\_receiver }\OperatorTok{+}\NormalTok{ dr ), is\_sparse }\OperatorTok{=} \VariableTok{False}\NormalTok{), obs}\OperatorTok{=}\NormalTok{result\_outcomes) }\CommentTok{\# is\_sparse = True; if the matrix has many zeros, it can help speed up computation.}

\NormalTok{m.data\_on\_model }\OperatorTok{=} \BuiltInTok{dict}\NormalTok{(}
\NormalTok{    idx }\OperatorTok{=}\NormalTok{ idx,}
\NormalTok{    result\_outcomes }\OperatorTok{=}\NormalTok{ m.net.mat\_to\_edgl(data[}\StringTok{\textquotesingle{}outcomes\textquotesingle{}}\NormalTok{]), }
\NormalTok{    dyad\_effects }\OperatorTok{=}\NormalTok{ m.net.prepare\_dyadic\_effect(kinship), }\CommentTok{\# Can be a jax array of multiple dimensions}
\NormalTok{    focal\_individual\_predictors }\OperatorTok{=}\NormalTok{ data[}\StringTok{\textquotesingle{}individual\_predictors\textquotesingle{}}\NormalTok{],}
\NormalTok{    target\_individual\_predictors }\OperatorTok{=}\NormalTok{ data[}\StringTok{\textquotesingle{}individual\_predictors\textquotesingle{}}\NormalTok{]}
\NormalTok{)}
...
\begin{itemize}
\item
  We can apply multiple variables as in
  \href{2.\%20Multiple\%20continuous\%20Variables.qmd}{chapter 2:
  Multiple Continuous Variables}.
\item
  We can apply interaction terms as in
  \href{3.\%20Interaction\%20between\%20continuous\%20variables.qmd}{chapter
  3: Interaction Between Continuous Variables}.
\item
  ...
  or absence of a link --- i.e., model binary networks.
\item
  If the network is undirected, then accounting for the correlation
  between the propensity to emit and receive links is not necessary, and
  the terms \(\lambda_i\), \(\pi_j\), and \(\delta_{ij}\) are no longer
...
\item
  In addition to the block model(s) being tested, we need to include a
...
\href{20.\%20Network\%20model.qmd}{Network model} example, with the
...
\NormalTok{    m.poisson(jnp.exp(B\_any }\OperatorTok{+}\NormalTok{ B\_Merica }\OperatorTok{+}\NormalTok{ B\_Quantum }\OperatorTok{+}\NormalTok{ sender\_receiver }\OperatorTok{+}\NormalTok{ dr), obs}\OperatorTok{=}\NormalTok{result\_outcomes)}
...
\href{20.\%20Network\%20model.qmd}{Network model} chapter:
...
diagonal elements represent the link probabilities within category
\(i\).
...
As we consider the link probability within categories to be higher than
the link probabilities between categories, we define different priors
for the diagonal and the off-diagonal. Priors should also depend on
sample size, N, so that the resultant network density approximates
empirical networks. Basic priors could be:
...
when modeling the interaction between individuals \emph{i} and \emph{j}.
...
Below is an example code snippet demonstrating a Bayesian network model
with a sender-receiver effect, a dyadic effect, and a block model effect
while accounting for exposure biases:
...
\NormalTok{      m.dist.binomial(total\_count }\OperatorTok{=}\NormalTok{ m.net.mat\_to\_edgl(exposure\_mat), logits }\OperatorTok{=}\NormalTok{ jnp.exp(B\_any }\OperatorTok{+}\NormalTok{ B\_Merica }\OperatorTok{+}\NormalTok{ B\_Quantum }\OperatorTok{+}\NormalTok{ sender\_receiver }\OperatorTok{+}\NormalTok{ dr), obs }\OperatorTok{=}\NormalTok{ result\_outcomes, name}\OperatorTok{=} \StringTok{\textquotesingle{}latent network\textquotesingle{}}\NormalTok{ )}
...
Below is an example code snippet demonstrating a Bayesian network model
with a sender-receiver effect, a dyadic effect, and a block model effect
while accounting for exposure biases and censoring biases:
...
\item
  ...
  \]
\item
  \(\phi_{[i,j]}\) is the probability of a true tie between \(i\) and
  \(j\).
\item
  \(\eta_{[i]}\) is the probability of individual \(i\) being
  detectable.
\item
  \(\eta_{[j]}\) is the probability of individual \(j\) being
  detectable.
\end{itemize}
...
\item
  ...
  example, if \(C\) is an animal-specific measure, like a binary
  variable for cryptic coloration, then the ellipsis may be replaced
  with \(\kappa_{[5]}C_{[i]}\) to give the effects of coloration on
  censoring probability.
\end{itemize}

\section{Note(s)}\label{notes-12}
...
Unlike other chapters, here we will present a suite of the most commonly
used network metrics and the corresponding BI functions under the class
\texttt{m.net.}, implemented with JAX and usable within any bi model.
This section is inspired by XXX, and users willing to dig further are
invited to read and cite this paper.

\section{Nodal metrics}\label{nodal-metrics}

Nodal metrics* enable the assessment of nodes' social heterogeneity and
the understanding of underlying mechanisms such as individual
characteristics (e.g., the ageing process), ecological factors (e.g.,
demographic variation), and evolutionary processes (e.g., differences in
social styles). Node measures are calculated at a nodal level and
assess, in different ways and with different meanings, how an individual
is connected. Connections can be ego's* direct links only (e.g., degree,
strength), its alters'* links as well (e.g., eigenvector, clustering
coefficient), or even all the links in the network (e.g., betweenness).
Node measures can also be used to describe the overall network structure
through distributions, means, and coefficients of variation.

\subsection{Degree and strength}\label{degree-and-strength}

The degree \texttt{m.net.degree} measures the number of links of a node.
When computed on an undirected network, the degree represents the number
of alters of an ego. When the network is directed\emph{, it represents
the number of either incoming} or outgoing* links of an ego, and it is
then called in-degree \texttt{m.net.indegree} or out-degree
\texttt{m.net.outdegree}, respectively. Note that degree can also be
...
Strength (or weighted degree) \texttt{m.net.strength} is the sum of the
...
in-strength \texttt{m.net.instrength} (the sum of weights of incoming
links) and out-strength \texttt{m.net.outstrength} (the sum of weights
...
frequently with a few social partners or vice versa (Liao, Sosa, Wu, \&
...
Eigenvector centrality \texttt{m.net.eigenvector} is the first
...
or undirected networks. It measures centrality} by examining the
connectedness of an ego as well as that of its alters. Thus, a node's
...
The local clustering coefficient \texttt{m.net.cc} measures the number
of closed triplets* over the total theoretical number of triplets (i.e.,
open and closed), where a triplet is a set of three nodes that are
connected by either two (open triplet) or three (closed triplet) edges.
This measure aims to examine the links that may exist between the alters
of an ego and measures the cohesion of the network. The main topological
effect of closed triplets is the clustering of the network, generating
cohesive clusters, and is thus strongly related to modularity (see
corresponding section). The local clustering coefficient can be computed
in a binary network by measuring the proportion of links between the
nodes of an ego-network* divided by the number of potential links
between them. In weighted networks, several versions exist, such as
...
\] Where \(\tau_\Delta\) represents closed triplets, and \(w\) is the
...
two nodes. The value of the betweenness indicates the theoretical role
of a node in social transmission (information, disease, etc., see Figure
1), as it indicates to what extent a node connects subgroups, as a
bridge, and thus is likely to spread an entity across the whole network
(Newman, 2005).
...
betweenness value can be considered zero.
...
interactions within a network and make it possible to test hypotheses
about the mechanisms underlying network connectivity. These types of
measures are generally used to test mechanistic biological questions,
such as what factors (e.g., ecological as well as sociodemographic)
affect individuals' interactions/associations.
...
1954). Assortativity values range from −1 (total disassortativity, i.e.,
all the nodes associate or interact with those with the opposite
characteristic, such as males interacting exclusively with females) to 1
(total assortativity, i.e., all the nodes associate or interact with
those with the same characteristic, such as males interacting only with
males). The assortativity coefficient measures the proportion of links
between and within clusters of nodes with the same characteristics.
Individuals' characteristics can be continuous (e.g., age, individual
network measure, personality) or categorical features (e.g., sex,
matriline belonging; Figure 2). Assortativity does not consider
directionality* and can be measured in weighted (Leung \& Chau, 2007) or
binary (Newman, 2003) networks using categorical or continuous
characteristics (Figure 2). The use of one or the other assortativity
variant depends on the type of characteristics being examined and,
whenever possible, the weighted version should be preferred since it is
more reliable than the binary version (Farine, 2014).
...
networks. These types of connections can be studied over time based on
the creation of links. From a static perspective, directionality can be
...
considered by using Opsahl's variants, which are discussed in the
...
(the clustering coefficient includes transitive triplets), not all
closed triplets are transitive. Transitive triplets are one of the 16
possible configurations of a triplet considering open and closed
triplets as well as link directionality (i.e., triad census).
...
diffusion features based on geodesic distances (see corresponding
section). Aspects of the structure and properties of a group (e.g.,
cohesion, sub-grouping) can be quantified using global network
measures\emph{. For instance, one may quantify properties such as
network resilience} (see Diameter), network clustering* (see Modularity)
through network connectivity analysis, or network transmission
efficiency* (see Global efficiency) through network theoretical social
diffusion analysis.

\subsection{Density}\label{density}

The density \texttt{m.net.density} is the ratio of existing links to all
potential links in a network. This measure is easy to interpret; it
assesses how fully connected a network is. Density considers neither
directionality nor link weights.
...
Geodesic distance \texttt{m.net.geodesic\_distance} is the shortest path
considering all potential dyads in a network. This measure thereby
indicates the fastest path of diffusion. Geodesic distance can be
...
network's mean weight), and the strongest or the weakest links can be
...
and know which one is the most appropriate according to their research
...
The diameter \texttt{m.net.diameter} of a network represents the longest
of the shortest paths in the network. The diameter is used in ASNA to
examine aspects such as network cohesion and the rapidness of
...
path length of diffusion required to reach all nodes.
...
exchanged among the nodes of the network. As global efficiency gives a
probability of social diffusion, it may help to better understand social
transmission phenomena in the short and long term (Migliano et al.,
2017). Pasquaretta et al.~(2014) found a positive correlation between
the neocortex ratio and global efficiency in primate species with a
higher neocortex ratio. By drawing a parallel between cognitive
capacities and social network efficiency, this study showed that in
species with a higher neocortex ratio, individuals may adjust their
social relationships to gain better access to social information and
...
decrease network efficiency when infected by a pathogen (Stroeymeyt et
al., 2018).

\subsection{Modularity}\label{modularity}

Modularity (WIP) is a measure designed to quantify the degree to which a
network can be divided into different groups or clusters, and its value
ranges from 0 to 1. Networks with high modularity have dense connections
within the modules but sparse connections between them. Modularity can
be computed in weighted, binary, directed, or undirected networks.
...
coefficient, evaluates how well the alters of an ego are interconnected
and measures the cohesion of the network. Its main topological effect is
the clustering of the network, generating cohesive clusters, and is thus
strongly related to modularity. However, it becomes highly correlated
with density and less so with modularity as density grows. Several
variants of the global clustering coefficient can be found: (a) the
ratio of closed triplets to all triplets (open and closed), and (b) the
binary local mean clustering coefficient derived from the node level
(see Local clustering coefficient). The binary local mean clustering
coefficient allows us to consider node heterogeneity and thus should be
preferred over the first variant. Weighted versions also exist and are
based on the same variants described in the section on the local
...
Where \(\tau\) is the total number of triplets and \(\tau_\Delta\)
...
\bibitem[\citeproctext]{ref-NM}
Sosa, Sebastian, Cédric Sueur, and Ivan Puga-Gonzalez. 2021. {``Network
Measures in Animal Social Network Analysis: Their Strengths, Limits,
Interpretations and Uses.''} \emph{Methods in Ecology and Evolution} 12
(1): 10--21.
https://doi.org/\url{https://doi.org/10.1111/2041-210X.13366}.
