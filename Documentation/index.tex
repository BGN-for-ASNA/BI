...
chapters present increasingly complex models. Each following chapters
will all have the same structure in order to allow users to easily find
...
🛈}} \(x\). Where we will not focus on the equations and their details
but rather used them to describe the different components of a Bayesian
model, how parameters optimization algorithm works in Bayesian analysis,
...
The \phantomsection\label{lk}{{\emph{likelihood} 🛈}} can be consider as
your main equation that combines the different terms of the model.
...
Where as both in the \emph{dependent variable} \(y\) and the
\emph{independent variable} \(x\) are continuous variables, we can use a
\emph{Gaussian model} to describe the relationship between these two
variables. As we are using Bayesian methods, we are describing this
relationship using a \phantomsection\label{pdist}{{\emph{probability
distribution} 🛈}} :
...
Basically, where our likelihood is saying that the \emph{dependent
variable} \(y\) is normally distributed with a mean \(\mu\) and a
standard deviation \(\sigma\). \emph{Probability distribution} are
available in BI through the class \texttt{bi.dist.XXX} where
\texttt{XXX} is the name of the distribution you want to use. Within any
\texttt{bi.dist.XXX} class, you can define which of the distribution
need to be used as a likelihood by adding an argument \texttt{obs},
e.g., \texttt{m.normal(alpha\ +\ beta\ *\ x,\ s,\ obs=y)}. You can see
...
Once the \emph{likelihood} is defined, we can know define the
...
\section{Link functions Link}\label{link-functions-link}
...
For the moment, we just need to know that those different distributions
required \emph{link function} (for each specific family we will discuss
...
functions} in BI can be aces through the class \texttt{bi.link.XXX}
...
For each parameters of our equation that describe \(\mu\) , we need to
...
will be able to use any probability distributions, ink functions and
declare any mathematical operations required for your model. BI have
been designed to allow you to declare your model as close as possible to
the mathematical notation. For example, the model above can be written
in BI as:
...
\NormalTok{bi }\OperatorTok{=}\NormalTok{ bi(platform}\OperatorTok{=}\StringTok{\textquotesingle{}cpu\textquotesingle{}}\NormalTok{)}
...
\NormalTok{    alpha }\OperatorTok{=}\NormalTok{ bi.dist.normal( }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}alpha\textquotesingle{}}\NormalTok{, shape}\OperatorTok{=}\NormalTok{ (}\DecValTok{1}\NormalTok{,))}
\NormalTok{    beta }\OperatorTok{=}\NormalTok{ bi.dist.normal( }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}beta\textquotesingle{}}\NormalTok{, shape}\OperatorTok{=}\NormalTok{ (}\DecValTok{1}\NormalTok{,))   }
\NormalTok{    sigma }\OperatorTok{=}\NormalTok{ bi.dist.uniform( }\DecValTok{0}\NormalTok{, }\DecValTok{50}\NormalTok{, name }\OperatorTok{=} \StringTok{\textquotesingle{}sigma\textquotesingle{}}\NormalTok{, shape }\OperatorTok{=}\NormalTok{ (}\DecValTok{1}\NormalTok{,))}
\NormalTok{    m.normal(alpha }\OperatorTok{+}\NormalTok{ beta }\OperatorTok{*}\NormalTok{ x , , obs}\OperatorTok{=}\NormalTok{y)}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  different data structures such as vector(s) and 2D array(s) (e.g.,
...
\item
  ...
  independent variable. By default, the shape is one, so technically you
  don't need to specify it in this. In this example, we highlight this
  feature explicitly.
\end{enumerate}
...
🛈}} is used to evaluate how well a given set of parameters value fits
the data:
...
Carlo (\emph{MCMC}). Description of this algorithms are out of the scope
of this document. For more information, please refer to the
\href{https://en.wikipedia.org/wiki/Markov_chain_Monte_Carlo}{Bayesian
Inference}. In BI, we use MCMC and it can be call as
\texttt{m.run(model)} where \texttt{model} is the function that describe
the model.
...
observed?'', and ``Have the chains mixed well?'' Mulitple diagnostics
...
🛈}} & Assess if model can reproduce observed data & Graphs, p-values,
summary stats & Good fit if simulated data resemble observed data \\
...
\begin{tcolorbox}[enhanced jigsaw, toprule=.15mm, leftrule=.75mm, left=2mm, colframe=quarto-callout-caution-color-frame, opacitybacktitle=0.6, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Caution}, bottomtitle=1mm, rightrule=.15mm, toptitle=1mm, colback=white, breakable, titlerule=0mm, arc=.35mm, bottomrule=.15mm, colbacktitle=quarto-callout-caution-color!10!white, opacityback=0, coltitle=black]
...
\begin{tcolorbox}[enhanced jigsaw, toprule=.15mm, leftrule=.75mm, left=2mm, colframe=quarto-callout-note-color-frame, opacitybacktitle=0.6, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, bottomtitle=1mm, rightrule=.15mm, toptitle=1mm, colback=white, breakable, titlerule=0mm, arc=.35mm, bottomrule=.15mm, colbacktitle=quarto-callout-note-color!10!white, opacityback=0, coltitle=black]
...
\CommentTok{\# Run mcmc {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{m.run(model) }\CommentTok{\# Optimize model m.summary()parameters through MCMC sampling}
...
\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{from}\NormalTok{ main }\ImportTok{import}\NormalTok{ bi}
...
\NormalTok{    a }\OperatorTok{=}\NormalTok{ dist.normal(}\DecValTok{0}\NormalTok{, }\FloatTok{1.5}\NormalTok{, shape }\OperatorTok{=}\NormalTok{ (}\DecValTok{7}\NormalTok{,), name}\OperatorTok{=}\StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{)}
\NormalTok{    b }\OperatorTok{=}\NormalTok{ dist.normal(}\DecValTok{0}\NormalTok{, }\FloatTok{0.5}\NormalTok{, shape }\OperatorTok{=}\NormalTok{ (}\DecValTok{4}\NormalTok{,), name}\OperatorTok{=}\StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{)}
\NormalTok{    p }\OperatorTok{=}\NormalTok{ a[actor] }\OperatorTok{+}\NormalTok{ b[treatment]}
\NormalTok{    m.lk(}\StringTok{"y"}\NormalTok{, dist.binomial(}\DecValTok{1}\NormalTok{, logits}\OperatorTok{=}\NormalTok{p), obs}\OperatorTok{=}\NormalTok{pulled\_left)}
...
\item
  ...
  \tightlist
  \item
    If both are equal or greater than 1, the distribution is bell-shaped
    and centered around 0.5.
  \item
    If 𝛼 \textgreater{} 𝛽 , the distribution is skewed toward 1 and if
...
\item
  ...
  individuals' gender, which will be used to account for independent
  rates.
\end{enumerate}
...
\item
  ...
  \emph{log}. The \emph{log} link (it ensures that \emph{λ} is always
  positive).
\item
...
\NormalTok{  p }\OtherTok{=}\NormalTok{ jax}\SpecialCharTok{$}\NormalTok{scipy}\SpecialCharTok{$}\NormalTok{special}\SpecialCharTok{$}\FunctionTok{expit}\NormalTok{(al)}
...
\item
  ...
  \[\sigma^2_\beta \sim Unifrom(0, 10)\]
...
\item
  ...
  \item
    ...
    \(\alpha_k\) and \(\beta_k\). For computational reasons, it is often
    better to implement a \phantomsection\label{RFcentered}{{centered
...
case, we apply the same principle, but with a covariance matrix of a
dimension equal to the number of varying slopes we define. For example,
if we want to generate random slopes for \(i\) actors in a model with
two independent variables \(X_1\) and \(X_2\), we can define the formula
as follows:
...
variable, measurement error can be generated by several factors, such as
sampling bias, censoring bias, and group size heterogeneity. It is an
important consideration in many fields, including statistics, economics,
and engineering, where accurate measurements are crucial for making
informed decisions. In order to account for measurement error, we can
use a \emph{measurement error model}. This model assumes that the
...
\NormalTok{    offset }\OperatorTok{=}\NormalTok{ m.dist.normal(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, shape}\OperatorTok{=}\NormalTok{(NY,}\DecValTok{1}\NormalTok{), name }\OperatorTok{=} \StringTok{\textquotesingle{}offset\textquotesingle{}}\NormalTok{)}
...
\href{14.\%20Varying\%20slopes.qmd}{effects}. By adding such parameters
...
\href{20\%20.\%20Network\%20model.qmd}{Network model} chapter:
