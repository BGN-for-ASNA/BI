...
chapters present increasingly complex models in a step‐by‐step manner.
Each following chapters will all have the same structure in order to
allow users to easily find the information they are looking for. The
...
\item
  \emph{Specific conciderations for the model.}
\item
...
\section{Model set-up, Bayesian Linear Regression
...
🛈}} (\(y\)) from a single \phantomsection\label{IP}{{independent
variable 🛈}} (\(x\)). We will present some equations and basic modeling
will be discuss in further details in
\href{1.\%20Linear\%20Regression\%20for\%20continuous\%20variable.qmd}{Chapter
1: Multiple Continuous Variables}. Where we will not focus on the
equations and their details but rather used them to describe the
different components of a Bayesian model, how parameters optimization
algorithm works in Bayesian analysis, and provide basic technical
vocabulary related to Bayesian methods.
...
\emph{independent variable} (\(x\)) are continuous variables, we can use
a linear model to describe the relationship between these two variables.
As we are using Bayesian methods, we are describing this relationship
using a \phantomsection\label{pdist}{{\emph{probability distribution}
🛈}} .

\subsection{Modeling Likelihood}\label{modeling-likelihood}

Once the \emph{likelihood} is defined, we can know define the
mathematical equation that describe our parameters and their
relationship with the \emph{dependent variable} (\(y\)). We can express
this relationship in the form of a linear function: \[
\mu = \alpha + \beta x
\]

Where \(\alpha\) is the \phantomsection\label{pdist}{{\emph{intercept}
🛈}} and \(\beta\) is the \phantomsection\label{slope}{{\emph{slope} 🛈}}
of the line. These parameters are the \emph{unknowns} that we want to
estimate to evaluate the strength and direction of the relationship
between the \emph{independent variable} (\(x\)) and the \emph{dependent
variable} (\(y\)).

\subsection{Link functions Link}\label{link-functions-link}

Depending on the type of problem you are trying to solve, the likelihood
will be different (continuous, discrete, binomial, etc.) and you will
need to choose the appropriate distribution to describe the relationship
between the parameters and the data. By using a different distribution,
you will need to use a different
\phantomsection\label{slope}{{\emph{link function} 🛈}}.
...
their typical applications, and how to interpret them.
...
\subsection{The Prior Distributions}\label{the-prior-distributions}

For each parameters of our equation, we need to define a
\phantomsection\label{prior}{{\emph{prior distribution} 🛈}} that encodes
our initial beliefs about the parameter. In the case of the linear
regression model, we need to specify \emph{prior distributions} for
(\(\alpha\)), slope (\(\beta\)), and standard deviation (\(\sigma\)):
...
\subsubsection{Which prior distribution range to
...
Once data are observed, Bayes' Theorem is used to update our beliefs
about the parameters. Bayes' theorem, which states that the posterior
probability of a parameter is proportional to the product of the prior
probability and the likelihood of the data given the parameter. This
approach allows for the incorporation of prior knowledge and the use of
Bayesian updating to improve the accuracy of the model.We express what
we know (or don't know) using probability distributions, and we update
these beliefs as evidence accumulates.
...
after observing the data. It tells us not only the most likely values of
\(\alpha\), \(\beta\), and \(\sigma\) but also quantifies the
uncertainty in these estimates.

We can use \emph{Bayesian updating} using the \emph{Bayesian theorem} to
`reshape' the prior distributions by considering every possible
combination of values for µ and σ and scoring each combination by its
relative plausibility in light of the data. These relative
plausibilities are the posterior probabilities of each combination of
values µ and σ: the \emph{posterior distributions}. Various techniques
can be used to approximate the mathematical definition of Bayes'
theorem: \emph{grid approximation}, \emph{quadratic approximation}, and
Markov chain Monte Carlo (\emph{MCMC}). In BI, we use MCMC.Description
of this algorithms are out of the scope of this document. For more
information, please refer to the \href{}{Bayesian Inference}.
...
key diagnostic tools and techniques available in BI.
...
\begin{tcolorbox}[enhanced jigsaw, opacitybacktitle=0.6, left=2mm, colframe=quarto-callout-caution-color-frame, breakable, coltitle=black, arc=.35mm, opacityback=0, bottomrule=.15mm, bottomtitle=1mm, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Caution}, colback=white, rightrule=.15mm, colbacktitle=quarto-callout-caution-color!10!white, toptitle=1mm, titlerule=0mm, toprule=.15mm, leftrule=.75mm]
...
\begin{tcolorbox}[enhanced jigsaw, opacitybacktitle=0.6, left=2mm, colframe=quarto-callout-note-color-frame, breakable, coltitle=black, arc=.35mm, opacityback=0, bottomrule=.15mm, bottomtitle=1mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, colback=white, rightrule=.15mm, colbacktitle=quarto-callout-note-color!10!white, toptitle=1mm, titlerule=0mm, toprule=.15mm, leftrule=.75mm]
