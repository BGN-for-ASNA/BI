...
\begin{tcolorbox}[enhanced jigsaw, colframe=quarto-callout-caution-color-frame, rightrule=.15mm, colbacktitle=quarto-callout-caution-color!10!white, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Caution}, bottomtitle=1mm, opacityback=0, leftrule=.75mm, arc=.35mm, breakable, opacitybacktitle=0.6, toprule=.15mm, colback=white, bottomrule=.15mm, toptitle=1mm, titlerule=0mm, left=2mm, coltitle=black]
...
In the Bayesian formulation, we define each parameter with
\hyperref[]{priors}. We can express the Bayesian model as follows:
...
\item
  \hyperref[]{Triptych 🛈} plots are very handy for understanding the
...
\begin{tcolorbox}[enhanced jigsaw, colframe=quarto-callout-note-color-frame, rightrule=.15mm, colbacktitle=quarto-callout-note-color!10!white, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, bottomtitle=1mm, opacityback=0, leftrule=.75mm, arc=.35mm, breakable, opacitybacktitle=0.6, toprule=.15mm, colback=white, bottomrule=.15mm, toptitle=1mm, titlerule=0mm, left=2mm, coltitle=black]
...
\item
  ...
  for continuous variable}.
\item
  We have the first \hyperref[]{link function} \emph{logit}. The
  \emph{logit} link function in the Bayesian binomial model converts the
  linear combination of predictor variables into probabilities, making
  it suitable for modeling binary outcomes. It helps estimate the
...
\hyperref[]{overdispersion 🛈}, we can use the Beta-Binomial model.
...
\item
  ...
  for continuous variable}.
\item
  We have the second \hyperref[]{link function}. The conventional link
  function for a Poisson model is the \emph{log} link (it ensures that
  \emph{λ} is always positive).
\item
...
more independent variables with \hyperref[]{overdispersion 🛈}, we can
use the \emph{Negative Binomial model}.
...
\[
\log(\lambda_i) = \exp(\text{rates_i} + \alpha + \beta X_i)
\]
...
\[
\log(\lambda_i) \sim \text{rates_i} + \alpha + \beta X
\]
...
\item
  ...
  \hyperref[]{simplex 🛈}. To do this, we convert the regression outputs
  using the softmax function (see the ``nn.softmax'' line in the code).
\item
...
\hyperref[]{overdispersion 🛈}, we can use a \emph{Dirichlet}
distribution.
...
\item
  ...
  \hyperref[]{hyper-prior 🛈}-. In the code below, the \emph{hyper-prior}
  is \texttt{\_a\_bar\_}.
\end{itemize}
...
\begin{tcolorbox}[enhanced jigsaw, colframe=quarto-callout-note-color-frame, rightrule=.15mm, colbacktitle=quarto-callout-note-color!10!white, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Notes}, bottomtitle=1mm, opacityback=0, leftrule=.75mm, arc=.35mm, breakable, opacitybacktitle=0.6, toprule=.15mm, colback=white, bottomrule=.15mm, toptitle=1mm, titlerule=0mm, left=2mm, coltitle=black]
...
\item
  ...
  we model a \hyperref[]{matrix of covariance 🛈}.
\item
...
\chapter{Bayesian Regression with Latent
Variables}\label{bayesian-regression-with-latent-variables}
