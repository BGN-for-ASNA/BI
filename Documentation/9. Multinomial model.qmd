...
- The reference category 𝐾 is often chosen to simplify the model (Note that we didi not did it in the code).
...
- $θ=(θ_1,θ_2,…,θ_K)$  is a simplex of category probabilities, with $𝜃_𝑘$ representing the probability of category 𝑘.
- $\frac{N!}{∏^K_{k=1}yk!}$ is the multinomial coefficient accounts for the number of ways to arrange the observations into the categories. This coefficient ensures that the likelihood function properly accounts for the permutations of the counts across different categories.
## Reference(s)
