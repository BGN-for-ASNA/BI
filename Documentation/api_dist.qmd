...
* https://stats.libretexts.org/Courses/Saint_Mary%27s_College_Notre_Dame/MATH_345__-_Probability_%28Kuter%29/3%3A_Discrete_Random_Variables/3.3%3A_Bernoulli_and_Binomial_Distributions "3.3: Bernoulli and Binomial Distributions - Statistics LibreTexts"
...
#### References
* https://en.wikipedia.org/wiki/Bernoulli_distribution "Bernoulli distribution"
* https://www.tensorflow.org/probability/api_docs/python/tfp/distributions/Bernoulli "tfp.        distributions.Bernoulli - Probability"
* https://mc-stan.org/docs/2_19/functions-reference/bernoulli-logit-distribution.html "11.2 Bernoulli Distribution, Logit Parameterization - Stan"
...
#### References:
* https://en.wikipedia.org/wiki/Beta_distribution "Beta distribution -        Wikipedia"
...
#### References:
* https://en.wikipedia.org/wiki/Beta-binomial_distribution "Beta-binomial distribution"
* https://www.statisticshowto.com/beta-binomial-distribution/ "Beta-Binomial Distribution: Definition - Statistics How To"
* https://mc-stan.org/docs/2_19/functions-reference/beta-binomial-distribution.html "12.3 Beta-Binomial Distribution | Stan Functions Reference"
...
#### References:
* https://www.pymc.io/projects/examples/en/latest/spatial/conditional_autoregressive_priors.html "Conditional Autoregressive (CAR) Models for Spatial Data"
* https://mc-stan.org/learn-stan/case-studies/icar_stan.html "Intrinsic Auto-Regressive Models for Areal Data - Stan"
...
Circulant Normal Multivariate normal distribution with covariance matrix ${C}` that is
positive-definite and circulant [1], i.e., has periodic boundary conditions. The
density of a sample ${x}\in{R}^n` is the standard multivariate
normal density
...
where ${det}$ denotes the determinant and $^\intercal$ the
transpose. Circulant matrices can be diagnolized efficiently using the discrete
Fourier transform, allowing the log likelihood to be evaluated in
$n \log n$ time for $n$ observations.
...
1. Wood, A. T. A., & Chan, G. (1994). Simulation of Stationary Gaussian Processes in
...
Integrating out ${p}$ gives you the marginal distribution of ${x}). 
...
```python
bf.dist.doubly_truncated_power_law(
alpha,
low,
high,
validate_args=None,
...
Let (X) be (n \times p), with mean matrix $$M\in R^{n\times p}$$, row-covariance matrix $U\in R^{n\times n}$ (positive-definite), and column-covariance matrix $V\in R^{p\times p}$(positive-definite). Then the PDF is:
...
#### Reference: 
* * https://en.wikipedia.org/wiki/Matrix_normal_distribution "Matrix normal distribution"
...
* Let $X_i$ be the number of trials (out of $n$) that produced outcome $i$. Then the vector $ X = (X_1, \dots, X_k)$ follows a multinomial distribution. 

It reduces to the binomial when (k=2). 

PMF (Probability Mass Function): 

For $ x = (x_1,\dots,x_k)$ with each $x_i \ge 0$ integer and $\sum_{i=1}^k x_i = n$,
$$
...
The PDF of the multivariate Student's t-distribution for a random vector ( {x} \in {R}^d ) is given by:
...
$$
where (\mu = {E}[Y]) and (\alpha>0) is the dispersion (heterogeneity) parameter.
...
```python
bf.dist.negative_binomial_logits(
total_count,
logits,
validate_args=None,
name='x',
...
```python
bf.dist.negative_binomial_probs(
total_count,
probs,
validate_args=None,
name='x',
...
* https://labdisia.disia.unifi.it/grilli/files/Papers/Ordered_Logit.pdf"
...
The projected normal distribution arises by taking a multivariate normal vector $  X \sim N (\mu, \Sigma) $ in $ R^n$ and projecting it to the unit sphere. This distribution is commonly used in directional statistics (data on circles or spheres) and supports asymmetric and even multimodal behaviours depending on parameters.
...
#### References
* https://www.tensorflow.org/probability/api_docs/python/tfp/distributions/RelaxedBernoulli "tfp.distributions.RelaxedBernoulli | TensorFlow Probability"
* https://rstudio.github.io/tfprobability/reference/tfd_relaxed_bernoulli.html "RelaxedBernoulli distribution with temperature and logits ..."
...
```python
bf.dist.relaxed_bernoulli_logits(
temperature,
logits,
validate_args=None,
name='x',
...
```python
bf.dist.sine_bivariate_vonmises(
phi_loc,
psi_loc,
phi_concentration,
psi_concentration,
...
* For $ \nu \to \infty $, this approaches $ {N}(\mu, \sigma^2) $.
...
* https://rstudio.github.io/tfprobability/reference/tfd_truncated_cauchy.html "The Truncated Cauchy distribution. — tfd_truncated_cauchy"
...
* The lower bound (a) can be $-\infty$ (so only an upper truncation), and the upper bound ($b$) can be $+\infty$ (so only a lower truncation). 
...
#### References: 
* https://en.wikipedia.org/wiki/Truncated_distribution "Truncated distribution"
* https://pages.stern.nyu.edu/~wgreene/Text/Edition8/PDF/M19_GREE1366_08_SE_C19.pdf "19"
* https://search.r-project.org/CRAN/refmans/LaplacesDemon/html/dist.Truncated.html "R: Truncated Distributions"
* https://dspace.cuni.cz/bitstream/handle/20.500.11956/127921/130308341.pdf?isAllowed=y&sequence=1&utm_source.com "Truncated data thesis"
...
(`scale`), lower bound  $a$ (`low`), and upper bound $b$ (`high`). In effect: if $X \sim  N(\mu, \sigma^2)$, then the truncated version $Y = X | (a \le X \le b)$ has the same “shape” but only supports values in $[a,b]$. This is used when you know that values outside a range are impossible or not observed (e.g., measurement limits, natural bounds). 
...
#### References:
* https://en.wikipedia.org/wiki/Truncated_normal_distribution "Truncated normal distribution"
* https://people.math.sc.edu/burkardt/m_src/truncated_normal/truncated_normal.html "TRUNCATED_NORMAL - The Truncated Normal Distribution"
...
#### References:
* https://en.wikipedia.org/wiki/Wishart_distribution "Wishart distribution"
* https://mc-stan.org/docs/2_32/functions-reference/wishart-cholesky-distribution.html"28.2 Wishart distribution, Cholesky Parameterization - Stan"
...
* https://larmarange.github.io/guide-R/analyses_avancees/modeles-zero-inflated.html "48 Modèles de comptage zero-inflated et hurdle – guide-R"
...
```python
bf.dist.zero_inflated_negative_binomial2(
mean,
concentration,
gate=None,
gate_logits=None,
...
where $J$ is the all-ones matrix and $n$ is the number of constrained elements. So the covariance is $\sigma^2 (I - \tfrac{1}{n} J)$, which ensures that the sum of the components is 0 (since $ J =  \sum_i x_i $).
