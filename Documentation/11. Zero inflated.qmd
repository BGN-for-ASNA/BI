...
N = 365
drink = m.BEE.binomial(1, prob_drink, shape = (N,), sample = True)
y = (1 - drink) * m.BEE.poisson(rate_work, shape = (N,), sample = True)
...
# sample one year of production
N = as.integer(365)
drink = bee.dist.binomial(total_count = as.integer(1), probs = prob_drink, shape = c(N), sample = T ) # An example of sampling a distribution with BI
y = (1 - drink) *  bee.dist.poisson(rate_work, shape = c(N), sample = T)
data = list()
data$y = y
...
# Define model ------------------------------------------------
model <- function(y){
  al = bee.dist.normal(1, 0.5, name='al', shape=c(1))
  ap = bee.dist.normal(-1, 1, name='ap', shape=c(1))
