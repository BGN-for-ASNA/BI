...
To discover group structures or clusters in data without pre-specifying the number of groups, we can use a **Dirichlet Process Mixture Model (DPMM)** @gershman2012tutorial. This is a [<span style="color:#0D6EFD"> unsupervised clustering method 🛈</span>]{#unsupervised}. Essentially, the model assumes the data is generated from a collection of different Gaussian distributions, and it simultaneously tries to figure out:
...
-   The key parameters and their priors are:
    ...
    - **Cluster Parameters (**$\mu$, $\Sigma$): Each potential cluster has a mean $\mu$ and a covariance matrix $\Sigma$. If the data have multiple dimensions, we use a multivariate normal distribution (see [Chapter 16: Varying Slopes Models](16.%20Varying%20slopes.qmd)). However, if the data is one-dimensional, we use a univariate normal distribution.
